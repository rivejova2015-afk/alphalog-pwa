...
-- logs: user+created_at (used in GET /api/logs pagination)
CREATE INDEX IF NOT EXISTS idx_logs_user_active_created
  ON logs (user_id, created_at DESC)
  WHERE deleted_at IS NULL;

-- logs: user+category_id (used when filtering by category)
...
CREATE INDEX IF NOT EXISTS idx_log_tags_log_id
  ON log_tags (log_id);
...
CREATE INDEX IF NOT EXISTS idx_trade_evidence_user_created
  ON trade_evidence (user_id, created_at DESC)
  WHERE deleted_at IS NULL;

-- tv_analysis_evidence: user+captured_at (legacy evidence pagination)
CREATE INDEX IF NOT EXISTS idx_tv_evidence_user_captured
  ON tv_analysis_evidence (user_id, captured_at DESC)
  WHERE deleted_at IS NULL;

-- account_categories: user active (used in account list joins)
CREATE INDEX IF NOT EXISTS idx_account_categories_user_active
