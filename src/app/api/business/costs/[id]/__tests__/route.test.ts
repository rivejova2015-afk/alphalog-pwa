// Integration tests for /api/business/costs/[id] PUT + DELETE handlers.
...
function makeRequest(opts: { method: string; body?: unknown; raw?: string }): NextRequest {
  return new NextRequest("http://localhost/api/business/costs/c-1", {
    method: opts.method,
...
function ctx(id = "c-1"): RouteParams {
...
describe("PUT /api/business/costs/[id]", () => {
  beforeEach(() => {
  ...
  it("401 cuando auth falla", async () => {
    unauthed();
    const res = await PUT(makeRequest({ method: "PUT", body: { amount: 200 } }), ctx());
  ...
  it("400 cuando amount no es positivo", async () => {
    authedUser();
    const res = await PUT(makeRequest({ method: "PUT", body: { amount: -5 } }), ctx());
    expect(res.status).toBe(400);
  });

  it("404 cuando el cost no existe", async () => {
    authedUser();
  ...
  it("Success: actualiza + audit log", async () => {
    authedUser("u-1");
    fromMock.mockReturnValue(makeUpdateSingleChain({ data: { id: "c-1", amount: 200 }, error: null }));
    const res = await PUT(makeRequest({ method: "PUT", body: { amount: 200 } }), ctx());
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.cost.amount).toBe(200);
    expect(logAuditMock).toHaveBeenCalledWith(
      expect.objectContaining({ action: "update", resourceType: "business_cost", resourceId: "c-1" }),
      expect.anything(),
    );
  });
});

describe("DELETE /api/business/costs/[id]", () => {
  beforeEach(() => {
    getUserMock.mockReset();
    fromMock.mockReset();
    logAuditMock.mockReset();
  ...
  it("Success: soft-delete + audit log", async () => {
    ...
    expect(logAuditMock).toHaveBeenCalledWith(
      expect.objectContaining({ action: "delete", resourceType: "business_cost", resourceId: "c-1" }),
      expect.anything(),
    );
  });
});
