// Integration tests for /api/business/milestones/[id] PUT + DELETE handlers.
...
function makeRequest(opts: { method: string; body?: unknown; raw?: string }): NextRequest {
  return new NextRequest("http://localhost/api/business/milestones/m-1", {
    method: opts.method,
...
function ctx(id = "m-1"): RouteParams {
...
describe("PUT /api/business/milestones/[id]", () => {
  beforeEach(() => {
  ...
  it("401 cuando auth falla", async () => {
    unauthed();
    const res = await PUT(makeRequest({ method: "PUT", body: { status: "completed" } }), ctx());
  ...
  it("400 cuando status fuera del enum permitido", async () => {
    authedUser();
    const res = await PUT(makeRequest({ method: "PUT", body: { status: "not-a-status" } }), ctx());
    expect(res.status).toBe(400);
  });

  it("404 cuando el milestone no existe", async () => {
    authedUser();
  ...
  it("Success: actualiza + audit log", async () => {
    authedUser("u-1");
    fromMock.mockReturnValue(makeUpdateSingleChain({ data: { id: "m-1", status: "completed" }, error: null }));
    const res = await PUT(makeRequest({ method: "PUT", body: { status: "completed" } }), ctx());
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.milestone.status).toBe("completed");
    expect(logAuditMock).toHaveBeenCalledWith(
      expect.objectContaining({ action: "update", resourceType: "business_milestone", resourceId: "m-1" }),
      expect.anything(),
    );
  });
});

describe("DELETE /api/business/milestones/[id]", () => {
  beforeEach(() => {
    getUserMock.mockReset();
    fromMock.mockReset();
    logAuditMock.mockReset();
  ...
  it("Success: soft-delete + audit log", async () => {
    ...
    expect(logAuditMock).toHaveBeenCalledWith(
      expect.objectContaining({ action: "delete", resourceType: "business_milestone", resourceId: "m-1" }),
      expect.anything(),
    );
  });
});
