...
export async function GET(request: NextRequest, { params }: Ctx) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select("id, symbol, range_from, range_to, params, baseline_metrics, equity_curve, final_balance, total_trades, duration_ms, monte_carlo, walk_forward, advanced, bars_loaded, created_at")
      .eq("algorithm_id", id)
      .eq("user_id", user.id)
