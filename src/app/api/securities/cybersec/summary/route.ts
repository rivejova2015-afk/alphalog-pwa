...
export async function GET() {
  try {
    ...
    const [quizQ, examQ, hwQ, progQ, stateQ] = await Promise.all([
      ...
      supabase.from("securities_user_state").select("daily_goal, milestone_snapshot, notify_streak, placement_done, gating, specialization").eq("user_id", user.id).maybeSingle(),
    ]);
    ...
    const placementDone = stateQ.data?.placement_done ?? false;
    const gating = (stateQ.data?.gating ?? "soft") as "soft" | "strict" | "block";
    const specialization = (stateQ.data?.specialization ?? null) as
      | "foundations" | "offensive" | "defensive" | "dfir" | "appsec" | "cloud" | "grc" | "infra" | null;
    ...
    return NextResponse.json(
      { xp, stats, achievements, streak, xpToday, dailyGoal, notifyStreak, placementDone, gating, specialization, sectionExams, milestones, mastery: xp.mastery },
