...
export default function SetPasswordPage() {
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        setError(err instanceof Error ? err.message : "No se pudo verificar la sesion.");
      }
    };
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage("Contraseña creada. Ya puedes iniciar sesion con email/contraseña.");
      setPassword("");
      setConfirmPassword("");
    } catch (err) {
    ...
    <main className="relative min-h-screen overflow-hidden px-6 py-12 text-slate-200">
      ...
      <section className="relative mx-auto w-full max-w-lg rounded-3xl border border-slate-700/70 bg-slate-900/80 p-8 shadow-[0_24px_60px_rgba(2,4,10,0.55)] backdrop-blur">
        ...
        <p className="mt-2 text-sm text-slate-400">
          Si ya iniciaste con Google, crea tu contraseña aqui para conservar los datos con el mismo
          email.
        </p>

        {!ready && !error && (
          <p className="mt-6 text-sm text-slate-400">Verificando sesion...</p>
        )}
        ...
        {ready && (
          <form onSubmit={handleUpdatePassword} className="mt-6 space-y-4">
            ...
            >
              {loading ? "Actualizando..." : "Crear contraseña"}
            </button>
          </form>
        )}
      </section>
