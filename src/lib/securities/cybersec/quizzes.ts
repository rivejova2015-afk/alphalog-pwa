...
export const QUIZZES: Record<number, QuizQuestion[]> = {
  ...
  29: [
    ...
    { q: "crt.sh se usa en recon para:", o: ["Enviar phishing", "Enumerar subdominios via Certificate Transparency logs (incluye staging/dev de años atrás)", "Escanear puertos"], c: 1, e: "Cada cert SSL de CA moderna se publica en logs públicos indexables." },
    { q: "subfinder combinado con httpx sirve para:", o: ["Bloquear tráfico", "Descubrir subdominios de 30+ fuentes + filtrar los que están vivos", "Cifrar la comunicación"], c: 1, e: "Pipeline moderno de ProjectDiscovery. Combinable con nuclei para scan de CVEs." },
    { q: "GitHub Dorking con `org:target-org filename:.env password` busca:", o: ["Repos vacíos", ".env files pusheados accidentalmente con credenciales en repos de la organización", "Documentación"], c: 1, e: "Uber 2016 fue exactamente esto: AWS creds en repo público → 57M records." },
    { q: "En Shodan, la query `vuln:CVE-2021-44228` devuelve:", o: ["El código del exploit", "Hosts confirmados vulnerables a Log4Shell (según fingerprint activo)", "Un tutorial"], c: 1, e: "Shodan corre banner grabbing continuo + cross-refs con CVE data." },
    { q: "En un pentest moderno, BloodHound-py se usa antes de:", o: ["Explotar vulnerabilidades", "Mapear todos los attack paths posibles en un AD comprometido con creds", "Instalar Nmap"], c: 1, e: "Post-initial-access: entender el grafo del AD antes de escalar reduce esfuerzo enorme." },
  ],
  30: [
    ...
    { q: "El NSE script `smb-vuln-ms17-010` detecta:", o: ["Un CVE de HTTP", "EternalBlue (SMB v1 vulnerability, exploit de WannaCry)", "Un DoS"], c: 1, e: "MS17-010, CVE-2017-0143. Uno de los NSE vuln scripts más críticos." },
    { q: "masscan es superior a Nmap cuando:", o: ["Necesitás service detection", "Necesitás escanear /16 o mayor a 10M pkts/sec (async I/O, no OS TCP stack)", "Escaneás localhost"], c: 1, e: "Pipeline: masscan (discovery) → Nmap sV (deep scan sobre lo abierto)." },
    { q: "El hop-limit=1 en Nmap `-f` fragmenta a:", o: ["16 bytes", "8 bytes por default (menos con --mtu)", "1 byte"], c: 1, e: "IDS sin state completo pierde fragments — bypass básico." },
    { q: "En UDP scanning, ICMP rate-limiting en Linux causa que:", o: ["Sea instantáneo", "Un UDP full scan pueda tardar 18 horas por default", "Solo funcione TCP"], c: 1, e: "Fix: `--min-rate 500 --defeat-icmp-ratelimit`." },
    { q: "El pipeline `subfinder | httpx | nuclei` sirve para:", o: ["Solo un scan", "Recon completo: subdomains → alive check → vuln scan en una línea", "Instalar deps"], c: 1, e: "ProjectDiscovery ecosystem estándar en bug bounty moderno." },
  ],
  31: [
    ...
    { q: "NetExec (nxc, ex-CrackMapExec) permite hacer:", o: ["Solo SSH", "Validar creds en masa contra 254 hosts SMB en un solo comando: `nxc smb 192.168.1.0/24 -u users -p pass`", "Cifrar archivos"], c: 1, e: "Swiss-army knife del pentest AD. Fork mantenido de CME desde 2023." },
    { q: "RID cycling en SMB null sessions permite:", o: ["Enviar phishing", "Enumerar usuarios del dominio iterando RIDs sin credentials (RID 500 = Admin, 501 = Guest, 1000+ = users)", "Cifrar traffic"], c: 1, e: "`nxc smb target -u '' -p '' --rid-brute 3000` o `impacket-lookupsid`." },
    { q: "BloodHound convierte data del AD en:", o: ["Un firewall", "Un grafo Neo4j con nodos (users/computers/groups) y edges (permisos/sesiones) para query cypher de attack paths", "Un pdf"], c: 1, e: "Query típica: `shortestPath((u:User)->(g:Group {name:'DA'}))` — encontrar camino a Domain Admin." },
    { q: "En ffuf, `-recursion -recursion-depth 3` sirve para:", o: ["Fuzzing rápido", "Fuzzing recursivo: si encuentra /admin, bucea en /admin/*, /admin/*/*, hasta 3 niveles", "Cifrar wordlist"], c: 1, e: "Feature clave para descubrir estructuras profundas. Más eficiente que gobuster secuencial." },
    { q: "Kerberoasting explota que:", o: ["Kerberos es inseguro", "Cualquier user autenticado puede pedir TGS para cualquier SPN → TGS encrypted con hash del password de la SA → crackeable offline", "El DC está caído"], c: 1, e: "hashcat -m 13100 hashes.txt rockyou.txt — passwords débiles caen en horas." },
  ],
  32: [
    ...
    { q: "Nuclei se diferencia de Nessus/OpenVAS en:", o: ["Es más lento", "Templates YAML community (8000+), async Go, focus en bug hunting y web CVEs modernas — templates disponibles <24h post-CVE", "Solo escanea SNMP"], c: 1, e: "Combinación con interactsh (OAST) permite blind detection de SSRF, XXE, Log4Shell." },
    { q: "EPSS (Exploit Prediction Scoring System) provee:", o: ["Un CVSS más granular", "La probabilidad (0-1) de que la CVE sea explotada en los próximos 30 días basado en ML", "Un exploit ready-to-use"], c: 1, e: "Cambia priorización: CVSS 9.8 + EPSS 0.001 → probablemente no explotada en la wild = menor prioridad." },
    { q: "CISA KEV (Known Exploited Vulnerabilities) lista:", o: ["Todas las CVEs", "~1200 CVEs confirmadas explotadas en la wild — US Federal debe remediar en 2-4 semanas (BOD 22-01)", "Solo web CVEs"], c: 1, e: "Cross-referencia clave con CVSS + EPSS para priorización moderna." },
    { q: "Trivy es el swiss-army knife de container security porque:", o: ["Solo escanea Docker", "Escanea imágenes, filesystems, K8s clusters, IaC (Terraform), y genera SBOMs (CycloneDX/SPDX) — todo en una tool", "Requiere licencia"], c: 1, e: "Reemplaza combinaciones de 4-5 tools legacy. Integrable en cada etapa del pipeline shift-left." },
    { q: "SBOM (Software Bill of Materials) es requerido en:", o: ["Solo open source", "US Federal procurement desde EO 14028 (post-SolarWinds/Log4Shell), UE NIS2, healthcare, finance", "Solo en Rust"], c: 1, e: "Formatos: CycloneDX (OWASP, más popular), SPDX (Linux Foundation)." },
  ],
  33: [
    ...
    { q: "En Meterpreter, el comando `migrate <PID>` sirve para:", o: ["Reiniciar el session", "Mover el session a otro proceso long-running (ej: explorer.exe) — evita que se muera cuando el user cierra el proceso original", "Cifrar"], c: 1, e: "Prioridad #1 post-explotación. Session muerto = perdiste el foothold." },
    { q: "La extension `kiwi` en Meterpreter integra:", o: ["Un editor", "Mimikatz — dump de LSA secrets, SAM hashes, Kerberos tickets, golden ticket creation", "Un game"], c: 1, e: "`load kiwi` + `creds_all` es la vía canonical para dumping credentials post-SYSTEM." },
    { q: "shikata_ga_nai encoder de MSF es en 2024:", o: ["El mejor evader", "MUY detectado por AV moderno — sirve para labs, NO engagements reales. Alternativa: Donut + ScareCrow + custom loaders Nim/Rust", "Obligatorio"], c: 1, e: "AV signatura polymorphic patterns desde años. EDR moderno lo detecta pre-execution." },
    { q: "Sliver (BishopFox, 2020) es:", o: ["Un scanner", "El C2 open source moderno más adoptado — reemplazo libre de Cobalt Strike, escrito en Go, multi-transport (mTLS/HTTPS/DNS/WireGuard)", "Un framework web"], c: 1, e: "En 2024 es el standard para pentest firms tier 2-3. Havoc (Rust/C++) es alternativa GUI-first." },
    { q: "Ligolo-ng (2023) reemplazó a proxychains porque:", o: ["Es más viejo", "Provee full TCP+UDP+ICMP via userland tun/tap con TLS/QUIC — Nmap normal funciona sin `proxychains` wrapper", "Es más lento"], c: 1, e: "Standard OSCP 2024 para chain de 3+ hops en AD segment." },
  ],
  34: [
    ...
    { q: "La extensión Burp `Autorize` sirve para:", o: ["Cifrar tráfico", "Detectar BOLA/IDOR automatizando requests con las cookies de otro user y comparando responses", "Ejecutar shellcode"], c: 1, e: "Killer feature para APIs modernas. Ahorra días de manual testing." },
    { q: "Turbo Intruder (Kettle 2018) es superior al Intruder clásico cuando:", o: ["Igual", "Necesitás race conditions o HTTP smuggling PoC — motor Jython + HTTP/2 = 10K req/sec vs 40 req/sec del clásico", "Solo para GET"], c: 1, e: "Coinbase 2019 race con Turbo Intruder → $100K bounty." },
    { q: "Burp Collaborator provee:", o: ["Un CDN", "OAST — dominios únicos que registran callbacks DNS/HTTP para detectar vulnerabilities blind (SSRF, XXE, Log4Shell)", "Un compiler"], c: 1, e: "Interactsh de ProjectDiscovery es alternativa gratis." },
    { q: "Match & Replace + Session Handling Rules en Burp permiten:", o: ["Cambio de UI", "Automation de auth flows: refresh Bearer OAuth expired, mantener CSRF tokens dinámicos, retry en 401 automático", "Solo colors"], c: 1, e: "Convierte Burp de manual a semi-automated. Setup 30 min ahorra horas en scans largos." },
    { q: "En Repeater moderno (2023+), Groups sirven para:", o: ["Cifrar tabs", "Organizar tabs en carpetas por vulnerability class (ej: 'IDOR-1', 'SSRF-1', 'SQLi-1')", "Solo colors"], c: 1, e: "Combinado con save state en .burp files, convierte Repeater en 'expediente de la vuln' completo." },
  ],
  35: [
    ...
    { q: "pspy es único porque:", o: ["Cifra procesos", "Monitorea procesos en tiempo real SIN necesitar root (polls procfs + inotify) → descubre cron jobs privilegiados invisibles en enum estático", "Solo funciona con root"], c: 1, e: "Descubre credentials pasadas en args por 30 segundos entre process spawn." },
    { q: "PwnKit (CVE-2021-4034) explota:", o: ["Un web server", "Bug de 12 años en pkexec (polkit) con argv[0]=NULL + env vars → LPE en casi todo Linux (Ubuntu/Debian/RHEL/SUSE)", "Un container"], c: 1, e: "Apocalyptic — casi 100% de Linuxes vulnerables en enero 2022." },
    { q: "DirtyPipe (CVE-2022-0847) permite:", o: ["Solo DoS", "Overwrite de cualquier archivo read-only (/etc/passwd, SUID binaries, container read-only images) via bug en pipe_buffer del kernel", "Solo network"], c: 1, e: "Kernel 5.8+ hasta 5.10.102/5.15.25/5.16.11. Un one-liner inyecta shellcode en sudo." },
    { q: "cap_setuid+ep en /usr/bin/python3 permite:", o: ["Ejecutar python rápido", "Escalada trivial: `python3 -c 'import os; os.setuid(0); os.system(\"/bin/sh\")'` → root shell sin ser SUID", "Solo lectura"], c: 1, e: "Capabilities es más difícil de detectar que SUID — no aparece en `find -perm -4000`. LinPEAS lo cubre." },
    { q: "Wildcard injection en script cron que hace `tar -cf backup.tar *`:", o: ["Nada", "Crear archivos con nombres como `--checkpoint-action=exec=sh runme.sh` → tar los interpreta como flags → RCE root", "Solo error"], c: 1, e: "Aplicable a tar, rsync, chown, chmod. Fix: sanitizar con `--` o rutas absolutas." },
  ],
  36: [
    ...
    { q: "WinPEAS es el equivalente Windows de LinPEAS y detecta:", o: ["Solo vulns web", "200+ misconfigs: unquoted service paths, weak service perms, AlwaysInstallElevated, credentials en registry, PowerShell history, PuTTY sessions", "Solo malware"], c: 1, e: "Standard OSCP 2024 Windows enumeration. Requiere AMSI bypass si usás la variante PowerShell." },
    { q: "GodPotato (2023) es superior a PrintSpoofer porque:", o: ["Es idéntico", "Usa RPC/DCOM en vez de Print Spooler → funciona incluso con Spooler off (Server 2022 hardened)", "Es más lento"], c: 1, e: "Best current option de la familia Potato en 2024. Cubre Win 8-11 + Server 2012-2022." },
    { q: "fodhelper.exe UAC bypass explota que:", o: ["Corre siempre como admin", "Es auto-elevate y lee HKCU\\Software\\Classes\\ms-settings\\Shell\\Open\\command — user medium puede escribir esa key → command hijack", "Requiere reboot"], c: 1, e: "UACME repo cataloga 78+ técnicas. Cada Windows Feature Update patches algunas." },
    { q: "PrintNightmare (CVE-2021-34527) permite:", o: ["Solo lectura", "RCE en cualquier server con Spooler (incluyendo DCs) — any user autenticado agrega impresora con driver malicioso → SYSTEM", "Solo screenshots"], c: 1, e: "Fix: deshabilitar Print Spooler en DCs + Point-and-Print restriction para non-admins." },
    { q: "BYOVD (Bring Your Own Vulnerable Driver) sirve para:", o: ["Nada", "Cargar driver legítimamente firmado pero vulnerable (RTCore64, iqvw64e, gdrv) → kernel-mode ops → bypass total de EDR user-mode", "Solo user-mode"], c: 1, e: "loldrivers.io cataloga 350+ drivers vulns. Herramientas: KDMapper, EDRSandBlast." },
  ],
  37: [
    ...
    { q: "Overpass-the-Hash (OPtH) vs Pass-the-Hash (PtH) tiene la ventaja:", o: ["Es más lento", "Usa Kerberos en vez de NTLM → funciona en dominios Kerberos-only + blend mejor (no aparece como NTLM auth event en SIEM)", "Requiere plaintext"], c: 1, e: "Rubeus asktgt /rc4:HASH /ptt es el go-to en engagements modernos." },
    { q: "Un Golden Ticket forge se hace con:", o: ["El hash NTLM del user víctima", "El hash del krbtgt account (obtenido via DCSync tras compromise de DA) — permite forge TGT para cualquier user con lifetime configurable (default 10 años!)", "SSH keys"], c: 1, e: "Fix: rotar krbtgt password 2 veces con Reset-KrbtgtKeys.ps1 invalida todos los golden tickets." },
    { q: "Diamond Ticket (SpecterOps 2022) vs Golden Ticket:", o: ["Idénticos", "Diamond MODIFICA un TGT legítimo (conserva flags/timestamps) en vez de crear desde cero → mucho más stealth contra detection avanzada", "Golden es mejor"], c: 1, e: "Golden tiene signatures conocidas (anomalías en lifetime, RC4 cuando dominio usa AES). Diamond es lo moderno." },
    { q: "DCSync (2015) explota que:", o: ["Un bug", "La replicación normal de AD — user con 'Replicating Directory Changes' pide al DC dump credentials como si fuera otro DC. DA + secretsdump = todos los hashes del dominio", "Un CVE web"], c: 1, e: "Detección: Event 4662 con Access Mask 0x00000100 desde IPs no-DC." },
    { q: "PetitPotam (2021) + Unconstrained Delegation = DC compromise porque:", o: ["Es un exploit remoto solo", "PetitPotam fuerza al DC a autenticarse hacia el server que control → server con Unconstrained Delegation captura el TGT del DC → Golden Ticket con krbtgt hash → full domain compromise", "Solo DoS"], c: 1, e: "Vector primary en < 5 minutos desde initial foothold hasta DA en muchos ambientes." },
  ],
  38: [
...
export const QUIZZES_IA: Record<number, { i?: QuizQuestion[]; a?: QuizQuestion[] }> = {
  ...
  29: {
    ...
    a: [
      ...
      { q: "amass usa 'active enumeration' cuando:", o: ["Solo consulta APIs", "Además del passive DNS de terceros, ejecuta DNS brute-force + reverse DNS + zone transfers → puede detectarse en target logs", "No usa DNS"], c: 1, e: "Trade-off velocidad vs OPSEC. `amass enum -passive` es pura consulta." },
      { q: "La HIBP Passwords API (K-Anonymity) permite:", o: ["Ver passwords", "Verificar si un password hash aparece leaked SIN revelar el password completo (envías primeros 5 chars SHA1, HIBP devuelve todos los matches de ese prefix)", "Crackear hashes"], c: 1, e: "Diseño elegante: mantiene privacidad + utilidad. Consulta rápida en pipelines de signup." },
      { q: "En BloodHound, un attack path desde owned user a Domain Admin en <5 hops indica:", o: ["Todo está bien", "Ambiente promedio — pentest típico produce 3-5 hops. >10 hops = AD bien segmentado", "Es un bug"], c: 1, e: "SpecterOps (creadores) mide 'path length' como métrica de madurez del AD." },
      { q: "Chaos-client de ProjectDiscovery mantiene un dataset de:", o: ["CVEs", "Subdominios activos de bug bounty programs, actualizados en tiempo real y consultables via API", "IPs de tor"], c: 1, e: "Muy útil en engagements: `chaos -d target.com` da subdominios ya validated." },
      { q: "Google Dork `intitle:\"index of\" (config|backup)` busca:", o: ["Un config file", "Directorios de Apache/nginx expuestos con listing habilitado que contienen 'config' o 'backup' en el nombre", "Solo backups"], c: 1, e: "Config listing accidental es hallazgo constante en pentests externos." },
    ],
  },
  30: {
    i: [
      ...
      { q: "El NSE script `vulners` requiere qué previo scan?", o: ["Nada", "-sV para tener versión de servicio — vulners cross-refs con NVD y devuelve CVEs aplicables", "Root privilege"], c: 1, e: "Combo canónico: `nmap -sV --script vulners target`." },
      { q: "En Nmap, `--min-rate 500` sirve para:", o: ["Rate limit para no crashear el target", "Forzar mínimo 500 packets/sec para speed en scans grandes", "Delay entre packets"], c: 1, e: "Útil combinado con `--defeat-icmp-ratelimit` en scans UDP en Linux." },
      { q: "masscan a `--rate 100000` puede:", o: ["Solo bajar la latencia", "Ser confundido con DDoS por el ISP y disparar throttling / abuse tickets", "Es imposible"], c: 1, e: "Usar en scope legítimo. Coordinar con network provider si escaneo agresivo IPv4." },
      { q: "Ndiff entre 2 scans del mismo target sirve para:", o: ["Comparar performance", "Detectar drift de configuración — nuevos puertos abiertos, servicios que cambiaron versión, hosts nuevos/removidos", "Cifrar"], c: 1, e: "Baseline security: `nmap -sV -oX baseline.xml target` + comparación mensual." },
      { q: "El pipeline `masscan → nmap sV` sobre 10.0.0.0/16 es superior a `nmap sV -p- 10.0.0.0/16` porque:", o: ["Es idéntico", "masscan descubre en minutos vs nmap en horas — nmap después hace deep scan solo sobre lo abierto (~99% menos work)", "Es más lento"], c: 1, e: "OSCP time-boxed uses this workflow constantly." },
    ],
    a: [
      ...
      { q: "En un NSE custom script, `portrule = shortport.port_or_service({80, 443}, {\"http\", \"https\"})` define:", o: ["Nada", "Que el script solo corre en puertos 80/443 o cuando el servicio es http/https — filtro de aplicabilidad", "El payload"], c: 1, e: "Sin portrule el script correría en todo puerto detectado — perf y seguridad." },
      { q: "HTTP/2 downgrade en el pipeline front-end → back-end abre HRS porque:", o: ["Nada", "El proxy convierte HTTP/2 a HTTP/1.1 al back-end — bugs en la conversión generan CL/TE headers inconsistentes que abren smuggling", "Es más rápido"], c: 1, e: "Kettle 2021 'HTTP/2: The Sequel is Always Worse'." },
      { q: "En Nmap, `--source-port 53` sirve para:", o: ["Solo DNS", "Aparentar tráfico como si viniera de un DNS resolver — firewalls mal configurados whitelist port 53 source y permiten paso", "Cifrar"], c: 1, e: "Bypass básico en firewalls stateless o con reglas anchas de trust." },
      { q: "El ACK scan `-sA` no descubre puertos abiertos sino:", o: ["Cifrado", "Firewalls stateful — ports que responden RST son 'unfiltered' (probablemente sin firewall), sin response son 'filtered'", "OS"], c: 1, e: "Útil para mapear políticas de firewall antes del scan real." },
      { q: "Un scan con `-f + -D RND:10 + -T2 + -g 53` combinado logra:", o: ["Solo lento", "Stealth compuesto — fragmentación + 10 decoys + timing lento + source port DNS = evade IDS con reglas simples", "Rompe TLS"], c: 1, e: "Modern IDS (Suricata, Zeek) puede aún detectar, pero eleva el bar significativamente." },
    ],
  },
  31: {
    i: [
      ...
      { q: "NetExec spider en SYSVOL busca:", o: ["Nada útil", "GPP passwords (cpassword) — atributo en preferencias GPO cifrado con AES key públicamente conocida → cracking trivial", "Wallpapers"], c: 1, e: "Impacket `secretsdump.py` extrae. MS14-025 patched el bug pero cpasswords antiguos siguen en muchos SYSVOLs." },
      { q: "El `--pass-pol` flag de NetExec sirve para:", o: ["Solo checkear password", "Ver la password policy del dominio ANTES de spraying (evita lockouts)", "Cifrar creds"], c: 1, e: "Password spraying con >1 pass/hora contra lockout threshold es un desastre en engagement." },
      { q: "El pipeline `subfinder | dnsx | httpx | nuclei` es superior a herramientas individuales porque:", o: ["Es idéntico", "Filtra en cada etapa: solo alive hosts pasan a nuclei → mucho menos tiempo perdido en muertos, resultados más accionables", "Es más lento"], c: 1, e: "ProjectDiscovery diseñado para composición Unix-style." },
      { q: "En LDAP, filtrar por `(userAccountControl:1.2.840.113556.1.4.803:=4194304)` encuentra:", o: ["Todos los users", "Users con DONT_REQ_PREAUTH → AS-REP roastable", "Passwords en clear"], c: 1, e: "1.2.840.113556.1.4.803 es el matching rule LDAP_MATCHING_RULE_BIT_AND." },
      { q: "En ffuf, `-fs 1234` sirve para:", o: ["Filtrar por tamaño de response — descartar responses del tamaño típico de 'not found' para reducir ruido", "Cifrar el request", "Solo limit"], c: 0, e: "Filtros: -fs (size), -fc (status codes), -fw (words), -fr (regex). Clave para tuning." },
    ],
    a: [
      ...
      { q: "Un cluster K8s con Kubelet API expuesto en 10250 sin auth es vulnerable a:", o: ["Nada", "RCE via `/exec` endpoint sin auth → shell en cualquier pod. Herramienta: kube-hunter detecta esto", "Solo DoS"], c: 1, e: "kubelet debería requerir client certs (--anonymous-auth=false). Common misconfig en homelab / auto-deploy." },
      { q: "NTLM relay con impacket-ntlmrelayx contra SMB sin signing puede:", o: ["Solo listar shares", "Autenticar el atacante como el user víctima contra otro servicio (LDAP → add user to Domain Admins, SMB → dump SAM)", "Cifrar"], c: 1, e: "Requiere: coercion (PetitPotam, PrinterBug) + SMB signing disabled en el target. Kerberos AES no ayuda porque NTLM se relayed independiente." },
      { q: "En Cloud enumeration, `aws s3 ls s3://bucket --no-sign-request` funciona cuando:", o: ["Siempre", "El bucket tiene ACL 'Public Read' o policy `\"Principal\": \"*\"` — anonymous access habilitado. Uber 2016 / Verizon 2017 / Accenture 2017", "Sos AWS staff"], c: 1, e: "s3scanner + cloud_enum automatizan discovery + permission check." },
      { q: "En Kerberoasting moderno, forzar AES-only (deshabilitar RC4) tiene qué impact defensivo?", o: ["Ninguno", "hashcat modo 19700 (AES) es ~100× más lento que 13100 (RC4) → mismo password crackea en meses vs horas → efectiva mitigation con user rotation policy", "Rompe Kerberos"], c: 1, e: "Muchos ambientes legacy tienen RC4 habilitado por back-compat. Ver Domain Functional Level." },
      { q: "El `--kerberoasting` flag de NetExec vs `GetUserSPNs.py` impacket:", o: ["Idéntico", "NetExec batch multiple targets + integra output; GetUserSPNs.py más granular, mejor para queries específicas single-DC", "Solo funciona en Linux"], c: 1, e: "Ambos son válidos. NetExec para automation en engagements grandes; impacket para one-off en scripts custom." },
    ],
  },
  32: {
    i: [
      ...
      { q: "Nuclei templates community están en:", o: ["Google Drive", "GitHub `projectdiscovery/nuclei-templates` — actualizados diariamente por community, categorías cves/, exposures/, misconfigurations/, default-logins/, technologies/", "Solo en la app"], c: 1, e: "8000+ templates gratis. `nuclei -update-templates` los baja." },
      { q: "El interactsh.com de ProjectDiscovery sirve para:", o: ["Un chat", "OAST (Out-of-band Application Security Testing) — dominio único por scan que registra callbacks DNS/HTTP → detección blind de SSRF, XXE, Log4Shell sin infra propia", "Un CDN"], c: 1, e: "Alternativa gratuita a Burp Collaborator." },
      { q: "En CVSS 3.1, el vector `AV:N/AC:L/PR:N/UI:N` es típico de:", o: ["Vulnerabilidad requiere acceso físico", "Vulnerabilidad ideal para el atacante — Network access, Low complexity, No privileges, No user interaction — el 'RCE not-auth' clásico", "Un DoS"], c: 1, e: "Componentes de score 9.8+ Critical típicamente tienen esta base." },
      { q: "Trivy `--severity CRITICAL,HIGH --exit-code 1` en CI:", o: ["Solo warning", "Falla el build si encuentra vulns HIGH o CRITICAL → gate en pipeline. Standard en shift-left security", "Ignora"], c: 1, e: "Combinable con `--ignore-unfixed` para reducir ruido de vulns sin fix aún disponible." },
      { q: "En SBOM CycloneDX vs SPDX:", o: ["Son incompatibles", "CycloneDX (OWASP, más moderno, VEX-friendly) vs SPDX (Linux Foundation, más viejo, license-focused). Ambos formats intercambiables con cyclonedx-cli", "Son idénticos"], c: 1, e: "Enterprise adoption: SPDX en healthcare/legal, CycloneDX en cloud/DevSecOps modernos." },
    ],
    a: [
      ...
      { q: "CVSS 4.0 introduce la métrica AT (Attack Requirements) porque:", o: ["No cambia nada", "Distingue mejor las precondiciones del target (más granular que Attack Complexity de 3.1) — mejor scoring de vulns condicionadas", "Es marketing"], c: 1, e: "Además de UI subdividida (None/Passive/Active) y nuevas Supplemental Metrics (Safety, Automatable, Recovery)." },
      { q: "VEX (Vulnerability Exploitability eXchange) declara:", o: ["Nuevas CVEs", "Para cada CVE en un componente, el status del producto (not_affected, affected, fixed) con justification — elimina alertas false positive at scale", "Solo hashes"], c: 1, e: "'vulnerable_code_not_in_execute_path' es justification común. Formal + machine-readable." },
      { q: "En un container SBOM, la distinción App vulns vs OS vulns es útil porque:", o: ["Son iguales", "OS vulns (base image) suelen ser fijeables con base image update; App vulns requieren dep upgrade. Priorización distinta", "Solo cosmético"], c: 1, e: "Trivy segrega automáticamente. Actualizar de `nginx:1.20` a `nginx:1.25` cierra muchas OS vulns transparente." },
      { q: "En un pentest engagement, un Nessus authenticated scan típicamente encuentra:", o: ["Menos vulns que sin auth", "Más vulns (100% coverage: patches, misconfigs, weak configs) vs ~30% sin auth (banner-based)", "Igual"], c: 1, e: "Diferencia enorme en value. Justifica el effort de config creds del cliente." },
      { q: "En Kubescape con framework NSA, se evalúa contra:", o: ["Solo tests custom", "NSA/CISA Kubernetes Hardening Guidance — controles como Pod Security Standards, RBAC least-privilege, network policies, secret encryption", "Random checks"], c: 1, e: "Otros frameworks: MITRE ATT&CK for K8s, ArmoBest, YAML best practices." },
    ],
  },
  33: {
    i: [
      ...
      { q: "La extension `kiwi` en Meterpreter provee acceso a:", o: ["Un juego", "Mimikatz — `creds_all`, `lsa_dump_sam`, `golden_ticket_create` — dump completo de credenciales in-memory", "Solo screenshots"], c: 1, e: "Requiere SYSTEM privileges. Con SeDebugPrivilege sin SYSTEM también funcionan varios comandos." },
      { q: "La extension `incognito` de Meterpreter sirve para:", o: ["Ocultar files", "Token manipulation — impersonate tokens de otros users logged-in (ej: DA logeado localmente)", "Cifrar"], c: 1, e: "`list_tokens -u` + `impersonate_token DOMAIN\\admin` es un vector clásico post-compromise." },
      { q: "En msfvenom, el flag `-x /path/legit.exe -k` sirve para:", o: ["Cifrar", "Inyectar el payload en un executable legítimo, keeping (-k) su original functionality — el user no nota", "Solo signing"], c: 1, e: "Vector clásico para phishing. Combinable con firma spoofing." },
      { q: "El framework Sliver (BishopFox 2020) tiene qué ventaja OPSEC vs Metasploit?", o: ["Ninguna", "Escrito en Go (menos entrenado por EDR ML), multi-transport (mTLS/HTTPS/DNS/WireGuard), sin IoCs conocidos de Meterpreter", "Es más lento"], c: 1, e: "Menor detection surface. Standard en pentest firms tier 2-3 en 2024." },
      { q: "Ligolo-ng vs Chisel vs proxychains, el moderno preferido en 2024 es:", o: ["proxychains", "Ligolo-ng — full userland tun/tap, TCP+UDP+ICMP, Nmap normal funciona sin wrapper", "Chisel"], c: 1, e: "Chisel solo TCP+SOCKS. proxychains legacy. Ligolo-ng standard OSCP 2024." },
    ],
    a: [
      ...
      { q: "Hell's Gate / SysWhispers2 evaden EDR haciendo:", o: ["Nada", "Syscalls directas (asm `syscall` instruction) bypassing los hooks user-mode que EDR instala en ntdll.dll — leen syscall numbers from disk copy of ntdll", "SQL"], c: 1, e: "EDR moderno (CrowdStrike, SentinelOne) hookea ntdll.dll for behavioral monitoring. Direct syscalls skip los hooks." },
      { q: "BYOVD (Bring Your Own Vulnerable Driver) permite:", o: ["Solo user-mode", "Cargar un driver firmado pero vulnerable (gmer.sys, procexp.sys, iqvw64e.sys) → kernel-mode operations que EDR user-mode no puede prevenir", "Nada"], c: 1, e: "Herramientas: KDMapper, EDRSandBlast. Vector usado por múltiples APTs y ransomware." },
      { q: "El AMSI bypass clásico `[Ref].Assembly.GetType('System.Management.Automation.AmsiUtils').GetField('amsiInitFailed'...` es en 2024:", o: ["Sigue funcionando", "MUY signaturado por Defender — cualquier variante literal genera alert. Bypasses modernos usan hardware breakpoints o parcheo de memory con syscalls directas", "Ilegal"], c: 1, e: "Muchas variantes públicas (Base64, string obfuscation) también en Defender signatures desde 2022." },
      { q: "Cobalt Strike Malleable C2 profiles permiten:", o: ["Solo config", "Customizar cada byte del tráfico C2 (headers, URIs, jitter, encoding) para blend con perfil legítimo (ej: Amazon Storefront) — feature superior a Sliver/Havoc", "Solo cambiar port"], c: 1, e: "Razón principal por la que Cobalt Strike sigue $$$ en red team profesional." },
      { q: "En pivoting moderno con Ligolo-ng, la ventaja QUIC vs TCP TLS es:", o: ["Idéntico", "QUIC multiplexes streams sin head-of-line blocking + integra encryption — mejor perf en NAT-heavy entornos + resiste better a network throttling", "Es más lento"], c: 1, e: "Feature de Ligolo-ng --agent-quic. Cobalt Strike no tiene esta opción built-in." },
    ],
  },
  34: {
    i: [
      ...
      { q: "En Burp, la extension Autorize configurada correctamente permite:", o: ["Renombrar tabs", "Detectar authorization issues (BOLA) automáticamente reenviando cada request con las cookies de un segundo user configurado y comparando responses", "Compilar C#"], c: 1, e: "Killer feature para engagement de APIs modernas con muchos endpoints y roles." },
      { q: "InQL (Burp extension) sirve para:", o: ["Cifrar traffic", "GraphQL testing — auto-introspection + genera queries de todos los types disponibles", "SQL injection"], c: 1, e: "Combinado con introspection habilitada en prod → bug goldmine para BOLA + info disclosure." },
      { q: "En Repeater, `Copy as curl command` es útil para:", o: ["Solo debugging", "Reports (curl one-liner por PoC), testing desde CLI, sharing con colegas sin Burp", "Cifrar"], c: 1, e: "Formato standard para bug bounty reports en HackerOne." },
      { q: "El Inspector panel de Burp Pro (2020+) decodifica automáticamente:", o: ["Solo hex", "Cookies (con SameSite/HttpOnly), URL params, JSON/multipart body, JWT decoded, Base64 auto-detected — sin necesidad de Decoder tool separada", "Solo Base64"], c: 1, e: "Ahorra clicks constantes al Decoder para manipulaciones de request en Repeater." },
      { q: "SHR (Session Handling Rules) con macro que corre `GET /csrf-token` antes de cada request permite:", o: ["Bloquear el session", "Automatizar Intruder/Scanner en endpoints protegidos con CSRF token dinámico — la macro extrae + inserta el token en cada request", "Solo GET"], c: 1, e: "Setup 30 min, ahorra horas. Alternativa: usar Turbo Intruder que puede fetch token in-script." },
    ],
    a: [
      ...
      { q: "El 'single-packet attack' de Turbo Intruder (Kettle 2020) es específico para:", o: ["Cifrar traffic", "Enviar N requests HTTP/2 en un SOLO paquete TCP → elimina network jitter → descubre races 'imposibles' con métodos tradicionales", "Solo GET"], c: 1, e: "Presentado en 'Smashing the state machine' de Kettle (Blackhat 2023)." },
      { q: "En Burp Match & Replace para eliminar CSP en response permite:", o: ["Nada", "Testing de XSS PoCs con screenshots (browser normalmente bloquearía el script inyectado), pero solo en tu Burp browser — no en la app real", "Solo debug"], c: 1, e: "Útil para demostrar impact visualmente en reports, no altera server-side security." },
      { q: "Burp Collaborator vs Interactsh vs canarytokens.org:", o: ["Idénticos", "Collaborator (Burp Pro, `oastify.com`), Interactsh (ProjectDiscovery gratis + self-hostable), canarytokens (Thinkst, focus en canarios producción)", "Solo Collaborator"], c: 1, e: "Self-hosted Interactsh es mejor OPSEC (dominios `oastify.com` bien conocidos por blue teams)." },
      { q: "El HTTP/2 tab en Repeater (Burp Pro 2022+) permite:", o: ["Solo velocidad", "Inspeccionar/editar frames HTTP/2 individuales — crítico para HRS (HTTP/2 downgrade smuggling) y algunos cache poisoning", "Solo POST"], c: 1, e: "Antes de esta feature, HTTP/2 testing requería tools externas como nghttp2." },
      { q: "En Burp SHR, `failurePolicy: Ignore` vs `Fail` para macros afecta a:", o: ["El color de UI", "Si la macro falla y `Fail`: bloquea el request (por ejemplo si el refresh Bearer no responde); `Ignore`: sigue con request original sin token refresh", "Solo logs"], c: 1, e: "Igual conceptualmente a admission controllers K8s. Config depende de criticality del auth path." },
    ],
  },
  35: {
    i: [
      ...
      { q: "LinPEAS con `-a` (all mode) vs sin flag:", o: ["Idénticos", "`-a` corre TODOS los checks (2-5 min, más ruidoso, encuentra edge cases); default es más quick y menos verbose", "-a es más lento sin razón"], c: 1, e: "En OSCP time-boxed, `-a` es worth el tiempo extra." },
      { q: "pspy detecta cron jobs privilegiados que LinPEAS no ve porque:", o: ["LinPEAS es peor", "pspy monitorea process spawning en TIEMPO REAL — cron jobs ejecutados en intervalos específicos no aparecen en enum estático de una foto snapshot", "pspy requiere root"], c: 1, e: "Feature clave: pspy no requiere root para hacer procfs polling agresivo + inotify." },
      { q: "PwnKit (CVE-2021-4034) requiere:", o: ["Root", "Solo un shell como user unprivileged — bug de 12 años en pkexec, argv[0]=NULL trigger", "Kernel exploit"], c: 1, e: "Explotable en casi 100% de Linux distros hasta enero 2022. Uno de los CVEs LPE más impactantes de la década." },
      { q: "DirtyPipe (CVE-2022-0847) overwriting SUID binary sudo funciona porque:", o: ["Nada", "Bug en pipe_buffer permite escribir a archivos read-only (incluyendo binaries SUID). Overwrite sudo con shellcode → correr sudo → root shell", "Solo container"], c: 1, e: "Kernel 5.8+ hasta 5.10.102 / 5.15.25 / 5.16.11 vulnerable." },
      { q: "El bit `+ep` en `cap_setuid+ep` significa:", o: ["Enabled+Persistent", "Effective+Permitted — la capability está activa por default cuando el binary ejecuta (no requiere que el process la activate)", "Encoding+Padding"], c: 1, e: "Otras flags: `+i` (inheritable), `+a` (ambient). Ver `man capabilities`." },
    ],
    a: [
      ...
      { q: "Baron Samedit (CVE-2021-3156) fue apocalyptic-level porque:", o: ["Solo Ubuntu", "Explotable SIN sudo permission previo — bug en parser de sudoedit, afectaba sudo 1.8.2-1.9.5p1 (casi toda instalación Linux)", "Solo Debian"], c: 1, e: "Enero 2021: descubierto por Qualys, disclosure coordinado. Un local user con shell podía escalar en 100%." },
      { q: "Wildcard injection en `chown` recursivo con root:", o: ["No es posible", "Un symlink `ln -s /etc/shadow rootfile` en dir writable + chown recursivo dereferencia → chown propietario del /etc/shadow", "Solo tar"], c: 1, e: "Aplicable a: tar, rsync, chown, chmod, cualquier command con wildcards + args interpretados." },
      { q: "cap_sys_admin+ep es 'casi root' porque:", o: ["Nombre", "Permite mount, load modules, +20 operaciones kernel — regularmente escalable a root full con mount bind del rootfs o similar", "Solo lectura"], c: 1, e: "Auditoría: cualquier binary con cap_sys_admin en `getcap -r /` requiere justificación fuerte." },
      { q: "En NFS con no_root_squash, un attacker con root local del cliente puede:", o: ["Nada", "Copiar SUID bash al share NFS montado → en el server NFS, ese SUID bash da root shell", "Solo lectura"], c: 1, e: "Fix: NFS con `root_squash` siempre (default en NFSv4). Auditar exports." },
      { q: "User namespace exploitation en Linux (unshare -U -r) fue vector para CVE-2022-0185 porque:", o: ["Nada", "Dentro del namespace, proceso es 'root' con capabilities — múltiples bugs (FSCONTEXT, OverlayFS+user_ns) permitieron kernel escape → root real", "Solo isolation"], c: 1, e: "Ubuntu post-CVE-2022-0185 muchos kernels tienen `unprivileged_userns_clone=0` por default." },
    ],
  },
  36: {
    i: [
      ...
      { q: "Seatbelt vs WinPEAS: Seatbelt tiene la ventaja de:", o: ["Ser más lento", "Escrito en C#/.NET → menos AMSI-detectable que WinPEAS.exe standalone; checks granulares como RDPSavedConnections, PuttySSH, WindowsCredentialFiles (DPAPI)", "Solo GUI"], c: 1, e: "SpecterOps mantained. Combinado con SharpDPAPI en pipelines post-compromise stealth." },
      { q: "GodPotato (2023) vs PrintSpoofer:", o: ["GodPotato es peor", "GodPotato usa RPC/DCOM en vez de Print Spooler → funciona incluso con Spooler off (Server 2022 hardened con Print Spooler disabled post-PrintNightmare)", "Idénticos"], c: 1, e: "Best current option para SeImpersonatePrivilege → SYSTEM en 2024." },
      { q: "SharpDPAPI + backup key (obtenida via DA) permite:", o: ["Solo cifrar", "Decrypt de DPAPI blobs de TODOS los users del dominio — Chrome passwords, Outlook profiles, PuTTY sessions, WiFi passwords, RDP saved creds — via SharpChrome/SharpDPAPI triage", "Solo un user"], c: 1, e: "Impact enorme: obtener domain-wide credentials cache sin tocar más de un DC." },
      { q: "PrivescCheck (PowerShell) vs WinPEAS:", o: ["Idénticos", "PrivescCheck genera HTML report elegante con --Extended flag — mejor para reports consultor formal. WinPEAS es más quick + broader coverage", "PrivescCheck es más viejo"], c: 1, e: "En engagements con delivery esperado (consultor), PrivescCheck HTML es más presentable que WinPEAS output plano." },
      { q: "fodhelper vs computerdefaults vs eventvwr UAC bypasses:", o: ["Todos idénticos", "Todos son auto-elevate binaries que leen registry keys writable por user medium — diferencian en qué key/path exploitan; ambos legítimos como técnica alternativa cuando uno es patched", "Solo fodhelper funciona"], c: 1, e: "UACME repo mantiene lista actualizada — cada Windows Update patches algunos → surface others." },
    ],
    a: [
      ...
      { q: "En 2024, `sekurlsa::logonpasswords` de mimikatz es detectado por casi todo EDR. Alternativa moderna es:", o: ["Nada funciona", "Nanodump (SpecterOps) — dump minimal, evade signature. Combinable con PPLdump para bypass LSASS PPL. O directo attack a Kerberos (Kerberoasting) evitando LSASS entirely", "Solo procdump"], c: 1, e: "OPSEC en 2024: Kerberoasting > LSASS dump. Fuerza a EDR a detectar a nivel Kerberos service tickets requests." },
      { q: "PrintNightmare (CVE-2021-34527) para RCE remoto contra un DC requiere:", o: ["Root local", "Cualquier user autenticado del dominio + Spooler running en el DC → agrega impresora con driver SMB path a share atacante → driver DLL corre como SYSTEM en el DC", "Físico acceso"], c: 1, e: "Fix: Deshabilitar Print Spooler en DCs (recomendación oficial post-PrintNightmare)." },
      { q: "BYOVD con RTCore64.sys (MSI Afterburner) es efectivo porque:", o: ["Nada", "Driver legítimamente firmado por MSI, permite DMA ilimitado a physical memory → kernel-mode operations → EDR user-mode hooks bypassed. Blocklist Microsoft tiene lag y ~150 drivers no cubiertos aún", "Requiere admin del vendor"], c: 1, e: "Ransomware 2022+ usó extensively para kill EDR pre-encryption." },
      { q: "Credential Guard en Windows 10+ mitiga LSASS dumping porque:", o: ["No mitiga", "Usa VBS/Hyper-V isolation — LSASS core está en un secure enclave (LsaIso.exe), separado de la memoria user-mode del sistema; dump de LSASS.exe main no expone secrets isolados", "Cifra el disk"], c: 1, e: "Bypassable con BYOVD → kernel-mode → attack VBS boundary. Pero mucho más difícil." },
      { q: "En Windows post-2022 hardened, dump de LSASS con procdump.exe:", o: ["Funciona", "Es DETECTED instantly por Defender for Endpoint / CrowdStrike — behavioral rule sobre process access a LSASS con GrantedAccess dump. Alternativa: nanodump, comsvcs.dll (LOLBAS con SetupCleanupTaskList), o pivotar a Kerberoasting", "Solo con admin"], c: 1, e: "Attack surface reduction rule 'Block credential stealing from LSASS' es habilitado por default en Enterprise." },
    ],
  },
  37: {
    i: [
      ...
      { q: "Rubeus asktgt vs mimikatz `sekurlsa::pth` para OPtH:", o: ["Idénticos", "Rubeus es standalone .NET (no requiere admin), request TGT fresh; mimikatz `pth` requiere admin + spawn cmd.exe con token — más ruidoso, más detectable", "mimikatz es mejor"], c: 1, e: "En 2024 Rubeus es preferred para OPSEC. Compilable con signatures modificadas también." },
      { q: "Un Silver Ticket para servicio HTTP/webserver.corp.local requiere:", o: ["Domain Admin", "Solo el hash NTLM del service account que corre IIS en ese server (Kerberoasteable o via PtH) + SID del dominio", "Physical access"], c: 1, e: "**No contacta al DC** → sin logs en el DC → más stealth que Golden Ticket." },
      { q: "PetitPotam (2021) es coercion attack que:", o: ["Cracks passwords", "Fuerza un Windows server (incluyendo DC) a autenticarse hacia una IP attacker-controlled via MS-EFSRPC — trigger para chain con Unconstrained Delegation → domain compromise", "Es solo DoS"], c: 1, e: "Fix: apply patch, deshabilitar EFSRPC en DCs, o restringir NTLM outbound." },
      { q: "SharpDPAPI backup key attack requiere previamente:", o: ["Nada", "Domain Admin equivalent (permissions para read AD's LSA secrets) — el backup key es RSA private key que descifra DPAPI blobs de cualquier user del dominio", "Solo user auth"], c: 1, e: "Impact enorme post-DA: exfil de todas las passwords de Chrome/Outlook/PuTTY de todos los employees." },
      { q: "Un attack con NetExec (nxc) `--gen-relay-list` genera:", o: ["User list", "Lista de hosts SMB con signing:False → candidatos para NTLM relay attack con impacket-ntlmrelayx (relay coerced auth desde otro host → agregar user al Domain Admins group, etc.)", "Passwords list"], c: 1, e: "Combinado con PetitPotam coercion, chain trivial en muchos ambientes internos." },
    ],
    a: [
      ...
      { q: "Golden Ticket es DETECTED en un blue team maduro por:", o: ["Nada", "Kerberos tickets con lifetime anómalo (>10h o >10 años), TGTs con RC4 cuando el dominio es AES, users con SIDs mal formateados, Event 4769 con user que 'no existe' obteniendo TGS", "Solo IDS"], c: 1, e: "Diamond Ticket (SpecterOps 2022) evade estas detections modificando TGT legítimo en vez de forge desde cero." },
      { q: "Rotar krbtgt password 2 veces (con Reset-KrbtgtKeys.ps1) es contramedida específica para:", o: ["Ransomware", "Invalidar todos los Golden Tickets outstanding. El '2 veces' asegura que se rota Password 1 → Password 2, invalidando ambos slots (Windows mantiene 2 versiones para replication)", "SQLi"], c: 1, e: "Recommendation post-DA compromise: rotar 2 veces + monitor. Sin rotate, golden tickets viven años." },
      { q: "RBCD (Resource-Based Constrained Delegation, 2019) permite:", o: ["Nada útil", "Si tenés control de ANY object con SPN + write access a msDS-AllowedToActOnBehalfOfOtherIdentity del target → arbitrary user impersonation via S4U. Elad Shamir's research", "Solo network"], c: 1, e: "Add computer account (default MS-DS-Machine-Account-Quota=10) + modify target → exploited via Rubeus s4u." },
      { q: "Certified Pre-Owned ESC1 (SpecterOps 2021) permite:", o: ["Nada", "Con cert template 'Enrollee Supplies Subject' habilitado, cualquier user puede request cert as ANYONE (incluyendo Administrator) → PKINIT → TGT → shell", "Solo local privesc"], c: 1, e: "Herramientas: Certipy (Python), Certify (C#). Fix: no 'Enrollee Supplies Subject' + Cert Manager approval." },
      { q: "DCShadow (2018) es más stealth que DCSync porque:", o: ["Es más viejo", "DCShadow registra al attacker como DC falso → hace cambios (agregar SID Enterprise Admins, etc) → replica al DC real como replication legítima. NO aparece en audit logs anómalos como DCSync", "DCSync es mejor"], c: 1, e: "Persistencia extrema. Detection: cambios de replication con serverGUID no reconocido en el forest." },
    ],
  },
  38: {
    i: [
