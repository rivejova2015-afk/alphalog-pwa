...
export const DEFINITIONS: ConceptDefinition[] = [
  ...
  // ── M13 · Windows Hardening (definitions 924-929) ───────────────────
  { id: 924, module: 13, term: "SDDL (Security Descriptor Definition Language)", short: "Formato de ACLs en Windows", explanation: "SDDL es la sintaxis para definir permisos de acceso en Windows. Estructura: Owner:Group:DACL:SACL. Cada ACE (Access Control Entry) tiene formato (type;flags;rights;object_guid;inherit_guid;account). Usado en regitro, archivos, objetos de kernel. Auditoría se hace también vía SDDL.", examples: ["SDDL de archivo: \"O:BAG:BAD:(A;;FA;;;SY)\" = Owner=Builtin Admin, Group=Builtin Admin, Allow Full Access a SYSTEM", "UAC prompts when SDDL requires elevation"], related: ["ACL", "Access Control", "Windows permissions"] },
  { id: 925, module: 13, term: "NTFS Permissions vs FAT32", short: "NTFS soporta ACLs; FAT32 no", explanation: "NTFS (New Technology File System) soporta permisos granulares por archivo/carpeta (Read, Write, Execute, Modify, Full Control, etc.) vía ACLs. FAT32 no tiene seguridad a nivel de FS — todos los archivos tienen los mismos permisos. Implicación: un USB con FAT32 cualquiera puede escribir cualquier archivo sin autenticación. Windows 10+ usa NTFS por defecto.", examples: ["Crear archivo en NTFS: icacls file.txt /inheritance:r /grant:r user:F", "USB con FAT32: cualquiera puede escribir sin ACL check"], related: ["NTFS", "File permissions", "Access Control"] },
  { id: 926, module: 13, term: "VSS (Volume Shadow Copy Service)", short: "Snapshots de archivos para backup", explanation: "VSS crea snapshots de volúmenes en puntos en el tiempo sin downtime. Usado para backups incrementales. Almacena en System Volume Information (visible solo a SYSTEM). Importante para disaster recovery pero también para malware (ransomware intenta borrar VSS copies para forzar pago; defenders usan VSS para recuperar archivos).", examples: ["Borrar VSS: vssadmin delete shadows /all (requiere admin)", "Recuperar archivo borrado: wmic shadowcopy call create"], related: ["Backup", "Disaster recovery", "Ransomware"] },
  { id: 927, module: 13, term: "Windows Defender Advanced Threat Protection (ATP)", short: "EDR de Microsoft para Windows", explanation: "ATP (ahora Microsoft Defender for Endpoint) es EDR integrado que monitorea procesos, red, archivo, registro. Detección de anomalías, sandbox de análisis, incident response. Compatible con Linux/Mac también. Requiere licencia Microsoft 365 E5 o standalone. Telemetría enviada a Azure.", examples: ["Alert de proceso de cmd.exe creado por Office (malware macro)", "Análisis de archivo sospechoso en sandbox cloud"], related: ["EDR", "Threat detection", "Incident response"] },
  { id: 928, module: 13, term: "SAM & LSA Secrets", short: "Bases de datos de credenciales Windows", explanation: "SAM (Security Account Manager) almacena hashes de usuarios locales en %SystemRoot%\\\\System32\\\\config\\\\SAM (solo readable por SYSTEM). LSA Secrets en registro guarda credenciales de servicios, cached domains, RDP keys. Ambos son targets críticos: comprometer SAM → offline cracking de passwords; comprometer LSA → reuso de credenciales.", examples: ["Dumping SAM: reg save hklm\\\\sam sam.hive → secretsdump.py", "Dumping LSA con mimikatz: sekurlsa::logonpasswords"], related: ["Password cracking", "Lateral movement", "Privilege escalation"] },
  { id: 929, module: 13, term: "Smart Card y PIV", short: "Hardware token para autenticación", explanation: "Smart Card es un hardware (tarjeta física) que almacena certificado X.509 y clave privada. PIV (Personal Identity Verification) es el standard federal NIST. Usado en: logon Windows (certificado del smart card), firma de documentos, acceso físico (RFID hybrid). Requiere PIN. Protege contra: credential theft (clave nunca sale del card), keylogging, brute force (3 intentos fallidos = lock).", examples: ["Logon Windows con smart card: Ctrl+Alt+Del → usar smart card cert", "Firma de correo con firma digital basada en smart card"], related: ["Multi-factor authentication", "PKI", "Hardware security"] },
  // ── M14 · Active Directory (definitions 930-937) ───────────────────────
  { id: 930, module: 14, term: "Domain Trusts", short: "Relaciones de confianza entre dominios", explanation: "Un forest puede tener múltiples dominios (parent-child). Las trusts definen si una cuenta de un dominio puede acceder recursos en otro. Tipos: One-way, Two-way, Transitive (confianza transitive a través de múltiples dominios), Non-transitive. Compromise de un dominio trusted = access transitivo a los demás. Auditoría critical: mapear todos los trusts.", examples: ["Trust transitivo: Corp.local confía CORP-EU.local → CORP-EU confía a user A → user A accede Corp", "Ataque: compromise CORP-EU → pivot a Corp via trust"], related: ["Forest", "Domain", "Lateral movement"] },
  { id: 931, module: 14, term: "SPN (Service Principal Name)", short: "Identificador de servicio en Kerberos", explanation: "SPN es un identificador único de un servicio en AD: format service/hostname@REALM. Ej: HTTP/webserver.corp.local@CORP.LOCAL. Los clientes Kerberos buscan el SPN en AD para pedir un TGS. Múltiples SPNs por cuenta de servicio son comunes. Kerberoasting ataca aquí: pedir TGS para SPN → crackearlo offline.", examples: ["Listar SPNs: setspn -L user", "Kerberoasting: GetUserSPNs.py para extraer TGS → hashcat"], related: ["Kerberos", "Kerberoasting", "Service accounts"] },
  { id: 932, module: 14, term: "gMSA (Group Managed Service Account)", short: "Cuenta de servicio con password automático", explanation: "gMSA es una cuenta de servicio que AD gestiona automáticamente (password rotado cada 30 días sin intervención manual). Seguro porque: password aleatoria de 240 caracteres, rotación automática, sin riesgo de password débil hardcodeado. Soporta solo for services en hosts dominio-unido. Reemplaza MSA simple (menos flexible).", examples: ["Crear gMSA: New-ADServiceAccount -Name svc-app", "Asociar a host: Add-ADComputerServiceAccount -ServiceAccount svc-app -Computer webserver01"], related: ["Service accounts", "Kerberoasting defense", "Password management"] },
  { id: 933, module: 14, term: "LAPS (Local Administrator Password Solution)", short: "Gestión automática de passwords de admin local", explanation: "LAPS es herramienta de Microsoft que almacena los passwords de admin local en AD (cifrados) con rotación automática cada X días (default 30). Cumplimiento: todos los servers tienen admin local diferente. Defensa contra: pass-the-hash (cada host tiene password único), Mimikatz offline (no password en el host, solo en AD). Requiere GPO y herramienta de lectura de AD.", examples: ["Rotar admin local: gpadmin policy apply → new password in AD", "Leer password desde AD (autorizado): Get-AdmPwd computername"], related: ["Local administrator", "Credential management", "Privilege escalation defense"] },
  { id: 934, module: 14, term: "Azure AD Connect", short: "Sincronización entre on-prem AD y Azure AD", explanation: "Azure AD Connect es agente que sincroniza usuarios/grupos de on-prem AD a Azure AD (cloud). Hash de password opcionalemente sincronizado. Permite SSO (single sign-on) entre on-prem y cloud. Risk: desempeño del servidor que corre Azure AD Connect (credential theft aquí = acceso a ambos directorios).", examples: ["Sincronizar nuevo user desde AD → aparece automáticamente en Office 365", "Azure AD Connect server comprometido → acceso a ambos directorios"], related: ["Hybrid identity", "Cloud integration", "Azure"] },
  { id: 935, module: 14, term: "RBAC en Active Directory", short: "Control de rol basado en AD", explanation: "RBAC en AD asigna permisos por rol (Domain Admins, Enterprise Admins, Backup Operators, Server Operators, Delegates, Cert Publishers, etc.). Cada rol tiene delegaciones específicas. Mínimo privilegio: usuario normal no es admin. Auditoría crítica: rescan group memberships (quien está en qué grupo privilegiado). Purple team: find creeping privileges (usuarios que acumulan permisos sin necesidad).", examples: ["Domain Admins tienen permisos ilimitados en todo el forest — access crítico", "Delegation: permitir que user X reset password solo para OU Sales"], related: ["Access control", "Delegation", "Privilege management"] },
  { id: 936, module: 14, term: "ATA & Defender for Identity", short: "Monitoreo de AD para detección de ataques", explanation: "ATA (Advanced Threat Analytics) y su sucesor Defender for Identity monitorean tráfico Kerberos/LDAP en AD buscando patrones sospechosos: Kerberoasting, brute force, lateral movement, DCSync, etc. Basado en firma y anomalía. Alertas: \"Golden Ticket generation suspected\", \"Abnormal sensitive group modification\". Requiere sensor en DC o acceso a SIEM.", examples: ["Alert: Multiple failed Kerberos TGT requests from user → possible password spray", "Alert: Unusual Kerberos ticket request for service → Kerberoasting"], related: ["Threat detection", "Kerberos", "Incident response"] },
  { id: 937, module: 14, term: "Shadow Admin", short: "Cuenta admin oculta o delegación excesiva", explanation: "Shadow Admin es un usuario no-administrativo que tiene permisos administrativos vía delegación, grupo anidado, o ACL explícita. Evasión de auditoría porque no está en Domain Admins. Ejemplo: contractor con permisos para crear users o resetear passwords sin estar en Admins. Auditoría: analizar ACLs de objetos críticos, nested groups profundos, delegaciones.", examples: ["User XYZ no está en Admins pero tiene permisos Reset Password en toda la OU → shadow admin", "Nested groups: Admins include Group A include Group B include User XYZ"], related: ["Privilege escalation", "Delegation", "Audit"] },
  // ── M15 · PowerShell (definitions 938-944) ────────────────────────────
  { id: 938, module: 15, term: "Execution Policy", short: "Restricción de scripts PowerShell", explanation: "Execution Policy controla qué scripts PowerShell se ejecutan: Restricted (ninguno), AllSigned (solo firmados), RemoteS signed (descargados deben estar firmados), Unrestricted (todos). Por política (GPO) vs usuario. No es boundary de seguridad (fácil bypass: Get-Content script.ps1 | Invoke-Expression). Defensa: combinar con AppLocker.", examples: ["Set-ExecutionPolicy Restricted → solo cmdlets, no scripts", "Bypass trivial: powershell -ExecutionPolicy Bypass -File script.ps1"], related: ["PowerShell security", "AppLocker", "Script execution"] },
  { id: 939, module: 15, term: "PowerShell Remoting (PS Remoting)", short: "Ejecución remota de comandos", explanation: "PowerShell Remoting permite ejecutar comandos en máquinas remotas (WinRM sobre HTTP(S), puerto 5985/5986). Enable-PSRemoting activa listeners. Authenticate con Kerberos (dominio) o CredSSP (riesgo credential theft). Auditación crítica: quién puede remotar, qué comandos ejecutan. Usado legítimo (admin) y malicioso (lateral movement).", examples: ["Invocar remoto: Invoke-Command -ComputerName server01 -ScriptBlock {Get-Process}", "PS Remoting session: Enter-PSSession server01"], related: ["Remote administration", "Lateral movement", "WinRM"] },
  { id: 940, module: 15, term: "DSC (Desired State Configuration)", short: "Automatización de configuración declarativa", explanation: "DSC permite definir el estado deseado de una máquina en formato declarativo (MOF file): qué servicio debe estar corriendo, qué registro debe tener cierto valor, etc. DSC Local Configuration Manager (LCM) verifica y repara drift. Uso: compliance (garantizar hardening), reparación automática. Riesgo: DSC con credential almacenada en claro en MOF (debe encriptarse).", examples: ["DSC config: servicio HTTP debe estar running → LCM reinicia si falla", "DSC pull server: múltiples clientes descargan config y la aplican"], related: ["Infrastructure automation", "Configuration management", "DevOps"] },
  { id: 941, module: 15, term: "JEA (Just Enough Administration)", short: "Delegación granular de tareas administrativas", explanation: "JEA permite delegar tareas administrativas específicas a usuarios sin ser admin local. Usa PS Remoting + role capabilities (rcbfile) que definen qué cmdlets y parámetros pueden usar. Ejemplo: permitir a helpdesk Reset-LocalUser pero no crear users. Logs completos del quién hizo qué cuándo. Defensivamente cierra acceso a herramientas poderosas (ps, Get-Member).", examples: ["JEA role para helpdesk: solo Reset-LocalUser y Set-LocalUser -PasswordNeverExpires", "Helpdesk intenta Disable-LocalUser → fail (no en capabilities)"], related: ["Least privilege", "Delegation", "Audit logging"] },
  { id: 942, module: 15, term: "PowerShell Logging", short: "Auditoría de comandos y scripts", explanation: "PowerShell génera logs en múltiples niveles: Module Logging (cada comando ejecutado, Event 4103), Script Block Logging (bloques de script, Event 4104), Transcription (sesión entera). Enviados a Security event log. Crítico para detección de malware: powershell.exe descargando payload, descifrando, ejecutando en memoria. Defense-in-depth: AMSI + logging + EDR.", examples: ["Event 4104: powershell encoded command → decode y analizar", "Transcript: Get-Content C:\\\\Users\\\\User\\\\Documents\\\\PowerShell_transcript.txt"], related: ["Threat detection", "Forensics", "AMSI"] },
  { id: 943, module: 15, term: ".NET Reflection", short: "Acceso dinámico a tipos y métodos", explanation: ".NET Reflection permite inspeccionar y invocar tipos/métodos en runtime sin importar librerías. Usado legítimo (dynamic proxies, plugins) y ofensivo (invocar métodos internos para evasión, desactivar ETW, etc.). Ejemplo: attackers usan reflection para cargar malware en memoria sin WinAPI logging.", examples: ["Reflection: [System.Reflection.Assembly]::LoadFrom('lib.dll')", "ETW bypass via reflection: [Diagnostics.Eventing.EventProvider]::Destroy()"], related: ["In-memory attack", "Evasion", ".NET internals"] },
  { id: 944, module: 15, term: "CIM (Common Information Model)", short: "Protocolo unificado para gestión", explanation: "CIM es estándar para consultar información de sistema (WMI es implementación Windows de CIM). Get-CimInstance reemplaza Get-WmiObject (deprecado en PS 7). Protocolo WSMAN (WS-Management). Útil para: inventario, auditoría, ejecución remota. Riesgo: lateral movement via CIM queries remotas a targets sin MFA.", examples: ["Get-CimInstance Win32_Process | Select ProcessId,Name,CommandLine", "WMI Event Subscription para persistencia (ejecuta cuando evento disparado)"], related: ["System management", "Remote execution", "WMI"] },
  // ── M16 · Symmetric Cryptography (definitions 945-951) ───────────────
  { id: 945, module: 16, term: "KDF (Key Derivation Functions)", short: "Derivación segura de claves", explanation: "KDF toma un secret (password, seed) y genera una o más claves. PBKDF2, bcrypt, scrypt, Argon2 son KDFs password-based (lentos a propósito). HKDF para material aleatorio (fast). Crítico para derivar claves de sesión TLS (PRF), expandir passwords débiles a claves fuertes, etc. Parámetros: iterations (cost), salt (única por instancia), output length.", examples: ["PBKDF2: 100K iterations de HMAC-SHA256 → key de 256 bits", "HKDF: material aleatorio 32 bytes → key y IV para cipher"], related: ["Password hashing", "Key management", "Cryptography basics"] },
  { id: 946, module: 16, term: "AEAD (Authenticated Encryption with Associated Data)", short: "Cifrado + autenticación en uno", explanation: "AEAD (GCM, ChaCha20-Poly1305) cifra **y** autentica en una operación. Detecta manipulación (tampering) automáticamente. Diferente a separado (cipher + HMAC) que requiere cuidado en el orden. AEAD es el standard moderno. Parámetro AAD permite autenticar datos sin cifrar (headers). Un nonce debe ser único por mensaje con misma clave (reutilización rompe GCM).", examples: ["AES-GCM: Encrypt(key, nonce, plaintext, aad) → ciphertext + tag", "Decryption: Decrypt(key, nonce, ciphertext, tag, aad) → falla si tag inválido"], related: ["Authentication", "Encryption", "Modes of operation"] },
  { id: 947, module: 16, term: "Stream vs Block Ciphers", short: "Diferencia fundamental", explanation: "Block cipher (AES) procesa datos en bloques fijos (128 bits AES). Necesita modo para datos >= bloque (CBC, CTR, GCM). Stream cipher (RC4, ChaCha20) genera keystream que XORea con plaintext bit a bit. Stream ciphers son rápido pero requieren sincronización. AES-CTR convierte un block cipher en stream cipher efectivamente. No confundir: CFB/OFB son stream-like pero basados en block cipher feedback.", examples: ["Block: AES encripta 128 bits por operación", "Stream: ChaCha20 genera 64 bytes de keystream por bloque"], related: ["Cipher modes", "Encryption", "Performance"] },
  { id: 948, module: 16, term: "IV & Nonce Requirements", short: "Inicialización y uniqueness", explanation: "IV (Initialization Vector) en CBC debe ser random e impredecible pero no necesariamente secreto. Nonce en CTR/GCM debe ser **único** por clave (reutilización = fallo catastrófico). Diferencia crucial: CBC puede reutilizar IV con diferentes keys; CTR/GCM NO. Tamaño típico: 128 bits IV (CBC), 96 bits nonce (GCM). Timestamp como nonce = peligroso (predecible).", examples: ["CBC: IV aleatorio cada mensaje, puede estar en claro", "GCM: nonce único per (key, message) — reutilizar rompe confidencialidad"], related: ["Modes of operation", "Randomness", "Cryptography"] },
  { id: 949, module: 16, term: "Padding Oracle Attack", short: "Explotación de padding en CBC", explanation: "Padding Oracle es ataque contra CBC cuando el servidor revela si padding es válido (timing, error message, etc). Atacante: manipula último byte → observa si valida → deduce byte. Iterar para cada byte = descifra todo sin clave. Mitigación: usar AEAD (no padding), MAC toda la ciphertext + padding, constant-time padding check. TLS 1.3 eliminó el riesgo usando AEAD.", examples: ["TLS < 1.0: servidor rechaza con 'Bad MAC' si padding inválido → oracle", "Padding oracle CVE: Aspnet-core, Ruby on Rails, etc."], related: ["CBC mode", "Padding", "Cryptographic attacks"] },
  { id: 950, module: 16, term: "Fast Symmetric Crypto (Hardware Acceleration)", short: "Aceleración AES con AES-NI", explanation: "CPUs modernas incluyen instrucciones AES-NI (Intel/AMD) que aceleran AES ~10x en software. TLS y bases de datos aprovechan. ChaCha20 no tiene hardware acceleration pero es competitivo en soft. GPU acceleration para GCM (Google Cloud). Relevancia: performance-critical systems (TLS terminator, DB encryption) dependen de AES-NI. Testing: verifiy /proc/cpuinfo tiene aes flag.", examples: ["Intel Core i7 con AES-NI: AES ~cycles/byte", "Sin AES-NI (VM sin emulación): AES mucho más lento"], related: ["Performance", "Hardware", "Encryption"] },
  { id: 951, module: 16, term: "Crypto Agility", short: "Capacidad de cambiar algoritmos", explanation: "Crypto Agility es capacidad de reemplazar algoritmos sin major refactor: cifrados (AES→ChaCha20), modos (CBC→GCM), hashes (SHA-1→SHA-256). Arquitectura: cipher suite abstraction, no hardcodear. Importancia: quantum computing (migrar a PQC), vulnerabilidades nuevas (cambiar fast), compliance (FIPS 140-2 vs otros). Downside: complejidad de testing.", examples: ["Abstraction: interface ICipher { Encrypt(key, data) }", "Config: cipher=AES-256-GCM vs cipher=ChaCha20-Poly1305"], related: ["Architecture", "Security by design", "Quantum computing"] },
  // ── M17 · Asymmetric Cryptography (definitions 952-958) ────────────────
  { id: 952, module: 17, term: "RSA Key Generation", short: "Generación segura de pares RSA", explanation: "RSA genera dos primos grandes (p, q) → n=p*q. Clave privada es (e,d,n); pública es (e,n). Seguridad depende de que es difícil factorizar n. Tamaño: 2048 bits mínimo (1024 roto en 2009), 4096 recomendado. Generación: usar función criptográfica random (no Mersenne Twister), verificar p y q son primos ciertos (pruebas probabilísticas). Tiempo: segundos para 2048 bits, minutos para 4096.", examples: ["openssl genrsa -out private.pem 2048", "Verificar: openssl rsa -in private.pem -check"], related: ["Asymmetric cryptography", "Key management", "Public key infrastructure"] },
  { id: 953, module: 17, term: "ECC (Elliptic Curve Cryptography)", short: "Criptografía de curva elíptica", explanation: "ECC usa puntos en una curva elíptica (y²=x³+ax+b mod p). Seguridad de 256-bit ECC ≈ 3072-bit RSA. Ventajas: claves cortas, rápidas, menos energía (móviles). Curvas: P-256 (NIST), Curve25519 (segura contra timing attacks). Desventaja: más complejo de entender, más fácil implementar mal (timing, side-channel). ECDH, ECDSA, EdDSA usan ECC.", examples: ["Curve25519: 32 bytes key, ~3072-bit RSA security", "ECDSA con P-256: solo 64-byte signature"], related: ["Asymmetric cryptography", "TLS 1.3", "Quantum resistance"] },
  { id: 954, module: 17, term: "Factorization Problem", short: "Dureza de factorizar números grandes", explanation: "RSA security depende en que es computacionalmente hard factorizar n=p*q cuando n es producto de dos primos grandes. No hay algoritmo polinomial conocido (P≠NP assumption). Mejores ataques: General Number Field Sieve (GNFS) en O(exp) time. Tamaño de bits crucial: 1024 bits factorizable en 2009 (150 core-years). Quantum: Shor's algorithm factoriza en poly time.", examples: ["2048-bit RSA: ~116 bits security (factorization ~2^116 operations)", "Post-quantum: necesita 256+ bits key against any known attack"], related: ["RSA", "Quantum computing", "Cryptanalysis"] },
  { id: 955, module: 17, term: "Discrete Logarithm Problem", short: "Dureza de encontrar exponente", explanation: "DLP: dado g, g^x mod p, encontrar x es difícil (Diffie-Hellman security). ECDLP es versión elíptica. No hay algoritmo polinomial conocido. Ataques: Pollard's rho O(sqrt(order)), index calculus. Elliptic curves con order grande (p-1 sin factores pequeños) resisten mejor. Post-quantum: ECDLP se resuelve con Shor's en poly time también.", examples: ["DH: g=2, p=23, g^x mod p = 8 → encontrar x es DLP", "Pollard's rho: O(sqrt(order)) ≈ O(2^128) para 256-bit ECDLP"], related: ["Diffie-Hellman", "Elliptic curves", "Discrete logarithm"] },
  { id: 956, module: 17, term: "ECDH & ECDHE (Forward Secrecy)", short: "Intercambio de claves con ephemeral keys", explanation: "ECDH (Elliptic Curve Diffie-Hellman) permite acordar clave compartida. ECDHE add \"ephemeral\": la clave es temporal (per-session) vs estática (per-certificate). Forward secrecy: incluso si la clave privada de long-term se compromete, sesiones pasadas están seguras (usaban ephemeral keys). TLS 1.3 siempre usa ECDHE (mandatory). Alternativa: DHE (Diffie-Hellman clásico).", examples: ["TLS 1.3 handshake: ambas partes usan ECDHE → acuerdan session key → después delete", "Compromise clave privada server → sesiones pasadas seguras gracias ECDHE"], related: ["Key exchange", "TLS", "Perfect forward secrecy"] },
  { id: 957, module: 17, term: "Digital Signatures (ECDSA, EdDSA)", short: "Autenticación mediante firma asimétrica", explanation: "Firma: hash mensaje → encriptar con clave privada. Verificación: desencriptar con pública, comparar hashes. ECDSA (Elliptic Curve DSA) estándar NIST, pero requiere random nonce correcto (k debe ser aleatorio, reutilizar k compromete clave). EdDSA (Ed25519) es más simple y seguro contra timing attacks (usa deterministic k derivado del mensaje). Ambos dan ~64 bytes signature para 256-bit key.", examples: ["ECDSA: sign(message, private_key) → signature (r, s)", "EdDSA más seguro: derivar k de hash(privkey || message)"], related: ["Non-repudiation", "Authentication", "Public key cryptography"] },
  { id: 958, module: 17, term: "Lattice-based PQC (Post-Quantum)", short: "Criptografía resistente a cuántica", explanation: "Lattice problems (Learning With Errors, Shortest Vector Problem) son difíciles incluso para computadoras cuánticas (Shor no aplica). NIST estandarizó ML-KEM (Kyber, key encapsulation) y ML-DSA (Dilithium, firmas) basados en lattices. Ventaja: proofs más fuertes que ECDLP. Desventaja: claves grandes (~1-2KB), operaciones más lentas. Transición: hybrid mode (clásico + PQC) durante 5-10 años antes de full PQC.", examples: ["ML-KEM public key: ~1184 bytes, ciphertext: ~1088 bytes", "Hybrid TLS: ECDHE_Kyber (ephemeral curve25519 + Kyber key encapsulation)"], related: ["Quantum computing", "Post-quantum cryptography", "Standardization"] },
  // ── M18 · Hashing & PKI (definitions 959-965) ────────────────────────
  { id: 959, module: 18, term: "Merkle Trees", short: "Estructura de árbol para verificación", explanation: "Merkle Tree es árbol binario donde cada hoja es hash de un dato y cada nodo interno es hash de sus hijos. Raíz es el Merkle Root. Ventaja: verificar uno de N datos requiere solo log(N) hashes (incluidos en la prueba). Usado en: Git (commits), blockchain (bitcoin), Certificate Transparency (CT logs), SBOM. Proof of inclusion: mostrar el path desde hoja a raíz.", examples: ["Bitcoin block: Merkle tree de transacciones → root en header", "Verificar tx X en block: solo necesitas hashes en el path, no todas las tx"], related: ["Hash functions", "Integrity", "Blockchain"] },
  { id: 960, module: 18, term: "X.509 Certificate Structure", short: "Formato de certificado digital", explanation: "X.509 es estándar para certificados digitales. Contiene: Subject (dominio/identidad), Issuer (CA que firmó), Validity (dates), Public Key, Extensions (v3 adds Subject Alt Name, Key Usage, etc.), y firma digital del Issuer. Versión: v1 (básica), v2 (raras), v3 (moderno, con extensions). Formato: DER (binario) o PEM (base64-encoded DER).", examples: ["Leer cert: openssl x509 -in cert.pem -text -noout", "Extensions críticas: basicConstraints, keyUsage, extendedKeyUsage"], related: ["PKI", "Certificates", "Public key infrastructure"] },
  { id: 961, module: 18, term: "CA Hierarchy & Trust Anchors", short: "Estructura de entidades de certificación", explanation: "PKI típicamente es jerarquía: Root CA → Intermediate CAs → End-entity (servidor) certificates. Root CA es el trust anchor (almacenado en OS/browser). Intermediate CAs tienen restricciones (path length, key usage) para limitar damage si comprometidas. Validación: navegar cadena desde servidor a root, verificando firmas en cada paso. Revocación: CRL o OCSP dice si certificado fue revocado.", examples: ["Chrome tiene ~130 root CAs (Google, Apple, Microsoft, etc)", "Cadena: let'sencrypt.org → R3 (intermediate) → ISRG Root X1 (root)"], related: ["Public key infrastructure", "Certificates", "Trust model"] },
  { id: 962, module: 18, term: "EV Certificates (Extended Validation)", short: "Certificados de validación extendida", explanation: "EV certificates requieren que la CA verifique identidad legal de la organización (not just domain control). Mostrar verde en navegador + organización en address bar (legacy, desapareciendo). Más caro, más process, ligeramente más seguro contra ataques. Hoy: la distinción EV es menos relevante (HTTPS es el mínimo, EV visual desapareció en Chrome 2022). Todavía usado en: banca, pagos.", examples: ["DV cert: verificar Domain ownership (email, DNS)", "EV cert: verificar corporate registration, legal entity, phone verification"], related: ["Certificates", "Validation", "Trust"] },
  { id: 963, module: 18, term: "OCSP Stapling", short: "Revocación sin latencia", explanation: "OCSP (Online Certificate Status Protocol) permite verificar si un cert fue revocado preguntando a CA online. Pero causa latencia (extra HTTP request a OCSP responder). OCSP Stapling: servidor pide OCSP response de la CA regularmente (cada 7 días) y la adjunta al handshake TLS → cliente no necesita contactar OCSP responder. Ventaja: privacidad (OCSP expone qué sitios visitas), performance (sin extra latency).", examples: ["TLS handshake include: server cert + OCSP response (firma de CA)", "Client verifica OCSP response sin contactar CA (trusts responder signature)"], related: ["Revocation", "OCSP", "Performance"] },
  { id: 964, module: 18, term: "Certificate Transparency (CT)", short: "Logs públicos de certificados", explanation: "CT es mecanismo para detectar certificados mal emitidos o fraudulentos. Toda emisión de cert registrada en CT log público (append-only, Merkle tree). Monitoreo: subscriptores pueden monitorear emisiones de su dominio. Browser: rechaza certs sin CT SCT (Signed Certificate Timestamp). Defensa contra: CAs comprometidas (Silent Triangle, Comodo hacks) registran sus fraudes públicamente (detectable).", examples: ["ct.googleapis.com, Comodo CT log, otras → todos los certs nuevos", "Monitoreo: certificate transparency log monitors (MailChimp, Google Safe Browsing)"], related: ["PKI", "Certificates", "Threat detection"] },
  { id: 965, module: 18, term: "Revocation: CRL vs OCSP vs OCSP Stapling", short: "Métodos de revocación", explanation: "CRL (Certificate Revocation List): lista completa de revocados publicada regularmente. Desventaja: grande (MB), latencia (actualización periódica). OCSP: query online a CA. Desventaja: latencia, privacy (CA ve qué sitios visitas). OCSP Stapling: servidor guarda OCSP response → adjunta. No privacy issue, sin latencia. Tendencia: browsers deprecan CRL, OCSP (confían en CT). TLS 1.3 soporta OCSP better.", examples: ["CRL: wget http://crl.ca.com/ca.crl (MB size)", "OCSP: query OCSP responder → respuesta incluye signature, nextUpdate", "OCSP Stapling: servidor renueva response cada 7 días, adjunta en handshake"], related: ["Revocation", "Certificates", "Performance"] },
  // ── M19 · TLS/SSL & VPN (definitions 966-972) ────────────────────────────
  { id: 966, module: 19, term: "Cipher Suite Selection", short: "Negociación de algoritmos en TLS", explanation: "Client y server intercambian cipher suites soportadas en ClientHello. Server elige una (o negocia). Cada suite combina: key exchange (ECDHE, DHE), auth (RSA, ECDSA), bulk cipher (AES, ChaCha20), y MAC/AEAD (SHA256, GCM). Recomendado: TLS 1.3 (AEAD solo, PFS mandatorio). Evitar: RSA key exchange (no PFS), RC4 (roto), DES/3DES (débil). Modernos: TLS_AES_256_GCM_SHA384, CHACHA20_POLY1305.", examples: ["Weak: TLS_RSA_WITH_AES_128_CBC_SHA (no PFS, CBC)", "Modern: TLS_AES_256_GCM_SHA384 (AEAD, PFS, 256-bit)"], related: ["TLS", "Encryption", "Key exchange"] },
  { id: 967, module: 19, term: "Certificate Pinning", short: "Fijación de certificado/CA", explanation: "Certificate Pinning es técnica donde cliente solo acepta un certificado/CA específico para un dominio. Dos tipos: pin-cert (exacto) o pin-pubkey (clave pública). Defensa contra: CAs comprometidas, MITM con cert válido pero diferente. Desventaja: riesgo de lockout (si cert se renueva, app falla). Usado: apps móviles (SSL pinning), apps críticas. Implementación: Custom ValidationCallBack, TrustKit, etc.", examples: ["PIN cert: app solo acepta cert de let'sencrypt.org", "PIN pubkey: app verifica clave pública antes de handshake"], related: ["TLS", "Certificate validation", "Security hardening"] },
  { id: 968, module: 19, term: "TLS 1.2 vs TLS 1.3", short: "Mejoras en la versión 1.3", explanation: "TLS 1.3 es completamente rediseñado: handshake 2-RTT → 1-RTT (más rápido). Elimina: RSA key exchange (no PFS), RC4/DES/CBC ciphers (inseguros). Mandatorio: AEAD (no CBC), PFS (ECDHE). Formato simplificado. Fallos: TLS 1.2 required fallback compatibility (legacy servers), implementaciones de 1.3 pueden tener bugs. Migración: todavía soportar 1.2 para sistemas viejos. Beneficio: security + performance.", examples: ["TLS 1.2: ClientHello → ServerHello → Cert → CertVerify → ChangeCipherSpec → Finished (múltiples RTTs)", "TLS 1.3: ClientHello with key share → ServerHello with key share + cert + done"], related: ["TLS", "Performance", "Security"] },
  { id: 969, module: 19, term: "mTLS (Mutual TLS / TLS bidireccional)", short: "Autenticación de cliente y servidor", explanation: "TLS estándar autentica solo servidor (cert). mTLS añade autenticación de cliente (client cert). Cliente tiene certificado X.509 con clave privada → presenta durante handshake. Servidor verifica. Ventaja: zero-trust (ambos se autentican), ideal para microservicios. Desventaja: complejidad (cert management para todas las apps). Usado: mesh (Istio, Linkerd), APIs internas, IoT. Renovación de cert critical (expiración = downtime).", examples: ["Istio mTLS: sidecar inyecta cert automáticamente", "Microservicio A →(mTLS)→ B: ambos verifican certs mutuamente"], related: ["Authentication", "Zero trust", "Microservices"] },
  { id: 970, module: 19, term: "IPSec Protocols (ESP, AH)", short: "Modos de seguridad a nivel red", explanation: "IPSec opera en L3 (red). Dos protocolos: ESP (Encapsulating Security Payload) — cifra y autentica datos, y AH (Authentication Header) — solo autentica. Dos modos: Transport (solo payload cifrado, IPs en claro) vs Tunnel (paquete IP entero dentro ESP, nuevas IPs). Usado: VPNs site-to-site, móvil VPN (IKEv2/IPSec). Alternativas: TLS (L4, más flexible), WireGuard (más simple).", examples: ["Transport: host A cifra TCP payload para host B", "Tunnel: host A envía paquete IP dentro ESP → llega a gateway B → desencapsula"], related: ["VPN", "Encryption", "Network security"] },
  { id: 971, module: 19, term: "WireGuard vs OpenVPN", short: "Comparación de protocolos VPN", explanation: "WireGuard: moderno, minimalista (4000 LOC vs 100K OpenVPN), criptografía fija (Curve25519, ChaCha20), rápido, auditable. Desventaja: menos flexible, no obfuscation nativa. OpenVPN: maduro, flexible, TLS basado, soporta obfuscation (evade DPI), lento. WireGuard ganar para la mayoría de casos. IPSec (IKEv2): complejo, maturo, usado en enterprise. Elección: WireGuard para usabilidad, OpenVPN para flexibility, IPSec para legacy corporate.", examples: ["WireGuard config: 4 líneas (key, endpoint, allowed-ips, keepalive)", "OpenVPN config: 50+ líneas (cipher, auth, protocol, etc.)"], related: ["VPN", "Encryption protocols", "Network security"] },
  { id: 972, module: 19, term: "VPN Leaks & DNS Leaks", short: "Filtración de tráfico en VPN", explanation: "VPN leak: tráfico que sale fuera del túnel VPN (ej. IPv6 sin encapsular, WebRTC revealing IP real). DNS leak: queries DNS van a ISP resolver, no a VPN provider (reveal domain visits). DefenseL: kill switch (bloquea tráfico si VPN cae), IPv6 leak detection, DNS leak test (dnsleaktest.com). Importancia: VPN da privacidad solo si bien configurada. Incluso good VPNs: cookies, fingerprinting, login pueden deanonymize.", examples: ["IPv6 leak: app conecta a [2001:db8::1] sin VPN", "DNS leak: curl -s https://api.ipify.org revela ISP, no VPN IP"], related: ["Privacy", "VPN", "Network security"] },
  // ── M20 · Web Security (definitions 973-979) ────────────────────────────
  { id: 973, module: 20, term: "Input Validation", short: "Verificación de datos de entrada", explanation: "Input validation es primera línea de defensa: verificar que data del usuario cumple formato esperado. Lado servidor: whitelist esperado (regex, type check, length). Lado cliente (JS) es eyecandy no-security (atacante bypassa). Critico: validar ANTES de usar (DB query, command execution). Ejemplos: email format, número de digitos, allowlist de caracteres. Evitar: blacklist (siempre incompleto), tipado implícito de lenguaje.", examples: ["Input: email → validar formato @, length <254", "Command injection: validar que parámetro es número, no comando"], related: ["Injection attacks", "Sanitization", "Secure coding"] },
  { id: 974, module: 20, term: "Output Encoding (Escaping)", short: "Neutralización en contexto de output", explanation: "Output encoding adapta string al contexto donde se renderiza para neutralizar caracteres especiales. Diferentes contextos requieren encoding diferente: HTML encode (<→&lt;), JavaScript encode (\"→\\\\\"), URL encode (space→%20), CSS encode (→\\\\). Librería: DOMPurify (HTML), OWASP Encoder, native (textContent en JS). Crítico: no confundir con encryption (encoding es transformación reversible, no segura).", examples: ["HTML: <script> → &lt;script&gt; (harmless)", "JS: \" → \\\" para que no cierre string", "URL: space → %20 para que no break path"], related: ["XSS prevention", "Injection attacks", "Secure coding"] },
  { id: 975, module: 20, term: "Authentication vs Authorization", short: "Verificación de identidad vs permisos", explanation: "Autenticación = verificar quién eres (credenciales). Autorización = verificar qué puedes hacer (permisos). Fácil confundir: \"user admin401\" es fallo de autenticación; \"user unauthorized to delete\" es autorización. Arquitectura: auth module + authz module. Modelo: RBAC (roles), ABAC (attributes), PBAC (policy-based). Cumplimiento: auditar ambas (log who tried what).", examples: ["Auth: login.com verifica password", "Authz: GET /api/admin/users rechaza si user no es admin"], related: ["Access control", "Identity management", "Audit"] },
  { id: 976, module: 20, term: "Session Management", short: "Manejo seguro de sesiones HTTP", explanation: "HTTP es stateless; sesiones se mantienen con cookies (opaco ID de sesión) o tokens (JWT, opaque). Cookies seguras: HttpOnly (no JS), Secure (solo HTTPS), SameSite (anti-CSRF). Tokens: problema — fácil de robar si mal almacenados (localStorage vulnerable a XSS). Lifetime: sesión debe expirar (server-side), refresh token pattern (short + long tokens). Revocación: blacklist, incrementing timestamp en token, server-side session store.", examples: ["Sesión: Set-Cookie: sessionID=abc123; HttpOnly; Secure; SameSite=Lax", "Token (JWT): problema de revocación → mantener issuer date para TTL"], related: ["Cookies", "Authentication", "CSRF"] },
  { id: 977, module: 20, term: "OWASP API Security Top 10", short: "Top 10 riesgos de seguridad en APIs", explanation: "OWASP publica API Security Top 10 (distinto del Web Top 10): A1 Broken Object Level Authorization (BOLA/IDOR), A2 Broken Authentication, A3 Broken Object Property Level Authorization, A4 Unrestricted Resource Consumption, A5 Broken Function Level Authorization, A6 Unrestricted Access to Sensitive Business Flows, A7 Server-Side Request Forgery (SSRF), A8 Lack of Protection from Automated Attacks, A9 Improper Assets Management, A10 Unsafe Consumption of APIs. Diferencia con Web Top 10: APIs son más simples (sin HTML rendering), más expuestas (machine-to-machine).", examples: ["A1 BOLA: GET /api/users/1 → devuelve datos user 1 sin verificar authorization", "A4 Resource: API sin rate limit → DOS"], related: ["API security", "OWASP", "REST security"] },
  { id: 978, module: 20, term: "Rate Limiting & DDoS Protection", short: "Limitación de tasa de requests", explanation: "Rate limiting restringe requests por IP/user/token en tiempo (ej. 100 req/min). Defenderse contra: brute force (login attempts), credential stuffing, scraping, DDoS. Implementación: contador en memoria (simple), Redis (distributed), WAF rules (cloud). Estrategias: token bucket, sliding window, leaky bucket. Respuesta: 429 Too Many Requests + Retry-After header. DDoS: distribuido (múltiples IPs) requiere CDN/cloud defense (Cloudflare, AWS Shield).", examples: ["Rate limit: 10 failed logins en 5 min → lockout 15 min", "API: 1000 req/day per API key → 429 si excedido"], related: ["DoS", "Brute force", "API security"] },
  { id: 979, module: 20, term: "CORS (Cross-Origin Resource Sharing)", short: "Controlar acceso cross-origin", explanation: "CORS es mecanismo para permitir cross-origin requests (normalmente bloqueados por SOP). Headers: Access-Control-Allow-Origin (qué origen), Access-Control-Allow-Methods, Access-Control-Allow-Headers, Access-Control-Allow-Credentials. Preflight: OPTIONS request para permisos. Riesgo: Allow-Origin: * con Credentials=true = catastrófico. Whitelist obligatorio, nunca reflection.", examples: ["CORS correcto: Access-Control-Allow-Origin: https://app.com", "CORS mal: Access-Control-Allow-Origin: * + Allow-Credentials: true"], related: ["Web security", "Cross-origin", "API security"] },
  // ── M21 · Container Security (definitions 980-989) ────────────────────────────
  { id: 980, module: 21, term: "Docker & Container Architecture", short: "Containerización y aislamiento de procesos", explanation: "Docker: platform para empaquetar apps + dependencias en containers (OCI image format). Container: proceso aislado con filesystem, network, PID namespace propios. Ventajas: portabilidad, aislamiento, reproducibilidad. Riesgos: si host kernel compromised, todos containers exposed; root en container = root en host si mal configurado. Layered images: cada layer es diff comprimida, reutilizable (registry sharing). Dockerfile: receta para build image (FROM, RUN, COPY, EXPOSE, CMD).", examples: ["Docker run: crea container desde image con namespaces separados", "Layering: base OS layer → app layer → config layer → ejecutable"], related: ["Containerization", "Orchestration", "Isolation"] },
  { id: 981, module: 21, term: "Kubernetes (K8s) Security Model", short: "Orquestación segura de containers", explanation: "Kubernetes: orchestrator de containers (deploy, scale, recover). Componentes: API server (control plane), kubelet (node agent), etcd (distributed state). Riesgos: misconfigured RBAC (roles), network policies (default: open), secrets en etcd sin encriptación. CIS Kubernetes Benchmark: 226 recomendaciones (90 nivel 1, 136 nivel 2). Pod Security: RestrictedPolicy (máximo strict), limitando capabilities, root access, privileged mode. Network policies: microsegmentation (default deny, explicit allow).", examples: ["RoleBinding: permisos granulares (get pods, create deployments, etc.)", "NetworkPolicy: deny all ingress, whitelist expected traffic"], related: ["Orchestration", "RBAC", "Microsegmentation"] },
  { id: 982, module: 21, term: "OCI Image Security & Scanning", short: "Validación de imágenes de container", explanation: "OCI (Open Container Initiative) image format: compatible entre Docker/Podman/others. Image scanning: análisis estático de vulnerabilidades (CVE database). Herramientas: Trivy (OSS, rápido), Grype (OSS, preciso), Aqua (enterprise). Registries: privados (ECR, Artifactory) vs públicos (Docker Hub — riesgo: typosquatting, malicious images). Signed images: notary/cosign para verificar integridad (verbiage sobre SLSA framework). Supply chain: firma en build time, validación en deploy time.", examples: ["Trivy scan image.tar: detecta 45 HIGH vulns en alpine:3.17", "cosign verify image:tag --key cosign.pub: verifica firma"], related: ["Supply chain", "Vulnerability management", "Image registry"] },
  { id: 983, module: 21, term: "Container Runtime Security", short: "Monitoreo y restricción de comportamiento runtime", explanation: "Runtime security observa comportamiento del container en ejecución (syscalls, network, file access). Herramientas: Falco (syscall monitoring + rules), Sysdig (forensics), Wazuh (agent-based). Use cases: detect privilege escalation (open capabilities), unauthorized network (C2 callback), suspicious files (crypto miners). Admission controller: webhook que rechaza containers (ej. no rootless, no liveness probe). AppArmor/SELinux: MAC (Mandatory Access Control) profiles restricting syscalls. Challenge: balance observabilidad vs performance (logging overhead).", examples: ["Falco rule: detecta 'execve' en containers cortos (crypto miners)", "Admission controller: rechaza image:latest (force explicit version tag)"], related: ["Monitoring", "Anomaly detection", "Container security"] },
  { id: 984, module: 21, term: "Secrets Management in Containers", short: "Gestión segura de credenciales en containers", explanation: "Problema: credenciales en Dockerfile/environment variables = exposed. Soluciones: externa secret store (Vault, K8s secrets — encriptado en etcd), runtime injection. K8s Secrets: base64-encoded en etcd (NO encriptado por default); fix: enable Encryption at Rest (aescbc/aesgcm). Vault: dynamic secrets (temporal, rotatable). Antis: hardcoding en image, committing .env, logging secrets (logs son recoverable). Best practice: short-lived tokens + external issuer (OIDC from K8s serviceaccount).", examples: ["Vault: app requests secret @ runtime, Vault rotates token after TTL", "K8s: SECRET=k8s_secret_name → kubelet injects into container env"], related: ["Secret management", "Credentials", "Compliance"] },
  { id: 985, module: 21, term: "Kubernetes RBAC & Service Accounts", short: "Control de acceso granular en K8s", explanation: "RBAC: Role (permisos en recursos), RoleBinding (bind role a user/SA), ClusterRole (global scope). Service accounts: identidad de pod, credencial automática montada en /var/run/secrets/kubernetes.io/serviceaccount. Riesgos: overpermissioned SAs (default: puede acceder API server), no isolation between namespaces. Best practice: least privilege (bind roles narrowly), separate SAs por workload, audit API access. TokenReview: validar tokens (útil para debugging). Nota: RBAC = acceso a K8s API, no contiene permisos de la aplicación.", examples: ["Role: allow get/list pods en namespace X", "ClusterRoleBinding: admin para namespace, user1 accede solo a pods"], related: ["Authorization", "Access control", "Kubernetes"] },
  { id: 986, module: 21, term: "Container Network Policies", short: "Segmentación de red en containers", explanation: "Por default K8s es flat network (pod-to-pod abierto). NetworkPolicy: regex-based selectors para origen/destino, protocolos. Tres tipos de policies: Ingress (tráfico que entra), Egress (tráfico que sale), selectors (label-based routing). Herramientas: Cilium (eBPF, L7-aware), Calico (L3-L4, enterprise). Use case: microservicios, backend pods NO acceden internet, frontend solo desde ingress. Challenge: debugging (policies transparentes si mal configuradas, aparenta disponible pero rechazado).", examples: ["Ingress: solo traffic from label app=web al pod database", "Egress: pods solo conectan a k8s DNS + external payment API"], related: ["Network security", "Microsegmentation", "Zero Trust"] },
  { id: 987, module: 21, term: "Container Privilege Escalation & Capabilities", short: "Evitar escalación de privilegios en containers", explanation: "Linux capabilities: granulares permisos sin ser root (CAP_NET_ADMIN, CAP_SYS_ADMIN, etc.). Container default: muchas capabilities (posibilidad de jailbreak). Defense: drop ALL, add only needed (dropCapabilities: [ALL], addCapabilities: [NET_BIND_SERVICE]). Rootless containers: docker daemon/pod corre como non-root (eliminates host root risk). Pod Security Policy (deprecated, reemplazado por Pod Security Standards): restricciones de capabilities, securityContext. Common escapes: CAP_SYS_ADMIN (mount filesystems, ptrace), CAP_NET_ADMIN (ip link, iptables).", examples: ["Pod: securityContext.capabilities.drop=[ALL].add=[NET_BIND_SERVICE]", "Rootless podman: daemon corre como user, no root"], related: ["Privilege escalation", "Linux security", "Container security"] },
  { id: 988, module: 21, term: "Kubernetes Etcd Security", short: "Protección del datastore distribuido", explanation: "etcd: persistent store de K8s (all resources, secrets, configs). Si etcd compromised: atacante accede todos datos (incluye secrets sin encriptación by default). Security: TLS entre components (kube-apiserver ↔ etcd), RBAC (etcd RBAC separado del K8s RBAC), Encryption at Rest (aesgcm en kube-apiserver). Backup: frecuent snapshots a lugar seguro. Acceso físico: no permitir acceso a etcd nodes. Nota: etcd lives en control plane (master nodes), no accesible a worker nodes por default.", examples: ["etcd encryption: --encryption-provider-config=encryption.yaml con aescbc", "etcd snapshot: etcdctl snapshot save backup.db + verify restore"], related: ["Kubernetes security", "Data protection", "Access control"] },
  { id: 989, module: 21, term: "Container Image Provenance & Signed Artifacts", short: "Verificación de origen e integridad de imágenes", explanation: "Provenance: quién construyó la imagen, cuándo, source code link, build logs. Signed images: criptographic signature (cosign, notary) verificable al deploy. SLSA framework: supply chain levels (L0 none, L1 source control, L2 build + provenance, L3 cryptographic integrity). Cosign: signs container images con private key, verifies con public key (compatible con sigstore — public transparency log). Registries (Docker/ECR/Artifactory): pueden enforcer signed images solo. Attack: unsigned image from compromised registry → malicious code deployed.", examples: ["cosign sign-blob --key cosign.key image_digest", "Admission controller: reject image unless cosign verify passes"], related: ["Supply chain", "Image security", "Trust"] },
  // ── M22 · Supply Chain Security (definitions 990-999) ────────────────────────────
  { id: 990, module: 22, term: "SBOM (Software Bill of Materials)", short: "Inventario de componentes y dependencias", explanation: "SBOM: lista de todas las dependencias (librerías, herramientas) que constituyen un software. Formatos: SPDX (estándar), CycloneDX (enfoque de seguridad), SWID. Importancia: vulnerabilidad en dependency → SBOM rápidamente revela qué apps afectadas. Generación: automática en build time (syft, cdxgen). Distribución: incluir SBOM en release (GitHub releases, artifact registry), comunicar a usuarios. Enterprise: trackerear SBOMs para todas las apps (compliance, incident response).", examples: ["SPDX: Package, PackageName, PackageDownloadLocation, ExternalRef (CPAL license)", "CycloneDX: XML con components array (name, version, vulnerabilityRating)"], related: ["Supply chain", "Dependencies", "Vulnerability management"] },
  { id: 991, module: 22, term: "Software Composition Analysis (SCA)", short: "Análisis automático de riesgos en dependencias", explanation: "SCA tools: escanean dependencies (lock files: package-lock.json, go.sum, Pipfile.lock) detectan vulnerabilidades conocidas (CVE database). Herramientas: Dependabot (GitHub nativo), Snyk (cloud SaaS), OWASP Dependency-Check (OSS). Problematic: outdated dependencies (versión 2.0 has CVE, app stuck en 1.0.5 legacy, forked + patched). Transitive deps: indirect vulnerabilities (A depends on B, B depends on C-vulnerable). Resolution: automated patch PRs, policy compliance (block usage de LGPL en proprietary code), license tracking.", examples: ["Dependabot: finds 3 vulnerabilities in npm dependencies, opens PR with updates", "Transitive: app uses lib-A v1.0 (OK), but A uses B v2.0 (CVE-2024-1234)"], related: ["Dependencies", "Vulnerability management", "Compliance"] },
  { id: 992, module: 22, term: "Dependency Confusion & Typosquatting", short: "Ataques de confusión en nombres de packages", explanation: "Dependency confusion: privado package X existe en enterprise repo, attacker publica public package X con mayor versión → package manager instala public malicious. Typosquatting: registra npm package como 'reqest' (vs 'request'), usuario tipea mal → instala malware. Defensa: private registry mirror (Nexus, Artifactory) como proxy, whitelist publishes, name resgistration (reserve similar names). npm: scoped packages (@company/pkg) en private registry. Casos históricos: 'ua-parser-js' typo, malware distribuido a millones. Risk: dev dependency (buildtime) pueden afectar distribución a usuarios.", examples: ["Confusion: attacker publishes @company/secret-cli v99.99 publicly", "Typo: npm typosquatting 'lodash' → 'lodash-es' registra nombre similar"], related: ["Supply chain", "Package management", "Malware"] },
  { id: 993, module: 22, term: "Artifact Signing & Verification", short: "Criptografía de integridad de artifacts", explanation: "Artifact: software entregable (binary, image, archive). Signing: privada key firma artifact → público key verifica firma. Estándares: Sigstore (open, free, uses Rekor transparency log), GPG (gnupg, maduro). Use cases: release binaries (GitHub releases), container images (cosign), npm packages (npm sign). Transparency log: público registro de todas las firmas (préviene re-escribir history). Challenge: key management (privada key rotación, revocation). CLI tools: cosign sign-blob, gpg --sign.", examples: ["Release binary: gpg --sign release.tar.gz → release.tar.gz.sig (verify: gpg --verify)", "npm publish: npm profile set publish-require-tfa → sign con OTP"], related: ["Supply chain", "Cryptography", "Verification"] },
  { id: 994, module: 22, term: "Build System Security & CI/CD", short: "Seguridad en pipeline de construcción", explanation: "Build system: source code → compiled artifact. Riesgos: compromised CI/CD (malware inyectado en build), no reproducible builds (same source → different output), insider threat. Security: restricted access (GitHub branch protection, CODEOWNERS), audit CI/CD logs, reproducible builds (deterministic, verifiable output). Framework: SLSA provides supply-chain-levels framework (L1 version control, L2 build system, L3 cryptographic guarantee, L4 hermetic + offline signing). Ephemeral runners: destroy después de cada build (reduces persistent compromise). Secrets: never in code, always via secrets manager (GitHub Secrets, Vault).", examples: ["GitHub: require code review + pass CI + dismiss stale before merge", "SLSA L3: build provenance in transparency log, verifiable artifact"], related: ["CI/CD", "Supply chain", "DevSecOps"] },
  { id: 995, module: 22, term: "Package Manager Security", short: "Seguridad en registries y distribución", explanation: "Package manager: distributes libraries (npm, PyPI, RubyGems, crates.io). Threats: malicious uploads, typosquatting, account takeover (weak password, no 2FA). Registry security: 2FA (PyPI mandated), signing (npm now supports), antivirus scanning (Artifactory), public transparency. PyPI 2FA (mandated 2023): protects against account hijack. crates.io: yanking (withdraw version si vulnerable). Private registries: Artifactory, Nexus, GitHub Packages para control de dependencies. Incident: event-stream npm package (4M downloads) compromised → malware in 1.4.0-1.4.4.", examples: ["PyPI: require 2FA + API token (no password) para publish", "crates.io: yank malicious version → cargo automatically retries"], related: ["Supply chain", "Package management", "Distribution"] },
  { id: 996, module: 22, term: "Source Code Provenance Tracking", short: "Trazabilidad de cambios y origen del código", explanation: "Provenance: quién escribió qué, cuándo, why (commit message). Git model: commits immutable (SHA = hash of content + parent + author + timestamp), tags signed. Verifying commits: `git verify-commit` (GPG), code signing in GitHub/GitLab (verified badge). Vulnerability disclosure: si developer X escribió vulnerable code, comunicar proactivamente. Licenses: track SPDX license per file (REUSE.software project). Forensics: git log --grep, git log -S (track when code moved/deleted). Challenge: GitHub doesn't enforce signed commits por default.", examples: ["Signed commit: git -S commit -m (requires GPG setup)", "git blame file.rs: see who wrote each line (attribute code ownership)"], related: ["Version control", "Compliance", "Audit trail"] },
  { id: 997, module: 22, term: "Supply Chain Security Frameworks (NIST, SLSA, SSDF)", short: "Estándares y mejores prácticas de seguridad", explanation: "NIST Secure Software Development Framework (SSDF v1.1): 4 main practices (PO = prepare org, PS = protect software, PO = produce well-formed, PO = prepare for deployment). SLSA (Supply-chain Levels for Software Artifacts) v1.0: Levels 1-4 based on build hermetic, provenance, reproducible. NTIA: minimum elements (version info, component repo, build records, hash). Frameworks overlap pero diferente perspectiva: NIST = process, SLSA = artifact verification, NTIA = transparency. Enterprise: mapa compliance (ISO 27001, SOC 2) + frameworks (SLSA L3 target).", examples: ["SSDF PO1.1: version control + change tracking", "SLSA L3: non-falsifiable provenance (witness sigstore transparency log)"], related: ["Compliance", "Supply chain", "Governance"] },
  { id: 998, module: 22, term: "Vulnerability Disclosure & Response", short: "Flujo seguro de comunicación de vulnerabilidades", explanation: "Responsible disclosure: finder → vendor privadamente (no public hasta fix ready). Timeline: 90 días (finder espera antes de publicar si no hay respuesta). Channels: security@company.com, HackerOne, Bugcrowd, GitHub Security Advisory. CVE: assigned por MITRE/NVD, identificación unificada. Vendor response: patch + release notes con security references (CVE-YYYY-XXXX). SLA: crítica (48h), alta (7d), media (30d). Publicación: CERT advisories, security mailing lists. Incident response: monitor CVE databases (nvd.nist.gov, cve.mitre.org), apply patches.", examples: ["Finder → security@gmail.com: 'SQLi in /api/users?search' + PoC", "90 days later if no fix: responsibly publish on Twitter/blog"], related: ["Vulnerability management", "Incident response", "Compliance"] },
  { id: 999, module: 22, term: "Third-Party Risk Management", short: "Evaluación de seguridad de proveedores", explanation: "Supply chain extends beyond own code: vendors, SaaS, APIs. Third-party risk: inherit their security posture (compromised vendor → compromise you). Assessment: security questionnaire (ISO 27001, SOC 2, penetration test reports), NDAs, liability clauses, incident notification SLA. Continuous: monitor vendor security news (news subscriptions, Cyber Threat Coalition), yearly re-assessment. Due diligence: if vendor integrates deeply (payment, auth), require higher bar (penetration test, code audit, SOC 2 Type II). Contract: breach notification clause (48h), liability cap, audit rights.", examples: ["Vendor questionnaire: 'Do you encrypt data at rest?' + evidence required", "Contract: 'Vendor notifies within 24h of any security breach'"], related: ["Supply chain", "Risk management", "Vendor management"] },
  // ── M23 · Threat Modeling & Risk Assessment (definitions 1000-1009) ────────────────────────────
  { id: 1000, module: 23, term: "STRIDE Threat Modeling Framework", short: "Metodología sistemática de identificación de amenazas", explanation: "STRIDE (Microsoft): 6 categorías de amenazas: Spoofing (forjar identidad), Tampering (modificar datos), Repudiation (negar acciones), Information Disclosure (exposición de datos), Denial of Service (indisponibilidad), Elevation of Privilege (escalación). Por cada componente (usuario, API, DB): enumerar amenazas por STRIDE. Outputs: threat list (mitigación, aceptación) + security requirements. Ventajas: sistemático, cobertura completa. Desventajas: puede ser tedioso, no prioriza por riesgo. Herramientas: Microsoft Threat Modeling Tool (free, UI), Miro templates.", examples: ["Component: login API. Spoofing: brute force attack. Repudiation: user denies login"], related: ["Threat modeling", "Risk assessment", "Security design"] },
  { id: 1001, module: 23, term: "PASTA Threat Modeling Methodology", short: "Modelo de amenazas orientado al proceso", explanation: "PASTA (Process for Attack Simulation and Threat Analysis): 7 stages. Análisis de negocio (business context) → Technical scope → application decomposition → threat analysis (vulnerabilities + controls existing) → vulnerability analysis (exploitability + impact) → attack modeling (cómo explotar) → countermeasure identification (mitigaciones). Enfoque: buscador de amenazas → defensor iterativo. Diferencia STRIDE: PASTA comienza con contexto de negocio (qué importa perder), STRIDE es bottom-up (components). Enterprise: PASTA mejor para risk-driven, STRIDE mejor para checklist.", examples: ["Stage 2: Identify assets (customer data, API keys, auth tokens)", "Stage 6: Attack tree para privilege escalation"], related: ["Threat modeling", "Risk assessment", "Attack trees"] },
  { id: 1002, module: 23, term: "Attack Trees & Attack Paths", short: "Descomposición visual de ataques", explanation: "Attack tree: root = meta goal (steal data), branches = sub-goals (authentication bypass, OR escalation). Each leaf: primitivo attack (brute force, social engineering). Markings: AND (todos deben ocurrir), OR (uno basta). Análisis: cost-benefit (defender gasta X para bloquear Y). Herramientas: draw.io, Lucidchart, or text (ATT&CK ADO library en JSON). Attack paths: secuencia de pasos (recon → phishing → credential theft → lateral movement → data exfil). Útil: training red team, comunicar riesgos a stakeholders (visual > spreadsheet).", examples: ["Attack tree: 'Access database' → (OR) 'Brute force password' / 'SQL injection' / 'Steal AWS key'", "Path: github phishing email → steal pat → merge malicious PR → production breach"], related: ["Threat modeling", "Attack analysis", "Security design"] },
  { id: 1003, module: 23, term: "Risk Assessment Methodologies", short: "Cuantificación y priorización de riesgos", explanation: "Risk = Likelihood × Impact. Qualitative (high/medium/low) vs Quantitative (probabilidad numéricas). Matrices: 3×3 (9 celdas) hasta 5×5 (25). Importancia: priorizar mitigaciones (high likelihood × high impact = critical). Frameworks: NIST (likehood, impact, risk level), CVSS (vulnerability severity 0-10), OWASP Risk Rating Methodology. Consideraciones: threat actor (insider vs external), detection capabilities (¿cuán rápido descubrimos?), business impact (how much $ loss?). Revisión: amenazas + riesgos cambian (software updates, threat landscape), revisión anual mínimo.", examples: ["Risk: SQL injection (likelihood=medium: requiere SQL knowledge, impact=critical: DB compromise) = RED risk", "Mitigation: input validation (reduce likelihood to low)"], related: ["Risk management", "Threat modeling", "Compliance"] },
  { id: 1004, module: 23, term: "Data Classification & Sensitivity Levels", short: "Categorización de datos por riesgo", explanation: "Clasificación: Public (ok expuesto), Internal (dentro org), Confidential (restricted), Secret (máximo). Ejemplos: tarjeta crédito = Secret, email interno = Internal, blog público = Public. Implicaciones: datos Secret requieren encriptación en tránsito+reposo, acceso limitado, audit log. Cumplimiento: GDPR personal data = mínimo Confidential, HIPAA medical = Secret. Implementación: DLP (Data Loss Prevention) tools detectan datos sensibles en emails/uploads (regex: credit card patterns, SSN, etc.). Challenge: sobre-clasificación (todo es Secret → fatiga), sub-clasificación (PII en Public).", examples: ["Data: customer email = Confidential (under GDPR)", "DLP rule: block upload de file conteniendo CREDIT_CARD pattern"], related: ["Data protection", "Compliance", "Risk management"] },
  { id: 1005, module: 23, term: "Risk Acceptance & Mitigation Strategy", short: "Decisión de tratamiento de riesgos", explanation: "Risk treatment options: Mitigate (reduce probabilidad/impacto), Avoid (cambiar diseño para eliminar amenaza), Transfer (insurance, outsource a provider), Accept (riesgo calculado documentado). Documentación: risk register (riesgo, owner, status, deadline). Escalation: board debe aceptar riesgos residuales (post-mitigation). Timeline: mitigation plan con deadlines, quarterly reviews. Fallacy: aceptar riesgo ≠ ignorarlo (debe monitorear, documentar razón). Ejemplo: si security audit muy caro, mitigar parcialmente (accept risk, add detective control).", examples: ["Risk: legacy system no recibe patches. Mitigation: air-gap from internet. Residual risk: unknown 0-days"], related: ["Risk management", "Decision making", "Compliance"] },
  { id: 1006, module: 23, term: "Security Baseline & Control Frameworks", short: "Estándares mínimos de seguridad", explanation: "Baseline: mínima configuración de seguridad (todos los sistemas deben cumplir). Ejemplos: TLS 1.2+, 2FA, encryption at rest, password policy. Estándares: CIS Benchmarks (199 baselines para software/OS), NIST Cybersecurity Framework (5 funciones: Identify, Protect, Detect, Respond, Recover). Frameworks: ISO 27001 (info security management), SOC 2 (cloud services), PCI-DSS (payment cards). Implementación: hardening guides (CIS Benchmarks detailed steps), automated compliance checking (Terraform modules, Kubernetes policies). Review: anual audit si cumple baseline, fallos = remediation plan.", examples: ["Baseline: All S3 buckets must have versioning + MFA delete enabled", "Audit: 2024 scan encontró 3 buckets sin versioning → 30d remediation"], related: ["Compliance", "Governance", "Security standards"] },
  { id: 1007, module: 23, term: "Threat Intelligence & Risk Trending", short: "Monitoreo de amenazas emergentes", explanation: "Threat intelligence: información sobre adversarios (TTPs, IOCs). Fuentes: vendor advisories (Microsoft, Apple), threat feeds (feeds.nvd.nist.gov, CISA), dark web monitoring, threat intel services (Mandiant, CrowdStrike reports). Trending: vulnerabilidad preciosa durante 30 días (antes exploit público), después patch Tuesday → exploitation masiva. Risk trending: track nuevas amenazas relevantes a org (ej. si usas Kubernetes → monitorear K8s CVEs). Aplicación: risk assessment actualizada trimestralmente. Participación: threat intel team → desarrollo (secure what's trendy), operaciones (IDS signatures), c-suite (board reporting).", examples: ["CVE-2024-1234 descubierto en Log4j → 3 días depois mass exploitation", "Trending: AI backdoor attacks in ML supply chain → revisamos SCA en ML dependencies"], related: ["Threat intelligence", "Risk management", "Incident response"] },
  { id: 1008, module: 23, term: "Compliance Gaps & Remediation Planning", short: "Identificación y cierre de deficiencias", explanation: "Gap analysis: audit (interno o externo) → comparar estado actual vs. requerimientos (estándar) → lista de gaps. Priorización: critical (bloqueador compliance), high (deadline soon), medium (mejora), low (nice-to-have). Remediation plan: dueño, recursos, timeline. Evidencia: documentación, pruebas (penetración test report), logs (audit trail). Compliance: SOC 2 anual, ISO 27001 cada 3 años, industry-specific (PCI-DSS yearly). Operacionalización: control nuevo debe ser sostenible (automático si posible). Retest: post-remediation verification antes marcar closed.", examples: ["Gap: GDPR requiere encryption at rest; current: datos en plain. Remediation: implementar KMS + rotate all data → 90d"], related: ["Compliance", "Audit", "Risk management"] },
  { id: 1009, module: 23, term: "Business Impact Analysis (BIA) & Continuity Planning", short: "Evaluación del impacto de disrupciones", explanation: "BIA: para cada servicio crítico, definir RTO (Recovery Time Objective = cuánto max abajo?), RPO (Recovery Point Objective = cuántos datos OK perder?). Ejemplo: API (RTO=1h, RPO=1min), email (RTO=24h, RPO=30min). Implicaciones: backup frecuencia, redundancia, failover speed. Disaster recovery (DR) plan: procedimientos, runbooks, regular drills. Business continuity (BC) plan: operación bajo conditions degradadas. Testing: anual DR drill (apagar infraestructura de verdad, validar recovery). Crisis: tiene 1 year para reconstruir SLA si cae completamente? Si no, requiere backups geográficamente distribuidos.", examples: ["RTO 1h → requiere hot standby o very fast backup restore", "RPO 5min → backups each 5min (puede ser caro)"], related: ["Risk management", "Resilience", "Compliance"] },
  // ── M24 · Forensics & Incident Response (definitions 1010-1019) ────────────────────────────
  { id: 1010, module: 24, term: "Digital Forensics & Evidence Collection", short: "Preservación y análisis de evidencia digital", explanation: "Forensics: investigación científica de crímenes digitales. Cadena de custodia (chain of custody): documentación rigurosa quién accedió qué cuándo (legal requirement). Preservación: no modificar datos originales (use copy), write-blocker device (hardware que previene writes). Volatilidad: RAM desaparece si apagamos, capturar primero (dump memory con dd, memdump tool). Volatility order: capture en secuencia CPU state → RAM → disk → network logs → etc. Tools: EnCase, FTK (Forensic Toolkit), Volatility (memory analysis), Autopsy (UI). Compliance: NIST SP 800-86 digital forensics guide.", examples: ["Memory dump: dd if=/dev/mem of=memory.dump bs=1M", "Chain of custody: evidencia log signed by investigators"], related: ["Incident response", "Forensics", "Compliance"] },
  { id: 1011, module: 24, term: "Timeline Reconstruction & Root Cause Analysis", short: "Reconstrucción cronológica del incidente", explanation: "Timeline: quién hizo qué cuándo (logs, file metadata, process history). RCA (Root Cause Analysis): no quedarse en síntoma (malware ejecutado) sino en causa raíz (how malware entered? patch missing). 5 Whys: repeatedly ask why (reducir a causa fundamental). Forensics sources: web server logs (access time), system logs (Windows Event Log, syslog), file metadata (mtime, atime, ctime), DNS queries, firewall logs. Challenge: timestamps no siempre confiables (atacante puede falsificar). Cross-referencing: corrobora con múltiples fuentes. Output: timeline report + visualization (Palantir, SplunkBase timeline).", examples: ["Timeline: 14:22 malware.exe dropped in temp folder (file metadata) → 14:23 connected to C2 (DNS log) → 14:24 began data exfil (firewall log)"], related: ["Incident response", "Forensics", "Investigation"] },
  { id: 1012, module: 24, term: "Log Analysis & Detection Engineering", short: "Análisis de logs para detección de amenazas", explanation: "Logs: source de verdad (si no logged, didn't happen desde perspectiva forensic). Tipos: access logs (quién accedió), audit logs (cambios), error logs (problemas). Detection: crear rules/queries que identifiquen IOCs (Indicators of Compromise). Tools: ELK Stack (Elasticsearch-Logstash-Kibana), Splunk (pago), Graylog (OSS). Use cases: failed login attempts (password spray), privilege escalation (sudo/runas), data exfiltration (large outbound connections). Challenge: log noise (volumen masivo dificulta detección), retention (logs grandes, storage cost). Tuning: baseline normal traffic → alertar en anomalías (statistical analysis).", examples: ["Query: find all 'cmd.exe /c whoami' in Windows Event Log (lateral movement indicator)", "Alert: >10 failed logins en 5 min for user X → possible brute force"], related: ["Detection", "SIEM", "Incident response"] },
  { id: 1013, module: 24, term: "Malware Analysis & Reverse Engineering", short: "Descomposición técnica de malware", explanation: "Malware analysis: estudio de comportamiento (funcionalidad, IOCs). Static: examine code sin ejecutar (strings, disassembly con IDA, Ghidra). Dynamic: ejecutar malware en sandbox seguro (Cuckoo, Any.run, VirusTotal), monitorear syscalls/network. Obfuscation: malware intenta evadir análisis (packed, encrypted, anti-debug). Deobfuscation: unpack, decrypt, patch anti-debug hooks. Tools: Ghidra (NSA reverse engineering), IDA Pro (pago, industry standard), radare2 (OSS), Wireshark (network). Honeypot: fake services (HTTP server, SMB share) que log malware intentos. Frameworks: MITRE ATT&CK mapear técnicas.", examples: ["String analysis: malware contiene 'cmd.exe /c del %temp%' + C2 IP", "Dynamic: malware intenta escribir en registry HKLM (requires admin) → UAC bypass"], related: ["Malware", "Reverse engineering", "Forensics"] },
  { id: 1014, module: 24, term: "Incident Response Plan & Playbooks", short: "Procedimientos documentados para respuesta a incidentes", explanation: "IR plan: roles (incident commander, analyst, comms), escalation matrix, communication channels (war room, Slack channel). Playbook: paso a paso detallado (ej. phishing playbook: preserve emails → scan for IOCs → identify affected users → disable accounts → rotate passwords). Stages: Detection → Triage (¿es verdadero positivo?) → Containment (stop spread) → Investigation (RCA) → Recovery (restore systems) → Lessons learned. Automation: scripts que ejecutan pasos comunes (block IOCs, kill processes, backup logs). Testing: IR drills trimestral (tabletop, full simulation). Contactos: vendor support, law enforcement (FBI en US), cyber insurance (requieren IR plan). SLA: critical = initial response en 1h, status updates cada 2h.", examples: ["Phishing playbook: if subject=PayPal and attachment=.exe → triage as malware", "Ransomware playbook: detect → isolate affected systems → notify backup → assess damage → decide restore vs negotiate"], related: ["Incident response", "Readiness", "Resilience"] },
  { id: 1015, module: 24, term: "Indicators of Compromise (IOCs) & Intelligence", short: "Marcadores técnicos de actividad maliciosa", explanation: "IOCs: IP addresses (C2 server), domains (phishing), file hashes (malware samples), email addresses, URLs. Formatos: STIX (Structured Threat Information Expression), OpenIOC, YARA rules (pattern matching para files/network). Sharing: MISP (Malware Information Sharing Platform), X-Force Exchange, Shodan queries. False positives: IP puede ser legitimo CDN, necessita contexto. TTL: IOC valid period (IP puede cambiar owner después de semana, domain registrado para short campaign). Automation: feed IOCs en IDS/firewall/EDR → block traffic automáticamente. Dark web monitoring: hunt IOCs before criminals use them (proactive).", examples: ["IOC: hash=sha256_of_malware, domain=attacker-c2.ru, IP=1.2.3.4", "YARA rule: detect ransomware encryption behavior across file system"], related: ["Threat intelligence", "Detection", "Incident response"] },
  { id: 1016, module: 24, term: "Forensic Imaging & Preservation Techniques", short: "Copia forense y conservación de evidencia", explanation: "Forensic image: bit-by-bit copia de storage (disk, USB, phone). Herramientas: dd (Unix), dcfldd (mejor: con hash verification), FTK Imager, EnCase. Hashing: SHA-256 de imagen original vs copia para verificar integridad (si hash mismatch: data corruption durante transfer). Write blocker: hardware device que permite reads pero no writes (legal defensibility). Almacenamiento: image guardada en secure location (encrypted, access-logged). Chain of custody form: nombre analyst, fecha, hora, método, resultado. Challenge: tamaño (terabytes de data = lento, caro storage). Compression: puede comprimir image pero afecta metadata.", examples: ["Disk image: dd if=/dev/sda of=disk.img bs=512 conv=noerror + sha256sum verify", "USB forensics: USB blocker device → read-only copy → analyze en sandbox"], related: ["Forensics", "Evidence collection", "Compliance"] },
  { id: 1017, module: 24, term: "Post-Incident Reporting & Lessons Learned", short: "Documentación y mejora post-incidente", explanation: "Post-incident report: resumen ejecutivo (qué pasó, impacto), technical findings (causa raíz, IOCs), timeline, impact (systems affected, data accessed), response actions (what we did). Lessons learned meeting (blameless): qué funcionó, qué no, cómo evitar próxima vez (foco en proceso, no personas). Action items: preventivas (patch CVE), detective (alertas en logs), corrective (cambiar proceso). Communication: transparente con stakeholders (user notification obligatorio en data breaches bajo GDPR). Incident rating: severity 1-5 para establecer tracking vs crítica. Database: track incidents (pattern analysis, trending improvements).", examples: ["Report: trojan en workstation (RCA: user clicked phishing link). Action item: increase phishing awareness training", "Lessons: EDR was disabled due to performance issues → mitigation: upgrade hardware"], related: ["Incident response", "Improvement", "Compliance"] },
  { id: 1018, module: 24, term: "Insider Threat Detection", short: "Identificación de amenazas desde adentro", explanation: "Insider threat: empleado, contratista, ex-empleado con acceso. Motivaciones: dinero, venganza, coerción, negligencia. Indicadores: acceso inusual (datos sensibles después de horas), download masivo, VPN conexión desde extraño país, lateral movement, printing sensitivo files. Tools: UBA (User Behavior Analytics, detecta anomalías), DLP (data exfil detection), SIEM (log analysis). Programa: awareness training (qué es insider threat), policies (document access, clean desk), seguridad física (badges, surveillance). Operacionalización: investigation discretamente, abogados informados, documentar bien. Privacy: balance vigilancia vs privacidad (empleados pueden sentirse acosados).", examples: ["Alert: user downloading entire customer database a las 2am (después de LinkedIn profile update)", "Insider threat: contractor copia source code a USB after 2 weeks notice"], related: ["Insider threat", "Detection", "User behavior"] },
  { id: 1019, module: 24, term: "Ransomware Response Strategy", short: "Defensa y recuperación de ransomware", explanation: "Ransomware: encrypt datos + demanda pago. Respuesta: NO PAGAR (financia criminalidad, no garantiza recovery), aislamiento de sistemas infectados, backup restore (si tienes limpio backup, ignora demanda). Prevención: backup offline (no accesible desde network), MFA, EDR (detect lateral movement), patch systems. Recovery: restore desde backup pre-infection, verificar backup limpio (checksum, date range), test antes de producción. Extensión: ransomware ahora hace doble extorsión (encrypt + exfil + amenaza publicar). Readiness: plan antes (RTO, RPO, backup strategy). Leyes: algunos gobiernos prohiben negociar con criminalidad.", examples: ["Ransomware detected → isolate segment → check backups → restore all data → investigate (forensics)", "No backup → opciones: negotiate (riesgoso), pagar (financia), accept loss (ouch)"], related: ["Ransomware", "Backup", "Resilience"] },
  // ── M25 · Threat Intelligence & MITRE ATT&CK Framework (definitions 1020-1029) ────────────────────────────
  { id: 1020, module: 25, term: "MITRE ATT&CK Framework", short: "Matriz de tácticas y técnicas de adversarios", explanation: "ATT&CK (Adversarial Tactics, Techniques & Common Knowledge): base de conocimiento de técnicas reales usadas por adversarios. Estructura: 13 tácticas (Reconnaissance, Resource Development, Initial Access, Execution, Persistence, Privilege Escalation, Defense Evasion, Credential Access, Discovery, Lateral Movement, Collection, Exfiltration, Impact). Cada táctica contiene técnicas (ej. Execution: Command and Scripting Interpreter, User Execution, etc.). Sub-técnicas: detalles más específicos (PowerShell, cmd, bash). Plataformas: Enterprise (Windows/Mac/Linux), Mobile (iOS/Android), ICS (Industrial Control Systems). Uso: map incidents a técnicas → identify gaps en detección → build playbooks.", examples: ["T1059: Command and Scripting Interpreter (sub: T1059.001 PowerShell)", "Adversary: APT28 primarily uses T1571 (non-standard port C2), T1105 (ingress tool transfer)"], related: ["Threat intelligence", "Adversary tactics", "Detection"] },
  { id: 1021, module: 25, term: "Tactics vs Techniques in Adversary Behavior", short: "Diferencia entre objetivos tácticos y métodos específicos", explanation: "Tácticas: objetivos de alto nivel (¿qué intenta lograr el atacante en esta fase?). Técnicas: método específico para lograr táctica. Ejemplos: Táctica = Credential Access (robar credenciales), Técnicas = Brute Force, Phishing, Keylogging, etc. Sub-técnicas: aún más específicas (Phishing → Phishing: Spearphishing Link, Phishing: Spearphishing Attachment). Enterprise mapping: versión reducida (~188 técnicas) vs mobile (~104) vs ICS (~197). Understanding: patrón de ataque = secuencia tácticas (Recon → Initial Access → Persistence → Privilege Escalation → Exfil). Tool usage: Caldera, VECTR (tracking detections).", examples: ["Tactic: Lateral Movement. Technique: T1021.001 (RDP), T1021.003 (Distributed Component Object Model)", "Campaign: attacker chains T1589 (gather victim org info) + T1598 (phishing) + T1078 (valid account)"], related: ["Threat intelligence", "Attack chains", "Adversary tactics"] },
  { id: 1022, module: 25, term: "Threat Actor Profiling & Attribution", short: "Identificación y caracterización de adversarios", explanation: "Attribution: atribuir ataque a threat actor específico (APT, criminal group, nation-state). Evidencia: TTPs (Tactics, Techniques, Procedures), malware code similarities, infrastructure reuse, language clues, timing (sleep schedules revelan timezone). Grupos conocidos: APT28 (Russia/Fancy Bear), Lazarus (North Korea), FIN7 (cybercrime). Motivaciones: espionaje (nation-states), dinero (cybercrime), hacktivismo (cause). Attribution desafío: falsos positivos (atacante usa técnicas de otro grupo propositamente), evolución (grupos cambian herramientas). Implicancia: respuesta diferente (si nation-state: alerta gobierno, vs cybercrime: involve ley). Inteligencia: reportes de vendors (Mandiant, CrowdStrike) publican análisis.", examples: ["APT28: usa malware Sofacy + PowerShell scripts + forja de documentos Office", "Attribution error: atacante intentionally usa técnicas de APT1 para crear falso flag"], related: ["Threat intelligence", "Adversary", "Attribution"] },
  { id: 1023, module: 25, term: "Threat Campaigns & Intrusion Sets", short: "Grupos organizados y campañas coordinadas", explanation: "Campaign: conjunto de ataques coordinados (misma víctima o industria) por mismo actor en tiempo. Intrusion Set: grupo persistente con identidad, objetivos, TTPs consistentes. Diferencia: set = grupos, campaign = acciones temporales (set X realizó campaign Y en 2024). Tracking: dar nombre (Mandiant: FIN1, CrowdStrike: Anchor, MISP: uuid). Kampanye histórica: SolarWinds supply chain (2020, Cozy Bear/APT29), Emotet (malware-as-a-service distribuida por múltiples grupos). Evolución: grupos se adaptan (defensas del target cambian → técnicas nuevas). Intelligence sharing: threat intel communities publican IOCs, TTPs de campaigns conocidas.", examples: ["Campaign: multiple victims en telecom/defense reciben phishing con Cobaltstrike", "Intrusion Set: Wizardspider (Conti ransomware gang) historically targeted healthcare"], related: ["Threat intelligence", "Adversary", "Campaign tracking"] },
  { id: 1024, module: 25, term: "Indicators of Compromise (IOCs) Intelligence Feeds", short: "Fuentes y distribución de inteligencia", explanation: "IOCs: IP, domain, hash, email, URL que indican actividad maliciosa. Feeds: listas auto-actualizadas de IOCs (comercial o gratuitas). Fuentes: vendor (Microsoft, Google, Cloudflare), government (CISA), industry (financial sector ISACs). Calidad: varía (falsos positivos frecuentes), vet antes usar en blockings. Formatos: JSON, CSV, STIX (XML). Integration: IDS (Suricata, Snort), firewall, EDR, SIEM. Timeliness: IOC viejo puede ser obsoleto (IP reciclada a ISP legítima). Relación MITRE ATT&CK: IOCs + técnicas = better context que IOCs solo.", examples: ["Feed: CrowdStrike Intelligence feed con APT28 IOCs updated daily", "CISA alerts: distribute IOCs de malware campaigns within 24h"], related: ["Threat intelligence", "IOCs", "Detection"] },
  { id: 1025, module: 25, term: "Threat Intelligence Platforms & Tools", short: "Tecnologías para gestión de inteligencia", explanation: "TIP (Threat Intelligence Platform): centraliza IOCs, reports, feeds. Ejemplos: Anomali ThreatStream, Google Chronicle (SIEM + threat intel), Flashpoint, recording.com. Funcionalidad: ingest feeds → deduplicate → normalize → score (confidence, severity) → distribute a tools (firewall, IDS, SOAR). SOAR (Security Orchestration, Automation and Response): orquesta respuesta (ej. IOC en feed → block IP → alert team → crear ticket). Analista experience: dashboards (trending threats), reporting (executive summary), collaboration (notes on IOC). Enterprise: MISP (open, self-hosted) vs pago (better UX). Integrations: slack, Jira, ticketing systems.", examples: ["TIP ingest: IP added to feed → SIEM query historical logs automatically", "SOAR: phishing email detected → extract IOC → block sender → disable account → notify user"], related: ["Threat intelligence", "SIEM", "Incident response"] },
  { id: 1026, module: 25, term: "Cyber Threat Coalition & Information Sharing", short: "Comunidades colaborativas de inteligencia", explanation: "Threat data sharing: industria se beneficia de compartir IOCs (early warning, faster response). Iniciativas: ISACs (Information Sharing and Analysis Centers, por industria: financial FSISAC, healthcare H-ISAC), STIX/TAXII (data formats + protocols para info sharing). Retos: privacidad (qué información revelar?), confianza (partner es legítimo?), compliance (GDPR limita sharing). Gratuitas vs. pagadas: CISA (gratuita, gobierno), AbuseIPDB (crowdsourced), Twitter/Reddit (community), pago (vendor reports). Transparency: companias publican threat reports (moral duty, marketing) pero withholding tactical details (OPSEC).", examples: ["FSISAC member bank shares APT info with peers within 24h", "Twitter: security researchers publish 0-day IOCs, community validates"], related: ["Threat intelligence", "Information sharing", "Community"] },
  { id: 1027, module: 25, term: "Threat Hunting & Proactive Detection", short: "Búsqueda activa de adversarios en la red", explanation: "Threat hunting: proactivamente buscar IOCs/anomalías sin alertas (contrario a reactive detection). Hypotesis-driven: sospechar X tactic/técnica en network → query logs/telemetry → corrobora o descarta. Tools: SIEM queries (ELK, Splunk), EDR platform (CrowdStrike, Microsoft Defender), netflow analysis (Zeek). Hunt cycle: forma hipótesis → gather data → analyze → iterate. Diferencia detection engineering: detection = automated rules, hunting = human intuition + data. Effectiveness: hunting descubre elusive threats (slow burn persistence, advanced adversaries). Organizations: dedicate threat hunt team, quarterly hunts. Example hunt: \"Detect all PowerShell execution by NT AUTHORITY\\SYSTEM\" (system privilege execution unusual).", examples: ["Hunt hypothesis: adversary uses rare port 8899 for C2 → query firewall logs", "Finding: 3 machines conectan same IP:8899 with high egress data → investigate"], related: ["Detection", "Threat intelligence", "Proactive security"] },
  { id: 1028, module: 25, term: "Adversarial Emulation & Red Teaming", short: "Simulación de ataques para validar defenses", explanation: "Red team: simular adversario real contra organización (authorized). Emulation: usar MITRE ATT&CK techniques para replica adversario known. Tooling: Atomic Red Team (ejecuta técnicas), Caldera (full simulation platform), Cobalt Strike (simulation + C2). Phases: planning → recon → delivery → execution → cover tracks (similar real APT). Objectives: validar detections (¿EDR detecta técnica X?), gaps (qué técnicas no detectadas?), training (teach team). Outputs: report con findings + recommendations. Disclaimer: autorización escrita + scoping crítico (no afectar producción). Diferencia pentesting: pentest = find vulnerabilities, red team = realistic attack simulation (usar exploits ya sabidas).", examples: ["Emulation: simulate APT28 campaign against org (phishing → implant → persistence)", "Finding: EDR no detecta T1027.11 (Compile After Delivery) → add detection"], related: ["Testing", "Red team", "Threat intelligence"] },
  { id: 1029, module: 25, term: "Dark Web Monitoring & Incident Response", short: "Vigilancia de amenazas en la dark web", explanation: "Dark web (Tor network, I2P): hidden services (mercados, forums, C2 servers). Monitoring: hunt for leaked credentials, intellectual property, threat actor discussions. Tools: Shodan (search engines para connected devices), Censys (internet-wide scans), dark web monitors (record.com, Flashpoint, Recorded Future). Aplicación: credential breach notification (si email en leak → email owner), early warning (threats discussing targeting org). Desafíos: lenguaje (ruso, chino, etc.), opsec atacante (encrypted comms), falsos positivos (datos from public breaches reshared). Respuesta: breach → change password, MFA, monitor account. Privacy: no acceder dark web sin VPN/proxy, legal risk si owner combate ley, desensitive naming (report sin revelar victim identity).", examples: ["Monitor: alguna data found en marketplace → contact victim org", "Discussion forum: threat actors talking about 'targeting healthcare orgs' → alert healthcare ISACs"], related: ["Threat intelligence", "Breach monitoring", "Dark web"] },
  // ── M26 · Secure SDLC (definitions 1030-1039) ────────────────────────────
  { id: 1030, module: 26, term: "Secure Development Lifecycle (SDLC)", short: "Integración de seguridad en todo el ciclo de desarrollo", explanation: "Secure SDLC: procesos de seguridad desde planning hasta deployment. Fases: Design (threat modeling), Dev (secure coding), Testing (SAST/DAST), Deployment (hardening), Maintenance (patching). Antis: bolted-on security (agregar security al final = caro, tarde). NIST SSDF v1.1: 4 practice groups (Prepare org, Protect software, Produce well-formed, Prepare deployment). Microsoft SDL: 7 fases. BSA: Microsoft, Apple, Google todos publican SDLC practices. Shift-left: mover security testing a principios (design time, unit test) vs solo end-to-end. Culture: developers como first line of defense (security training, code review culture).", examples: ["Design: threat model identifies IDOR risk → dev implements authorization checks", "SAST: sonarqube scan detecta SQL injection antes de commit"], related: ["Secure coding", "Testing", "Development"] },
  { id: 1031, module: 26, term: "Secure Coding Practices & Guidelines", short: "Patrones y estándares de desarrollo seguro", explanation: "Secure coding: escribir code que previene vulnerabilidades comunes (OWASP Top 10, CWE Top 25). Principios: input validation (whitelist), output encoding (context-specific), least privilege (app user minimales permisos), defense in depth (multiple layers). Lenguaje-specific: C/C++ (buffer overflow risk, use safe string functions), Python (code injection via eval), JS (XSS vía DOM manipulation). Guidelines: OWASP Secure Coding Practices Cheat Sheet, CWE documentation. Code review: peer review crucial (detecta lógica flaws). Herramientas: linters (eslint, pylint), SAST (SonarQube, Fortify), automated code analysis en CI/CD pipeline.", examples: ["Anti-pattern: `eval(userInput)` en Python → use ast.literal_eval", "Pattern: user data never used en SQL query string → use parameterized queries"], related: ["Secure coding", "Vulnerability prevention", "Development"] },
  { id: 1032, module: 26, term: "SAST & Code Analysis Tools", short: "Análisis estático de seguridad en código fuente", explanation: "SAST (Static Application Security Testing): escanear source code sin ejecutar. Detecciones: hardcoded secrets, SQL injection patterns, XSS vulnerabilities, weak crypto. Herramientas: SonarQube (open + premium), Fortify (pago, industry standard), Checkmarx (enterprise), GitHub CodeQL (integrado en GitHub). Falsos positivos: common (40-60%), requiere tuning. Blind spots: lógica flaws (authorization, race conditions) difíciles detectar. Pipeline: SAST en pre-commit (local), CI/CD (gating PR), deploy time. Políticas: fail build si critical/high findings, unless approved + documented.", examples: ["Finding: hardcoded AWS key en source → secret rotated, key revoked", "Finding: sprintf without size check → potential buffer overflow → rewrite con safer function"], related: ["Testing", "Code security", "CI/CD"] },
  { id: 1033, module: 26, term: "DAST & Runtime Application Security", short: "Pruebas dinámicas de seguridad ejecutando la app", explanation: "DAST (Dynamic Application Security Testing): ejecutar aplicación + enviar traffic (requests, payloads) para detectar vulnerabilidades runtime. Detecta: injection flaws, authentication bypasses, session management issues (detectar by observing behavior). Herramientas: OWASP ZAP (gratuito, good for learning), Burp Suite (pago, industry standard), Acunetix (enterprise). Pruebas: crawler da descubrimiento (map app flows), scanner envía payloads (test cada endpoint). Challenge: falsos positivos alto (app requiere specific state), lento (cada test corre real transaction). Integración: DAST en staging (pre-production), no siempre posible en CI/CD (requiere live server).", examples: ["Finding: form field vulnerable a XSS → DAST payload='<script>alert(1)</script>' ejecutado → stored", "Finding: predictable session tokens → DAST correlates tokens temporales, detecta pattern"], related: ["Testing", "Vulnerability assessment", "Security"] },
  { id: 1034, module: 26, term: "Supply Chain Dependencies & Dependency Scanning", short: "Gestión de riesgos en librerías y dependencias", explanation: "Dependencies: aplicación usa librerías (npm, pip, gems, Maven). Risk: cada librería = attack surface (vulnerable version, malicious code). Scanning: herramientas buscan known CVEs en lock files (package-lock.json, go.sum). Herramientas: Snyk (commercial + free tier), OWASP Dependency-Check (free), Dependabot (GitHub), Sonatype Nexus (enterprise). Remediation: update a versión parched (puede romper compatibility), or patch internamente (fork + apply patch). Transitive: X depends on Y, Y has CVE → X indirectamente vulnerable (scanner debe descubrir). Policy: bloquear LGPL en proprietary, track license compliance.", examples: ["Snyk: app depends on log4j 2.14.1 (CVE-2021-44228 Log4Shell detected) → update 2.17.0+", "Transitive: app safe, pero dependency tree contiene vulnerable jackson-databind"], related: ["Supply chain", "Dependency management", "Vulnerability"] },
  { id: 1035, module: 26, term: "Threat Modeling in Design Phase", short: "Identificación de amenazas antes de implementar", explanation: "Threat modeling en design time vs. post-hoc. Beneficios: prevenir vulnerabilidades antes de código, más barato que post-deployment fix (dev time vs. forensics). Metodología: STRIDE o PASTA (ver M23). Artefactos: data flow diagrams (DFDs), process-level decomposition, trust boundaries. Output: threat list → mitigaciones → requirements de seguridad. Participantes: arquitecto, dev, security. Tools: Microsoft Threat Modeling Tool (free), Threagile (open), miro templates. Iterative: threat model actualizada con cambios de diseño. Seguimiento: verificar en code review que mitigaciones implementadas.", examples: ["Design: login API tiene trust boundary (cliente → API → DB). Threat: network eavesdropping → mitigation: TLS encrypt", "Threat: database compromise → mitigation: encryption at rest, IP allowlist"], related: ["Security design", "Architecture", "Threat assessment"] },
  { id: 1036, module: 26, term: "Secure Code Review & Peer Review Process", short: "Validación de código por pares con enfoque seguridad", explanation: "Code review: otro developer examina cambios antes de merge. Security-focused: buscar vulnerabilities, violated patterns, security assumptions. Checklist: input validation, output encoding, auth/authz, error handling (no sensitive info), crypto correctness. Tools: GitHub pull requests, GitLab merge requests, Gerrit (self-hosted). Efectividad: peer review detecta 20-40% bugs (mejor que testing solo). Culture: blameless, no finger-pointing, aprender juntos. Challenge: review burden (time-consuming), expertise (todos deben entender security). Automatización: SAST en PR puede flagging obvious patterns, reviewer enfocarse en lógica. SLA: critical security changes reviewed por senior dev.", examples: ["Review finds: input not validated → request changes before merge", "Review: crypto uses ECB mode (insecure) → ask to switch AES-CBC or GCM"], related: ["Code review", "Security", "Development culture"] },
  { id: 1037, module: 26, term: "Security Testing & Penetration Testing", short: "Pruebas ofensivas para encontrar vulnerabilidades", explanation: "Penetration testing (pentest): autorizado ataque simulado contra sistema para encontrar vulnerabilidades. Scope: cuál sistemas, qué tipos de tests (black-box, white-box), qué no tocar (production data destruction). Fases: recon (passive info gathering), scanning (active probing), exploitation (verifica vulnerabilidades), reporting. Diferencia DAST: pentest = humano intenta creativamente explotar, DAST = automated scanner patterns. Frequency: anual mínimo, antes de major releases. Vendor: contrata tercera parte (external eyes, unbiased), o red team interno. Output: severity ratings, remediation prioritization, evidence.", examples: ["Pentest finds: default credentials en admin panel → change immediately", "Pentest finds: SQL injection en search → dev patches + SAST añade rule"], related: ["Testing", "Security assessment", "Vulnerability"] },
  { id: 1038, module: 26, term: "Security Bug Bounty Programs", short: "Incentivizar a security researchers a encontrar bugs", explanation: "Bug bounty: ofrecer dinero a researchers que encuentren y reporten vulnerabilidades responsibly. Plataformas: HackerOne, Bugcrowd, Intigriti (facilitan relación). Policy: qué es in-scope (application, API, web), qué out-scope (social engineering, physical), responsible disclosure (90d grace period). Rewards: basado severidad (critical $5K+, high $1K, low $100). Incentivos: researchers construyen reputación, recognition, swag, hall of fame. Beneficios: encuentra bugs que inhouse testing missed, cheaper que pentest full-time. Riesgos: malicious submissions (scams), DOS attempts (attackers usando bounty programa como cover). Trust: security team investigates claims thoroughly.", examples: ["Bounty submission: XSS en user profile (stored) → researcher awarded $500 + public credit", "Invalid: 'server returned 404' no es vulnerability → declined"], related: ["Bug bounty", "Community security", "Crowdsourced testing"] },
  { id: 1039, module: 26, term: "Release Management & Security Baselines", short: "Control de cambios y hardening para cada release", explanation: "Release management: proceso formal de deployment (versioning, changelog, testing antes go-live). Security aspects: release notes documento CVE fixes, security updates, breaking changes. Baselines: security configuration mínima para cada release (CIS benchmark compliance). Hardening: disable unnecessary services, restrictive permissions, security updates baked into image. Version lifecycle: LTS (Long Term Support) vs. regular releases. Deprecation: old versions stop recibir patches (força upgrades). Comunicación: security advisory publicada con CVE numbers, severity, mitigation. Tracking: maintain compatibility matrix (qué versiones fixed qué CVEs).", examples: ["Release 2.1.0: fixes CVE-2024-1234 (RCE), release notes marca CRITICAL", "Baseline: TLS 1.2+ only, 2FA mandatory, encryption at rest", "EOL: version 1.0 reaches end-of-life → customers must upgrade"], related: ["Release management", "Patching", "Lifecycle"] },
  // ── M27 · Zero Trust Architecture (definitions 1040-1049) ────────────────────────────
  { id: 1040, module: 27, term: "Zero Trust Architecture Principles", short: "Paradigma de 'nunca confiar, siempre verificar'", explanation: "Zero Trust: no confiar automáticamente en nada (internal user, corporate network, etc.). Principios NIST (SP 800-207): (1) verify every access, (2) assume data is breach-able, (3) authenticate & authorize every session, (4) microsegment networks, (5) encrypt everything, (6) maintain audit logs, (7) take least privilege approach. Différencia legacy: perimeter-based (firewall border = trust boundary) vs. zero trust (every request verificado). Implementación: identity-centric (autenticación fuerte), micro-segmentation (red dividida en zonas), continuous verification (behavioral analysis). Frameworks: Google BeyondCorp, Gartner Zero Trust maturity model. Adoption: gradual (perimeter first, luego internal).", examples: ["Zero trust: user desde cafe con laptop → requiere MFA + location check + device posture", "Micro-segment: frontend only speaks a backend service X (no to Y/Z) via network policy"], related: ["Network security", "Identity", "Access control"] },
  { id: 1041, module: 27, term: "Identity & Access Management (IAM)", short: "Control centralizado de identidades y permisos", explanation: "IAM: sistema para autenticar + autorizar usuarios/servicios. Componentes: identity provider (Okta, Azure AD), policy engine (RBAC, ABAC, PBAC), session manager. Autenticación: verify identidad (MFA, passwordless). Autorización: verify permiso (acceso a recurso específico). Policies: qué roles pueden hacer qué en qué recurso (Principle of Least Privilege). RBAC (Role-Based): admin, user, guest roles. ABAC (Attribute-Based): permisos basados en atributos (IP, device type, time of day, location). Integración: empresa usa Okta/Azure AD para SSO (single sign-on) a todas apps. Audit: log quién accedió qué cuándo (audit trail).", examples: ["RBAC: 'admin' role puede delete users, 'user' role no", "ABAC: 'access customer data' IF location=US AND device=corporate AND time=business-hours"], related: ["Access control", "Authentication", "Authorization"] },
  { id: 1042, module: 27, term: "Microsegmentation & Network Segmentation", short: "División de red en zonas de seguridad aisladas", explanation: "Segmentación: dividir red grande en zonas pequeñas (segmentos) con control de tráfico entre ellos. Tradicional: DMZ (perimeter) vs. internal network (flat, trust interno). Micro-segmentation: cada app/tier es zona (frontend, backend, DB, admin). Beneficios: reduce blast radius (si frontend breached, backend no immediately accessible), contiene lateral movement. Implementación: network policies (K8s), firewall rules, VLAN, SDN (Software-Defined Network). Desafíos: complejidad (muchas reglas = administration heavy), visibilidad (entender traffic flows). Tools: Illumio (micro-segmentation), Cisco, VMware. Default-deny: bloquea todo by default, whitelist tráfico permitido.", examples: ["Segment 1 (public): web servers. Segment 2 (internal): app servers (solo recibe from segment 1)", "Segment 3 (protected): database (solo recibe from app servers, no de web)"], related: ["Network security", "Zero trust", "Defense in depth"] },
  { id: 1043, module: 27, term: "Device Posture & Compliance Checks", short: "Verificación de estado de seguridad del dispositivo", explanation: "Device posture: verificar dispositivo está secure antes de permitir acceso. Checks: OS patched?, antivirus running?, disk encrypted?, no jailbroken/rooted?. Condicional access: permite acceso solo si device posture es bueno (sino requiere remediation). Herramientas: MDM (Mobile Device Management: Intune, Jamf), device compliance policies, UEM (Unified Endpoint Management). Enforcement: app rechaza acceso si device no cumple (nadie puede bypass). Enterprise: BYOD (Bring Your Own Device) requiere device check más estricto que company devices. Cloud access: antes de acceso a resources, verifica device state (healthcheck endpoint).", examples: ["Conditional access: 'if device OS outdated → require password reset before accessing Salesforce'", "Remediation: device detectado no encrypted → user must enable BitLocker before acceso"], related: ["Device security", "Access control", "Compliance"] },
  { id: 1044, module: 27, term: "Behavioral Analytics & Anomaly Detection", short: "Detección de actividad inusual basada en comportamiento", explanation: "Behavioral analytics: aprender qué es comportamiento normal (baseline) para usuario/device, detectar anomalías. Anomalías: login desde país inusual, acceso a datos sensibles post-hours, bajadas grandes de datos. Tools: UEBA (User and Entity Behavior Analytics): Gurucul, Exabeam, Azure AD Threat Intelligence. Algoritmos: statistical baselines, machine learning (temporal patterns, peer groups). Falsos positivos: común (user travel, job change), tuning necesario. Response: adaptive MFA (rechaza acceso o pide extra verification), slow down risky actions (admin approve). Privacy: comportamiento tracking despierta concerns, requiere transparency + policies.", examples: ["Alert: user normally logs from US, hoy from Russia 2 hours ago → challenge con MFA", "Alert: user acceding customer PII database después horas → alert security team"], related: ["Detection", "Anomaly detection", "Behavioral security"] },
  { id: 1045, module: 27, term: "Encryption & Key Management (Zero Trust Perspective)", short: "Cifrado obligatorio en tránsito y reposo (ZT)", explanation: "Zero Trust encryption: cifra TODAS las comunicaciones (no solo sensitive). Tránsito: TLS 1.2+ para todo (HTTP→HTTPS, internal services mTLS). Reposo: todo datos cifrado en disk/DB (AES-256-GCM). Gestión de claves: centralizada (KMS = Key Management Service), rotación automática, access logged. Desafíos: performance (cifrado es CPU-intensive), monitoring (cifrado end-to-end = logs ciegos si no structured). Implementación: TLS everywhere (enforce via network policies, block plaintext), encryption at rest (Kubernetes secret encryption, S3 SSE), key rotation automática. Challenge: intra-service comms (microservices mTLS mutual authentication agrega complexity).", examples: ["Zero Trust: even internal service A→B communication encrypted via mTLS", "KMS: rotation cada 90d automática, previous keys retained para decryption de old data"], related: ["Cryptography", "Data protection", "Access control"] },
  { id: 1046, module: 27, term: "Continuous Authentication & Reauthentication", short: "Verificación continua de sesión en lugar de one-time login", explanation: "Continuous auth: no confiar en login único (session token perpetuo). Verificación continua: cada X minutos o cada acción sensible, reverify identidad. Métodos: adaptive auth (pide MFA si risk score alto), session timeout (5-15 min inactivity), device re-check. Transacciones: acciones críticas requieren re-auth (cambiar password, delete records). Implementación: challenge-response (push notification to phone), passwordless (fido2 key), facial recognition. User experience: balance seguridad vs friction (request MFA each minute = frustrate users). Fallback: si device offline, cached credentials + verify cuando vuelve online.", examples: ["Sensitive: deleting user account requires MFA verification (not just session token)", "Inactivity: session expires después 15 min sin activity → require login again"], related: ["Authentication", "Zero trust", "Identity"] },
  { id: 1047, module: 27, term: "BeyondCorp & Google's Zero Trust Model", short: "Arquitectura de confianza cero implementada por Google", explanation: "BeyondCorp: Google's internalized Zero Trust model (2011-2015). Conceptos: (1) perimeter menos importante (VPN muere, no network layer trust), (2) acceso basado en device + user identity (not network IP), (3) authenticate to service no a network. Arquitectos: Access Proxy (reverse proxy que valida access), Device Inventory (todo devices tracked), Identity Provider (auth centralizada). Ventajas: permite remote work (no VPN), reduce complexity (no perimeter hardening). Desventajas: requires cloud infrastructure (no on-premise friendly), migration painful para legacy. Papers: Google publicó 6 papers detallando architecture + lessons learned. Industry adoption: Stripe, GitHub, otros siguen BeyondCorp design.", examples: ["BeyondCorp: no VPN → employee connects directly al access proxy con strong auth", "Access Proxy: verifies device posture, identity, location before granting access"], related: ["Zero trust", "Architecture", "Access control"] },
  { id: 1048, module: 27, term: "Service-to-Service (mTLS) Authentication", short: "Autenticación mutua entre microservicios", explanation: "mTLS (mutual TLS): both client AND server verify cada otros (mutual authentication). Diferencia TLS normal: server autentica, client no (web browsing). Implementación: cada service tiene certificate (issued by internal CA). Handshake: client envía cert → server verifica, server envía cert → client verifica. Beneficios: previene rogue service (si credentials leaked, rogue service sin cert válido puede acceder), encrypted channel. Tools: Istio (service mesh), Linkerd, Consul (mTLS por defecto). Operacional: certificate rotation automática (mesi expired antes deploy a production). Desafío: debugging (encrypted = logs ciegos sin structured logging).", examples: ["Service A→B: A presents cert signed by corporate CA. B verifies cert authenticity", "Rogue service intenta suplantar A sin cert valid → B rechaza connection"], related: ["Authentication", "Cryptography", "Microservices"] },
  { id: 1049, module: 27, term: "Zero Trust Governance & Policy Enforcement", short: "Control centralizado y auditoría en arquitectura zero trust", explanation: "Policy: declarativo (qué está permitido) vs imperativo (cómo ejecutarlo). Central policy engine: define permisos globales (identidad X puede acceder recurso Y si conditions Z). Enforcement: network policies, RBAC, conditional access (automatizada). Audit: log TODAS las decisiones (permitido, denegado, por qué). Frameworks: NIST SP 800-207 governance recommendations, ISO 27001. Challenge: policy explosion (100s de reglas), conflict resolution (regla A vs B conflict). DevOps: policy-as-code (Terraform, Rego/OPA language) para version control + review. Compliance: policies tracked → audit reports demuestran compliance.", examples: ["Policy: 'dev team puede read logs, no delete'. Enforced by RBAC en logging service", "Policy-as-code: rego rule define 'only corp devices can access customer data'"], related: ["Policy", "Governance", "Compliance"] },
  // ── M28 · Governance, Risk & Compliance (definitions 1050-1059) ────────────────────────────
  { id: 1050, module: 28, term: "Governance Frameworks (ISO 27001, SOC 2, COBIT)", short: "Estándares de gestión de seguridad e información", explanation: "ISO 27001: estándar internacional para sistemas de gestión de seguridad (ISMS). Requerimientos: política, roles, risk assessment, controls, audit, improvement. SOC 2 (Service Organization Control): auditoría externa de cloud services (criterios: security, availability, processing integrity, confidentiality, privacy). COBIT (Control Objectives for Information Technology): framework de governance de IT. Mapeo: ISO 27001 = 114 controls, SOC 2 = 5 trust service categories, COBIT = 40 objetivos. Cumplimiento: necesita tercera parte (auditor) verificación anual. Diferencias: ISO = comprehensive, SOC 2 = cloud-focused, COBIT = IT governance. Adopción: startup puede usar ISO checklist como baseline, enterprise → compliance tracking tool (Archer, Gallup).", examples: ["ISO 27001: A.6 Organization of information security (6 controls sobre policies, roles)", "SOC 2 Type II: audit covers 6+ months operación (verifica controls working, not just defined)"], related: ["Compliance", "Governance", "Risk management"] },
  { id: 1051, module: 28, term: "Regulatory Compliance (GDPR, HIPAA, PCI-DSS, SOX)", short: "Cumplimiento de leyes y regulaciones por industria", explanation: "GDPR (EU, personal data): derechos de data subject (access, delete, portability). HIPAA (US healthcare): protección de PHI (Protected Health Information). PCI-DSS (payment cards): 12 requerimientos para almacenar/procesar card data. SOX (US, público companies): controls sobre financial reporting (IT también impactado). CCPA (California personal data): similar GDPR US-scope. Implicaciones: breach notification, fines big (GDPR 4% revenue or €20M), data subject rights. Implementación: data mappings, processing agreements (DPA), subprocessor audits, incident response plans. Variedad: cada región/industria tiene requerimientos distintos → multinacional = compliance complex.", examples: ["GDPR: user right to be forgotten → delete all personal data within 30 days", "PCI-DSS: no almacenar full credit card number (solo last 4 digits OK)", "HIPAA: unauthorized access to medical records = criminal liability"], related: ["Compliance", "Regulations", "Data protection"] },
  { id: 1052, module: 28, term: "Compliance Audits & Assessments", short: "Verificación externa de cumplimiento normativo", explanation: "Auditoría externa: third-party auditor verifica compliance contra estándar (ISO 27001 audit, SOC 2 audit, PCI-DSS assessment). Tipos: initial (first time), surveillance (yearly SOC 2), full (tri-yearly ISO). Process: scoping (qué systems), planning (timeline), fieldwork (interviews, document review, testing), reporting (findings, remediation timeline). Gap findings: non-compliant areas, criticidad varies. Remediación: must fix critical before cert issued, medium/low = remediation plan. Re-audit: post-remediation verification. Cost: auditor fees $ per diem, interno staff effort. Records: audit reports guardan para compañía reference + customer RFPs.", examples: ["SOC 2 audit: auditor tests access controls → finds 1 non-compliant bucket without MFA → require fix", "ISO 27001 audit: testing access logs, change management, incident response, física security"], related: ["Compliance", "Audit", "Governance"] },
  { id: 1053, module: 28, term: "Risk Management & Risk Registers", short: "Documentación sistemática de riesgos organizacionales", explanation: "Risk register: spreadsheet/tool que lista todos los riesgos identificados. Por cada risco: descripción, propietario, probabilidad, impacto, score (likelihood × impact), estado (open/mitigated/accepted), mitigation plan, deadline. Periodicidad: identificar riesgos continuamente, revisar registro trimestral. Escalación: riesgos críticos → board reporting. Tracking: riesgos residuales (post-mitigation) vs. total (pre-mitigation). Elementos: risk universe (completitud), control mapping (si existe control mitigation), audit status (verificado?). Reporting: executive summary de top 10 risks, trending (nuevo risks, resolved). Desafíos: risk fatigue (demasiados riesgos, menos atención), over-scoring (todos critical = meaningless prioritization).", examples: ["Risk: unpatched systems → prob=medium, impact=high (ransomware risk) → mitigation: patch management", "Risk register dashboard: 45 open risks, 12 resolved this quarter, trend: decreasing"], related: ["Risk management", "Governance", "Audit"] },
  { id: 1054, module: 28, term: "Incident Reporting & Regulatory Notification", short: "Reporte obligatorio de brechas a reguladores y afectados", explanation: "Breach notification: si datos personales comprometidos, notificar afectados + reguladores. Timelines: GDPR 72h a reguladores (sin demora, si 72h impracticable = asap), California 60d. Contenido: qué data, quién accedió, cómo se descubrió, remediation actions. Channels: GDPR → data protection authority (local), users → email/mail, HIPAA → HHS (US), PCI-DSS → payment processors + cardholders. Cost: notificación catica (mailing), credit monitoring para víctimas (tercera parte pays), reputation damage. Preparación: pre-draft template (llenadores in a breach), legal review proceso, contact list regulators. Implicación: no notificar = fines adicionales (GDPR +2% revenue).", examples: ["Breach notification: 'Unauthorized access detected 2024-01-15. 50K customers affected. Full name, email, SSN (last 4 digits).'", "GDPR: breach notified regulators 2024-01-16 (72h limit). User notification sent 2024-01-17"], related: ["Incident response", "Compliance", "Breach notification"] },
  { id: 1055, module: 28, term: "Data Protection Impact Assessments (DPIA)", short: "Evaluación anticipada de riesgos de privacidad en proyectos", explanation: "DPIA (Data Protection Impact Assessment): GDPR requerimiento. Triggering: si procesamiento alto-riesgo (large-scale data, automated decision, vulnerable people). Proceso: describir procesamiento (qué data, para qué, quién tiene acceso), identificar riesgos, proponer mitigaciones. Outcome: puedes proceder, o requiere risk mitigation antes. Stakeholders: DPO (Data Protection Officer) típicamente conduce, involves dev/product. Documentación: DPIA report retenida para audit. Herramientas: GDPR DPA templates, ISO checklist. Preventivo: DPIA al inicio de proyecto, no post-facto (changes expensive después).", examples: ["DPIA: new facial recognition feature. Trigger: GDPR high risk (biometric data). Outcome: proceed con mitigations (consent, storage limit, transparency)", "DPIA: customer data sharing con third party. Risk: data access broad → mitigation: DPA, contractual restrictions"], related: ["Privacy", "Data protection", "Compliance"] },
  { id: 1056, module: 28, term: "Metrics, KPIs & Security Dashboards", short: "Cuantificación del postura de seguridad", explanation: "Metrics: medidas de postura (% sistemas patched, # vulnerabilities open, incident response time, MTTR). KPIs (Key Performance Indicators): métricas importante para negocio (reduce incidents 50%, mean time to detect <1h). Ventajas: objectivos, trend tracking, evidence de improvement. Desafíos: gaming metrics (report only part of truth), vanity metrics (looks good pero no significa nada). Security KPIs examples: vulnerability remediation rate, patch rate, training completion, incident detection time, false positive ratio (SIEM). Reporting: dashboard/report mensual/trimestral (board visibility). Caution: riesgos decrease no lineal (primeros fixes cheaper, últimos harder). Balancing: too many metrics = confusing, too few = miss blind spots.", examples: ["KPI: 'mean time to remediate critical vulns: 7 days' → target 5d → achieved 4.2d this quarter", "Dashboard: 89% sistemas patched (target 95%), 23 open vulns (target <10)"], related: ["Governance", "Measurement", "Reporting"] },
  { id: 1057, module: 28, term: "Data Retention & Disposal Policies", short: "Ciclo de vida de datos desde creación a eliminación", explanation: "Data lifecycle: creation → usage → retention → disposal. Retention policies: guardar datos solo el tiempo necesario (GDPR: purpose limitation, storage limitation). Ejemplos: logs 90d, backup 1 year, customer data while customer active, then 30d grace period. Disposal: borrar securely (no solo rm, debe ser cryptographically irrecoverable). Métodos: DBAN (Darik's Boot and Nuke), secure delete tools, vendor data destruction certificate. Regulatoria: GDPR penaliza retention excesiva, PCI-DSS require deletion de card data post-auth. Challenge: dependencies (datos en backup, replication, archives difíciles trackear). Automation: policias automáticas (DB scheduled delete + archivo), Kubernetes GC (garbage collection).", examples: ["Retention policy: logs deleted after 90d per compliance, except if breach under investigation", "Disposal: decommissioned hard drive → send to certified destruction vendor, get CoD (Certificate of Destruction)"], related: ["Data lifecycle", "Compliance", "Privacy"] },
  { id: 1058, module: 28, term: "Third-Party Risk Management & Vendor Security", short: "Evaluación y monitoreo continuo de proveedores", explanation: "Third-party risk: heredas la seguridad de vendors. Ej: if payment processor breached → customers' data exposed. Assessment: vendor questionnaire (ISO 27001?, penetration test?, SLAs?), audit report (SOC 2), references calls. Due diligence: crítica para cloud/SaaS (deep integration). Continuous: contract includes security audit rights, incident notification SLA (24-48h). Escrow: fuente código escrow (si vendor bankruptcy, tienes código). Liability: contract deve specify liability cap, indemnification, notification obligations. Risk tiers: Tier 1 (crítica: payment, auth) = máxima scrutiny, Tier 3 (office tools) = mínimo. Reasessment: anual risk assessment vendors.", examples: ["Vendor assessment: SaaS payment processor → SOC 2 Type II report, 99.95% uptime SLA, 24h breach notification", "Escrow agreement: if vendor dies, code released bajo NDA (competidores can't access)"], related: ["Supply chain", "Vendor management", "Risk management"] },
  { id: 1059, module: 28, term: "Security Culture & Awareness Training", short: "Desarrollo de mentalidad y conocimiento de seguridad", explanation: "Security culture: toda la org entiende importancia seguridad, reporta risks/incidents, actúa securely. Training obligatorio: OWASP Top 10 developers, phishing simulations usuarios, compliance training. Efectividad: awareness reduce human-caused incidents (phishing, accidental data exposure). Medidas: quiz post-training, simulated phishing campaigns (% que clickea = metric), security champions (employees advocates). Incentivos: recognition, competitions (bug bounty interna), integrado performance reviews. Recompensas: employee que descubre vulnerability = reward, positivo tone (vs punishment). Desafíos: compliance mindset (checkbox training) vs genuine understanding. Leadership: CISO/security team must model behavior (security first, no shortcuts).", examples: ["Phishing simulation: 30% empleados clicked link (too high) → targeted training → retry: 8% (improved)", "Security awareness month: daily tip, lunch-and-learn talks, swag (mugs, stickers)"], related: ["Awareness", "Training", "Culture"] },
];
];
