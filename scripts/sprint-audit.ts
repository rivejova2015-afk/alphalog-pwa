...
function findRoutes(baseDir: string = 'src/app'): string[] {
  const routes: string[] = [];
...
function findEndpoints(apiDir: string = 'src/app/api'): string[] {
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (stat.isDirectory()) {
          ...
          const routeFile = path.join(fullPath, 'route.ts') || path.join(fullPath, 'route.js');
...
function findMigrations(migrationsDir: string = 'supabase/migrations'): string[] {
...
function findEdgeFunctions(functionsDir: string = 'supabase/functions'): string[] {
...
function determineSprints(
  ...
  const coveredSprints = new Set<number>();
  ...
  return Array.from(coveredSprints).sort();
}
...
function generateMarkdownReport(auditResult: AuditResult): string {
...
See \`scripts/sprint-audit.ts\` for details.
`;
